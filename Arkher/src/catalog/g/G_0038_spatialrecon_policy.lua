-- ARKHER SYSTEM G.0038 :: Spatial Reconstruction Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0038"
	S.key = "arkher.neural.spatialrecon.heuristic_policy"
	S.name = "Spatial Reconstruction Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.spatialrecon.result_cache" }
	S.tags = { "g", "spatialrecon", "policy", "neural" }
	S.description = "Spatial Reconstruction Heuristic Policy: rule fallback when the model is unavailable for the Spatial Reconstruction subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.spatialrecon.heuristic_policy", algorithm = "edf", starvationGuard = 61 })
		inst.system = S
		inst.ctx = ctx
