--!strict
-- OpenWeather — ANIMATION Profiler #0079
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="OpenWeather — ANIMATION Profiler #0079", size=Vector2.new(640, 500), pos=UDim2.fromOffset(327, 123), parent=parent, icon="⚙"})
...
Components.TextField(form, "Name", "Object_"..79, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
