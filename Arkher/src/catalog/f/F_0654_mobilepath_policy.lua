-- ARKHER SYSTEM F.0654 :: Mobile Fast Path Render Policy
...
return function(A)
	...
	local S = {}
	S.id = "F.0654"
	S.key = "arkher.render.mobilepath.render_policy"
	S.name = "Mobile Fast Path Render Policy"
	S.category = "F"
	S.family = "RENDERING"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.render.mobilepath.draw_budget" }
	S.tags = { "f", "mobilepath", "policy", "render" }
	S.description = "Mobile Fast Path Render Policy: device tier rules for the frame for the Mobile Fast Path subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 560,
		baseWeight = 0.52,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.render.mobilepath.render_policy", algorithm = "wfq", starvationGuard = 62 })
		inst.system = S
		inst.ctx = ctx
