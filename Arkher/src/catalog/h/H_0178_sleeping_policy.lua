-- ARKHER SYSTEM H.0178 :: Sleeping Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0178"
	S.key = "arkher.physics.sleeping.simulation_policy"
	S.name = "Sleeping Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Sleeping"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.sleeping.budget_governor" }
	S.tags = { "h", "sleeping", "policy", "physics" }
	S.description = "Sleeping Simulation Policy: device tier rules for substeps, iterations and sleep for the Sleeping subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 200,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.sleeping.simulation_policy", algorithm = "edf", starvationGuard = 41 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.sleeping.*", function(payload) inst.lastSignal = payload end)
			end
