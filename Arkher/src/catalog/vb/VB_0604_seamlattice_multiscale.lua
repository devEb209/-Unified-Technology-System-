-- ARKHER SYSTEM VB.0604 :: Seam Lattice Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0604"
	S.key = "arkher.conttime.seamlattice.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.seamlattice.sparse_residency" }
	S.tags = { "vb", "seamlattice", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.seamlattice.multi-scale_fidelity", levels = 6, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.seamlattice.*", function(payload) inst.lastSignal = payload end)
			end
