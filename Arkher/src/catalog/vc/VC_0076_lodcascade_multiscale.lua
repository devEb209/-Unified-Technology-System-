-- ARKHER SYSTEM VC.0076 :: LOD Cascade Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0076"
	S.key = "arkher.contneural.lodcascade.multi-scale_fidelity"
	S.name = "LOD Cascade Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.lodcascade.sparse_residency" }
	S.tags = { "vc", "lodcascade", "multiscale", "contneural" }
	S.description = "LOD Cascade Multi-Scale Fidelity: distance-driven multi-scale level selection for the LOD Cascade subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 320,
		baseWeight = 0.86,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.lodcascade.multi-scale_fidelity", levels = 5, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.lodcascade.*", function(payload) inst.lastSignal = payload end)
			end
