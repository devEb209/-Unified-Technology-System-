-- ARKHER SYSTEM O.0003 :: Footstep Spatialization
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0003"
	S.key = "arkher.audio.footstep.spatialization"
	S.name = "Footstep Spatialization"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Footstep"
	...
	S.kit = "spatialaudio"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.footstep.mixer_bus" }
	S.tags = { "o", "footstep", "spatialaudio", "audio" }
	S.description = "Footstep Spatialization: distance attenuation, panning, occlusion and doppler for the Footstep subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.64,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("spatialaudio", { id = "arkher.audio.footstep.spatialization", model = "exponential", refDistance = 5, maxDistance = 160, rolloff = 1.04, maxVoices = 20 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.footstep.*", function(payload) inst.lastSignal = payload end)
			end
