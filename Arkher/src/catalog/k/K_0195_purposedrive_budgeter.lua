-- ARKHER SYSTEM K.0195 :: Purpose Drive Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0195"
	S.key = "arkher.npc.purposedrive.thinking_budget"
	S.name = "Purpose Drive Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Purpose Drive"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.purposedrive.daily_schedule" }
	S.tags = { "k", "purposedrive", "budgeter", "npc" }
	S.description = "Purpose Drive Thinking Budget: allocation of per-frame thinking time across NPCs for the Purpose Drive subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 560,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.purposedrive.thinking_budget", total = 190, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.purposedrive.*", function(payload) inst.lastSignal = payload end)
			end
