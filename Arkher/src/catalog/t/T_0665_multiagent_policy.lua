-- ARKHER SYSTEM T.0665 :: Multi Agent Coordination Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0665"
	S.key = "arkher.ai.multiagent.decision_policy"
	S.name = "Multi Agent Coordination Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.multiagent.reasoning_graph" }
	S.tags = { "t", "multiagent", "policy", "ai" }
	S.description = "Multi Agent Coordination Decision Policy: ordered rules deciding what the agent does next for the Multi Agent Coordination subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 320,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.multiagent.decision_policy", algorithm = "roundrobin", starvationGuard = 32 })
		inst.system = S
		inst.ctx = ctx
