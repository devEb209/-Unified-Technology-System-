-- ARKHER SYSTEM O.0032 :: Weapon Sound Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0032"
	S.key = "arkher.audio.weaponsound.compression_codec"
	S.name = "Weapon Sound Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Weapon Sound"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.weaponsound.asset_registry" }
	S.tags = { "o", "weaponsound", "codec", "audio" }
	S.description = "Weapon Sound Compression Codec: payload encoding of audio state and events for the Weapon Sound subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 360,
		baseWeight = 0.55,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.weaponsound.compression_codec", format = "quantized", quantBits = 9 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.weaponsound.*", function(payload) inst.lastSignal = payload end)
			end
