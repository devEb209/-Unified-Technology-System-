-- ARKHER SYSTEM P.0273 :: Reputation Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0273"
	S.key = "arkher.play.playerreputation.difficulty_controller"
	S.name = "Reputation Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Reputation"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.playerreputation.balance_analysis" }
	S.tags = { "p", "playerreputation", "controller", "play" }
	S.description = "Reputation Difficulty Controller: closed-loop tuning toward a target challenge for the Reputation subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 200,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.playerreputation.difficulty_controller", mode = "hysteresis", target = 0.630, kp = 0.380, ki = 0.090, kd = 0.040, min = 0.05, max = 1.0, initial = 0.53, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.playerreputation.*", function(payload) inst.lastSignal = payload end)
			end
