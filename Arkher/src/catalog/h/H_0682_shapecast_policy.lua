-- ARKHER SYSTEM H.0682 :: Shapecast Query Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0682"
	S.key = "arkher.physics.shapecast.simulation_policy"
	S.name = "Shapecast Query Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Shapecast Query"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.shapecast.budget_governor" }
	S.tags = { "h", "shapecast", "policy", "physics" }
	S.description = "Shapecast Query Simulation Policy: device tier rules for substeps, iterations and sleep for the Shapecast Query subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 200,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.shapecast.simulation_policy", algorithm = "edf", starvationGuard = 41 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.shapecast.*", function(payload) inst.lastSignal = payload end)
			end
