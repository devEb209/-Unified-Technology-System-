-- ARKHER SYSTEM O.0332 :: Engine Audio Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0332"
	S.key = "arkher.audio.engineaudio.compression_codec"
	S.name = "Engine Audio Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Engine Audio"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.engineaudio.asset_registry" }
	S.tags = { "o", "engineaudio", "codec", "audio" }
	S.description = "Engine Audio Compression Codec: payload encoding of audio state and events for the Engine Audio subsystem."
	S.params = {
		backlogLimit = 47,
		baseRadius = 280,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.engineaudio.compression_codec", format = "json", quantBits = 15 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.engineaudio.*", function(payload) inst.lastSignal = payload end)
			end
