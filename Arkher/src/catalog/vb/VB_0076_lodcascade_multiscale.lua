-- ARKHER SYSTEM VB.0076 :: LOD Cascade Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0076"
	S.key = "arkher.conttime.lodcascade.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.lodcascade.sparse_residency" }
	S.tags = { "vb", "lodcascade", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.lodcascade.multi-scale_fidelity", levels = 4, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.lodcascade.*", function(payload) inst.lastSignal = payload end)
			end
