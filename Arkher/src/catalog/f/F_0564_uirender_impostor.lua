-- ARKHER SYSTEM F.0564 :: UI Rendering Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0564"
	S.key = "arkher.render.uirender.geometry_virtualization"
	S.name = "UI Rendering Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "UI Rendering"
	...
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.uirender.visibility_set" }
	S.tags = { "f", "uirender", "impostor", "render" }
	S.description = "UI Rendering Geometry Virtualization: impostor and HLOD synthesis under a budget for the UI Rendering subsystem."
	S.params = {
		backlogLimit = 37,
		baseRadius = 200,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.uirender.geometry_virtualization", atlasSlots = 48, viewCount = 8, errorThreshold = 2.90 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.uirender.*", function(payload) inst.lastSignal = payload end)
			end
