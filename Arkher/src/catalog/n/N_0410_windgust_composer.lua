-- ARKHER SYSTEM N.0410 :: Wind Gust Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0410"
	S.key = "arkher.vfx.windgust.effect_composition"
	S.name = "Wind Gust Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Wind Gust"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.windgust.effect_graph" }
	S.tags = { "n", "windgust", "composer", "vfx" }
	S.description = "Wind Gust Effect Composition: blending of several effect signals into one output for the Wind Gust subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 160,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.windgust.effect_composition", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.windgust.*", function(payload) inst.lastSignal = payload end)
			end
