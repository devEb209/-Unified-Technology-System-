-- ARKHER SYSTEM K.0125 :: Forgetting Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0125"
	S.key = "arkher.npc.forgetting.thinking_budget"
	S.name = "Forgetting Thinking Budget"
	S.category = "K"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.forgetting.daily_schedule" }
	S.tags = { "k", "forgetting", "budgeter", "npc" }
	S.description = "Forgetting Thinking Budget: allocation of per-frame thinking time across NPCs for the Forgetting subsystem."
	S.params = {
		backlogLimit = 34,
		baseRadius = 240,
		baseWeight = 0.66,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.forgetting.thinking_budget", total = 126, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.forgetting.*", function(payload) inst.lastSignal = payload end)
			end
