-- ARKHER SYSTEM I.0006 :: Skeleton Rig Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0006"
	S.key = "arkher.anim.skeletonrig.retarget_graph"
	S.name = "Skeleton Rig Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Skeleton Rig"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.skeletonrig.physical_blend" }
	S.tags = { "i", "skeletonrig", "graph", "anim" }
	S.description = "Skeleton Rig Retarget Graph: bone mapping and retarget path resolution for the Skeleton Rig subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 240,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.skeletonrig.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.skeletonrig.*", function(payload) inst.lastSignal = payload end)
			end
