-- ARKHER SYSTEM P.0237 :: Quest Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0237"
	S.key = "arkher.play.questsys.difficulty_controller"
	S.name = "Quest Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Quest"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.questsys.balance_analysis" }
	S.tags = { "p", "questsys", "controller", "play" }
	S.description = "Quest Difficulty Controller: closed-loop tuning toward a target challenge for the Quest subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 360,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.questsys.difficulty_controller", mode = "hysteresis", target = 0.550, kp = 0.300, ki = 0.100, kd = 0.010, min = 0.05, max = 1.0, initial = 0.45, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.questsys.*", function(payload) inst.lastSignal = payload end)
			end
