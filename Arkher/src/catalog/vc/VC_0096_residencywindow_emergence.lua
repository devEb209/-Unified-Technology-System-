-- ARKHER SYSTEM VC.0096 :: Residency Window Emergence Continuum
...
return function(A)
	...
	local S = {}
	S.id = "VC.0096"
	S.key = "arkher.contneural.residencywindow.emergence_continuum"
	S.name = "Residency Window Emergence Continuum"
	S.category = "VC"
	...
	S.kit = "emergence"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.residencywindow.reality_continuum" }
	S.tags = { "vc", "residencywindow", "emergence", "contneural" }
	S.description = "Residency Window Emergence Continuum: emergence detection over the continuum for the Residency Window subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.9,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("emergence", { id = "arkher.contneural.residencywindow.emergence_continuum", windowSize = 6, threshold = 1.20, minCount = 2 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.residencywindow.*", function(payload) inst.lastSignal = payload end)
			end
