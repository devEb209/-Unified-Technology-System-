-- ARKHER SYSTEM VB.0472 :: VFX Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0472"
	S.key = "arkher.conttime.vfxcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.vfxcont.sparse_residency" }
	S.tags = { "vb", "vfxcont", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.vfxcont.multi-scale_fidelity", levels = 4, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.vfxcont.*", function(payload) inst.lastSignal = payload end)
			end
