-- ARKHER SYSTEM W.0150 :: Point Of View Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0150"
	S.key = "arkher.cine.pointofview.playback_orchestrator"
	S.name = "Point Of View Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Point Of View"
	S.aspect = "Playback Orchestrator"
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.pointofview.cut_policy" }
	S.tags = { "w", "pointofview", "orchestrator", "cine" }
	S.description = "Point Of View Playback Orchestrator: the state machine that drives a cinematic from start to end for the Point Of View subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 400,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.pointofview.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.pointofview.*", function(payload) inst.lastSignal = payload end)
			end
