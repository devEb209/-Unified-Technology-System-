-- ARKHER SYSTEM O.0392 :: Accessibility Audio Compression Codec
-- Category O - AUDIO
-- ARKHER Audio Framework capability: buses, DSP, spatialization and music that reacts to the world.
-- Kit: codec (payload encoding of audio state and events)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "O.0392"
	S.key = "arkher.audio.audioaccessibility.compression_codec"
	S.name = "Accessibility Audio Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Accessibility Audio"
	S.aspect = "Compression Codec"
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.audioaccessibility.asset_registry" }
	S.tags = { "o", "audioaccessibility", "codec", "audio" }
	S.description = "Accessibility Audio Compression Codec: payload encoding of audio state and events for the Accessibility Audio subsystem."
	S.params = {
		backlogLimit = 19,
		baseRadius = 280,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.audioaccessibility.compression_codec", format = "json", quantBits = 11 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.audioaccessibility.*", function(payload) inst.lastSignal = payload end)
			end
