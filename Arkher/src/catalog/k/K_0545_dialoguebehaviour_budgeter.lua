-- ARKHER SYSTEM K.0545 :: Dialogue Behaviour Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0545"
	S.key = "arkher.npc.dialoguebehaviour.thinking_budget"
	S.name = "Dialogue Behaviour Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Dialogue Behaviour"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.dialoguebehaviour.daily_schedule" }
	S.tags = { "k", "dialoguebehaviour", "budgeter", "npc" }
	S.description = "Dialogue Behaviour Thinking Budget: allocation of per-frame thinking time across NPCs for the Dialogue Behaviour subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 200,
		baseWeight = 0.57,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.dialoguebehaviour.thinking_budget", total = 417, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.dialoguebehaviour.*", function(payload) inst.lastSignal = payload end)
			end
