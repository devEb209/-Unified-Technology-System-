-- ARKHER SYSTEM H.0444 :: Penetration Recovery Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0444"
	S.key = "arkher.physics.penetration.simulation_policy"
	S.name = "Penetration Recovery Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Penetration Recovery"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.penetration.budget_governor" }
	S.tags = { "h", "penetration", "policy", "physics" }
	S.description = "Penetration Recovery Simulation Policy: device tier rules for substeps, iterations and sleep for the Penetration Recovery subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.penetration.simulation_policy", algorithm = "wfq", starvationGuard = 22 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.penetration.*", function(payload) inst.lastSignal = payload end)
			end
