-- ARKHER SYSTEM P.0258 :: Dialogue Choice Rule Pipeline
-- Category P - GAMEPLAY
-- ARKHER Gameplay Framework capability: attributes, items, objectives, combat and progression.
-- Kit: pipeline (ordered rules applied to every gameplay event)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "P.0258"
	S.key = "arkher.play.dialoguechoice.rule_pipeline"
	S.name = "Dialogue Choice Rule Pipeline"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Dialogue Choice"
	S.aspect = "Rule Pipeline"
	S.kit = "pipeline"
	S.version = "1.0.0"
	S.deps = { "arkher.play.dialoguechoice.state_machine" }
	S.tags = { "p", "dialoguechoice", "pipeline", "play" }
	S.description = "Dialogue Choice Rule Pipeline: ordered rules applied to every gameplay event for the Dialogue Choice subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 320,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("pipeline", { id = "arkher.play.dialoguechoice.rule_pipeline", budgetMs = 5.00 })
		inst.system = S
		inst.ctx = ctx
