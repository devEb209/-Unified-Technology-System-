-- ARKHER SYSTEM F.0332 :: Motion Blur Render Policy
...
return function(A)
	...
	local S = {}
	S.id = "F.0332"
	S.key = "arkher.render.motionblur.render_policy"
	S.name = "Motion Blur Render Policy"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Motion Blur"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.render.motionblur.draw_budget" }
	S.tags = { "f", "motionblur", "policy", "render" }
	S.description = "Motion Blur Render Policy: device tier rules for the frame for the Motion Blur subsystem."
	S.params = {
		backlogLimit = 47,
		baseRadius = 440,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.render.motionblur.render_policy", algorithm = "lottery", starvationGuard = 23 })
		inst.system = S
		inst.ctx = ctx
