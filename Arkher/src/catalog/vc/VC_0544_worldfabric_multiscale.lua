-- ARKHER SYSTEM VC.0544 :: World Fabric Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0544"
	S.key = "arkher.contneural.worldfabric.multi-scale_fidelity"
	S.name = "World Fabric Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.worldfabric.sparse_residency" }
	S.tags = { "vc", "worldfabric", "multiscale", "contneural" }
	S.description = "World Fabric Multi-Scale Fidelity: distance-driven multi-scale level selection for the World Fabric subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 600,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.worldfabric.multi-scale_fidelity", levels = 6, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.worldfabric.*", function(payload) inst.lastSignal = payload end)
			end
