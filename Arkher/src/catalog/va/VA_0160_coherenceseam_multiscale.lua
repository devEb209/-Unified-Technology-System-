-- ARKHER SYSTEM VA.0160 :: Coherence Seam Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0160"
	S.key = "arkher.contworld.coherenceseam.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.coherenceseam.sparse_residency" }
	S.tags = { "va", "coherenceseam", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.coherenceseam.multi-scale_fidelity", levels = 5, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.coherenceseam.*", function(payload) inst.lastSignal = payload end)
			end
