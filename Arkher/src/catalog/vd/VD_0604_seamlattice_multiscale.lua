-- ARKHER SYSTEM VD.0604 :: Seam Lattice Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0604"
	S.key = "arkher.contsim.seamlattice.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.seamlattice.sparse_residency" }
	S.tags = { "vd", "seamlattice", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.seamlattice.multi-scale_fidelity", levels = 5, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.seamlattice.*", function(payload) inst.lastSignal = payload end)
			end
