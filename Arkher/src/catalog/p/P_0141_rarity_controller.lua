-- ARKHER SYSTEM P.0141 :: Rarity Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0141"
	S.key = "arkher.play.rarity.difficulty_controller"
	S.name = "Rarity Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Rarity"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.rarity.balance_analysis" }
	S.tags = { "p", "rarity", "controller", "play" }
	S.description = "Rarity Difficulty Controller: closed-loop tuning toward a target challenge for the Rarity subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 400,
		baseWeight = 0.74,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.rarity.difficulty_controller", mode = "pid", target = 0.840, kp = 0.490, ki = 0.050, kd = 0.050, min = 0.05, max = 1.0, initial = 0.74, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.rarity.*", function(payload) inst.lastSignal = payload end)
			end
