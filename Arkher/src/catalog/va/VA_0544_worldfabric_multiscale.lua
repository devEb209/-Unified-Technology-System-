-- ARKHER SYSTEM VA.0544 :: World Fabric Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0544"
	S.key = "arkher.contworld.worldfabric.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.worldfabric.sparse_residency" }
	S.tags = { "va", "worldfabric", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.worldfabric.multi-scale_fidelity", levels = 6, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.worldfabric.*", function(payload) inst.lastSignal = payload end)
			end
