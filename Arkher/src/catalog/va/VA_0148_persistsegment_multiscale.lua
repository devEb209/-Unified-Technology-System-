-- ARKHER SYSTEM VA.0148 :: Persistence Segment Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0148"
	S.key = "arkher.contworld.persistsegment.multi-scale_fidelity"
	S.name = "Persistence Segment Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.persistsegment.sparse_residency" }
	S.tags = { "va", "persistsegment", "multiscale", "contworld" }
	S.description = "Persistence Segment Multi-Scale Fidelity: distance-driven multi-scale level selection for the Persistence Segment subsystem."
	S.params = {
		backlogLimit = 14,
		baseRadius = 560,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.persistsegment.multi-scale_fidelity", levels = 5, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.persistsegment.*", function(payload) inst.lastSignal = payload end)
			end
