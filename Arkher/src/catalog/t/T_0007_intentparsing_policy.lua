-- ARKHER SYSTEM T.0007 :: Intent Parsing Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0007"
	S.key = "arkher.ai.intentparsing.decision_policy"
	S.name = "Intent Parsing Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.intentparsing.reasoning_graph" }
	S.tags = { "t", "intentparsing", "policy", "ai" }
	S.description = "Intent Parsing Decision Policy: ordered rules deciding what the agent does next for the Intent Parsing subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 560,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.intentparsing.decision_policy", algorithm = "wfq", starvationGuard = 26 })
		inst.system = S
		inst.ctx = ctx
