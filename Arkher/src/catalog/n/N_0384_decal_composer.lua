-- ARKHER SYSTEM N.0384 :: Decal Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0384"
	S.key = "arkher.vfx.decal.effect_composition"
	S.name = "Decal Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Decal"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.decal.effect_graph" }
	S.tags = { "n", "decal", "composer", "vfx" }
	S.description = "Decal Effect Composition: blending of several effect signals into one output for the Decal subsystem."
	S.params = {
		backlogLimit = 23,
		baseRadius = 600,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.decal.effect_composition", mode = "overlay" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.decal.*", function(payload) inst.lastSignal = payload end)
			end
