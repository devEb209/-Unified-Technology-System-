-- ARKHER SYSTEM H.0052 :: Mass Properties Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0052"
	S.key = "arkher.physics.massproperties.simulation_policy"
	S.name = "Mass Properties Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Mass Properties"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.massproperties.budget_governor" }
	S.tags = { "h", "massproperties", "policy", "physics" }
	S.description = "Mass Properties Simulation Policy: device tier rules for substeps, iterations and sleep for the Mass Properties subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.78,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.massproperties.simulation_policy", algorithm = "roundrobin", starvationGuard = 56 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.massproperties.*", function(payload) inst.lastSignal = payload end)
			end
