-- ARKHER SYSTEM H.0164 :: Angular Damping Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0164"
	S.key = "arkher.physics.angulardamping.simulation_policy"
	S.name = "Angular Damping Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Angular Damping"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.angulardamping.budget_governor" }
	S.tags = { "h", "angulardamping", "policy", "physics" }
	S.description = "Angular Damping Simulation Policy: device tier rules for substeps, iterations and sleep for the Angular Damping subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 240,
		baseWeight = 0.52,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.angulardamping.simulation_policy", algorithm = "wfq", starvationGuard = 54 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.angulardamping.*", function(payload) inst.lastSignal = payload end)
			end
