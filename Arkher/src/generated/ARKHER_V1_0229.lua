--!strict
-- Anvil Cluster — ANIMATION Profiler #0229
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Anvil Cluster — ANIMATION Profiler #0229", size=Vector2.new(640, 500), pos=UDim2.fromOffset(277, 333), parent=parent, icon="✈"})
...
Components.TextField(form, "Name", "Object_"..229, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
