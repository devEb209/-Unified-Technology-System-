-- ARKHER SYSTEM G.0400 :: Fallback Heuristic Compute Budget
...
return function(A)
	...
	local S = {}
	S.id = "G.0400"
	S.key = "arkher.neural.fallbackheuristic.compute_budget"
	S.name = "Fallback Heuristic Compute Budget"
	S.category = "G"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.fallbackheuristic.quality_predictor" }
	S.tags = { "g", "fallbackheuristic", "budgeter", "neural" }
	S.description = "Fallback Heuristic Compute Budget: inference time budget allocation for the Fallback Heuristic subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 320,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.neural.fallbackheuristic.compute_budget", total = 220, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
