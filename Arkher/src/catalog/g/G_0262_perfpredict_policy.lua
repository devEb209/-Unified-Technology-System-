-- ARKHER SYSTEM G.0262 :: Performance Prediction Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0262"
	S.key = "arkher.neural.perfpredict.heuristic_policy"
	S.name = "Performance Prediction Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.perfpredict.result_cache" }
	S.tags = { "g", "perfpredict", "policy", "neural" }
	S.description = "Performance Prediction Heuristic Policy: rule fallback when the model is unavailable for the Performance Prediction subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 560,
		baseWeight = 0.68,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.perfpredict.heuristic_policy", algorithm = "wfq", starvationGuard = 38 })
		inst.system = S
		inst.ctx = ctx
