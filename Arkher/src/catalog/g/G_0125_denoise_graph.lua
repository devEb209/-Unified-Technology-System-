-- ARKHER SYSTEM G.0125 :: Denoising Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0125"
	S.key = "arkher.neural.denoise.sampling_graph"
	S.name = "Denoising Sampling Graph"
	S.category = "G"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.denoise.training_ledger" }
	S.tags = { "g", "denoise", "graph", "neural" }
	S.description = "Denoising Sampling Graph: dependency graph of sampling stages for the Denoising subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 360,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.denoise.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.denoise.*", function(payload) inst.lastSignal = payload end)
			end
