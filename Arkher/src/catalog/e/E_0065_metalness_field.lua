-- ARKHER SYSTEM E.0065 :: Metalness Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0065"
	S.key = "arkher.material.metalness.procedural_detail"
	S.name = "Metalness Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Metalness"
	...
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.metalness.texture_sampling" }
	S.tags = { "e", "metalness", "field", "material" }
	S.description = "Metalness Procedural Detail: coherent noise detail driving the surface for the Metalness subsystem."
	S.params = {
		backlogLimit = 40,
		baseRadius = 480,
		baseWeight = 0.72,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.metalness.procedural_detail", seed = 53072, frequency = 0.0120, octaves = 3, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.metalness.*", function(payload) inst.lastSignal = payload end)
			end
