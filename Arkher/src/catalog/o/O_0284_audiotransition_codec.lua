-- ARKHER SYSTEM O.0284 :: Transition Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0284"
	S.key = "arkher.audio.audiotransition.compression_codec"
	S.name = "Transition Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Transition"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.audiotransition.asset_registry" }
	S.tags = { "o", "audiotransition", "codec", "audio" }
	S.description = "Transition Compression Codec: payload encoding of audio state and events for the Transition subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 400,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.audiotransition.compression_codec", format = "rle", quantBits = 10 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.audiotransition.*", function(payload) inst.lastSignal = payload end)
			end
