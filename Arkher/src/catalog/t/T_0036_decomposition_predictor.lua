-- ARKHER SYSTEM T.0036 :: Task Decomposition Outcome Prediction
...
return function(A)
	...
	local S = {}
	S.id = "T.0036"
	S.key = "arkher.ai.decomposition.outcome_prediction"
	S.name = "Task Decomposition Outcome Prediction"
	S.category = "T"
	...
	S.kit = "predictor"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.decomposition.decision_policy" }
	S.tags = { "t", "decomposition", "predictor", "ai" }
	S.description = "Task Decomposition Outcome Prediction: forecasting the result before the work is done for the Task Decomposition subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 400,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("predictor", { id = "arkher.ai.decomposition.outcome_prediction", method = "ema", alpha = 0.28, window = 98 })
		inst.system = S
		inst.ctx = ctx
