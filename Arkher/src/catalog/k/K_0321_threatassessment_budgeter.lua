-- ARKHER SYSTEM K.0321 :: Threat Assessment Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0321"
	S.key = "arkher.npc.threatassessment.thinking_budget"
	S.name = "Threat Assessment Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Threat Assessment"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.threatassessment.daily_schedule" }
	S.tags = { "k", "threatassessment", "budgeter", "npc" }
	S.description = "Threat Assessment Thinking Budget: allocation of per-frame thinking time across NPCs for the Threat Assessment subsystem."
	S.params = {
		backlogLimit = 41,
		baseRadius = 360,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.threatassessment.thinking_budget", total = 253, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.threatassessment.*", function(payload) inst.lastSignal = payload end)
			end
