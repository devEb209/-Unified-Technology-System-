-- ARKHER SYSTEM VD.0096 :: Residency Window Emergence Continuum
...
return function(A)
	...
	local S = {}
	S.id = "VD.0096"
	S.key = "arkher.contsim.residencywindow.emergence_continuum"
	...
	S.kit = "emergence"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.residencywindow.reality_continuum" }
	S.tags = { "vd", "residencywindow", "emergence", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("emergence", { id = "arkher.contsim.residencywindow.emergence_continuum", windowSize = 4, threshold = 1.50, minCount = 2 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.residencywindow.*", function(payload) inst.lastSignal = payload end)
			end
