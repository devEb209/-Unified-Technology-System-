-- ARKHER SYSTEM E.0095 :: Height Displacement Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0095"
	S.key = "arkher.material.displacement.procedural_detail"
	S.name = "Height Displacement Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Height Displacement"
	S.aspect = "Procedural Detail"
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.displacement.texture_sampling" }
	S.tags = { "e", "displacement", "field", "material" }
	S.description = "Height Displacement Procedural Detail: coherent noise detail driving the surface for the Height Displacement subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 560,
		baseWeight = 0.82,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.displacement.procedural_detail", seed = 38182, frequency = 0.0095, octaves = 5, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.displacement.*", function(payload) inst.lastSignal = payload end)
			end
