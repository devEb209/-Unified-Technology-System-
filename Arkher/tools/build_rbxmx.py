...
def build_plugin(path):
    """ARKHER STUDIO plugin payload. Skip if plugin.server.lua missing."""
    plug = os.path.join(ROOT, "roblox/plugin.server.lua")
    if not os.path.exists(plug):
        return 0
    out = [HEADER]
    source = open(plug, encoding="utf-8").read()
...
if __name__ == "__main__":
    mpath = os.path.join(REL, "ARKHER_%s.rbxmx" % ROUND)
    ppath = os.path.join(REL, "ARKHER_%s.rbxlx" % ROUND)
    ...
    if c: print("plugin %s  (%.1f MB)" % (gpath, c / 1048576))
