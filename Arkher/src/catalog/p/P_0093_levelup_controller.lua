-- ARKHER SYSTEM P.0093 :: Level Up Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0093"
	S.key = "arkher.play.levelup.difficulty_controller"
	S.name = "Level Up Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Level Up"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.levelup.balance_analysis" }
	S.tags = { "p", "levelup", "controller", "play" }
	S.description = "Level Up Difficulty Controller: closed-loop tuning toward a target challenge for the Level Up subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 480,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.levelup.difficulty_controller", mode = "pid", target = 0.660, kp = 0.510, ki = 0.100, kd = 0.020, min = 0.05, max = 1.0, initial = 0.56, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.levelup.*", function(payload) inst.lastSignal = payload end)
			end
