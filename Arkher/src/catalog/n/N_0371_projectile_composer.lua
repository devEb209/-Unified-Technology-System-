-- ARKHER SYSTEM N.0371 :: Projectile Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0371"
	S.key = "arkher.vfx.projectile.effect_composition"
	S.name = "Projectile Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Projectile"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.projectile.effect_graph" }
	S.tags = { "n", "projectile", "composer", "vfx" }
	S.description = "Projectile Effect Composition: blending of several effect signals into one output for the Projectile subsystem."
	S.params = {
		backlogLimit = 37,
		baseRadius = 360,
		baseWeight = 0.79,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.projectile.effect_composition", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.projectile.*", function(payload) inst.lastSignal = payload end)
			end
