-- ARKHER SYSTEM G.0388 :: Deterministic Seeding Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0388"
	S.key = "arkher.neural.detseed.heuristic_policy"
	S.name = "Deterministic Seeding Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.detseed.result_cache" }
	S.tags = { "g", "detseed", "policy", "neural" }
	S.description = "Deterministic Seeding Heuristic Policy: rule fallback when the model is unavailable for the Deterministic Seeding subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 480,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.detseed.heuristic_policy", algorithm = "roundrobin", starvationGuard = 36 })
		inst.system = S
		inst.ctx = ctx
