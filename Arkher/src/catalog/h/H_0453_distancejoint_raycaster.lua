-- ARKHER SYSTEM H.0453 :: Distance Joint Query
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0453"
	S.key = "arkher.physics.distancejoint.query"
	S.name = "Distance Joint Query"
	S.category = "H"
	S.family = "PHYSICS"
	...
	S.kit = "raycaster"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.distancejoint.constraint_solver" }
	S.tags = { "h", "distancejoint", "raycaster", "physics" }
	S.description = "Distance Joint Query: rays, sweeps and overlaps against the simulation for the Distance Joint subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 480,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("raycaster", { id = "arkher.physics.distancejoint.query", cellSize = 24 })
		inst.system = S
		inst.ctx = ctx
