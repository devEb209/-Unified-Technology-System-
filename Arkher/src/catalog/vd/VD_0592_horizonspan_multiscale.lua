-- ARKHER SYSTEM VD.0592 :: Horizon Span Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0592"
	S.key = "arkher.contsim.horizonspan.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.horizonspan.sparse_residency" }
	S.tags = { "vd", "horizonspan", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.horizonspan.multi-scale_fidelity", levels = 4, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.horizonspan.*", function(payload) inst.lastSignal = payload end)
			end
