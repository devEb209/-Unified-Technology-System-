#!/usr/bin/env python3
"""GENESIS V1.69 SPECIALIZED 10af — +10 únicos 32/33 (326/332) sem parar"""
...
from build_genesis_v1_67_specialized10ad import patch67
from build_genesis_v1_68_specialized10ae import patch68
...
def patch69(roots):
    items=[
        ("ARKHER_Kids","KIDS — Parent  •  Limit",0xFF6F00,"Parent  •  Limit  •  Time","Parent Limit Time  •  Kids Mode"),
        ("ARKHER_Family","FAMILY — Share  •  Limit",0xFFD600,"Share  •  Limit  •  Invite","Share Limit Invite  •  Family"),
        ("ARKHER_Parental","PARENTAL — Filter  •  Time",0x33691E,"Filter  •  Time  •  Report","Filter Time Report  •  Parental"),
        ("ARKHER_Safe","SAFE — Chat  •  Filter",0x1B5E20,"Chat  •  Filter  •  Report","Chat Filter Report  •  Safe Chat"),
        ("ARKHER_Block","BLOCK — User  •  Reason",0xB71C1C,"User  •  Reason  •  Duration","User Reason Duration  •  Block"),
        ("ARKHER_Mute","MUTE — User  •  Duration",0x37474F,"User  •  Duration  •  Reason","User Duration Reason  •  Mute"),
        ("ARKHER_Report2","REPORT2 — Type  •  Evidence",0xAD1457,"Type  •  Evidence  •  Status","Type Evidence Status  •  Report"),
        ("ARKHER_Appeal","APPEAL — Case  •  Review",0x4A148C,"Case  •  Review  •  Decision","Case Review Decision  •  Appeal"),
        ("ARKHER_Trust","TRUST — Score  •  Level",0x1B5E20,"Score  •  Level  •  Badge","Score Level Badge  •  Trust Level 5"),
        ("ARKHER_Verified","VERIFIED — Badge  •  Check",0x0288D1,"Badge  •  Check  •  Request","Badge Check Request  •  Verified"),
    ]
...
for _,n in ipairs({"ARKHER_Kids","ARKHER_Family","ARKHER_Parental","ARKHER_Safe","ARKHER_Block","ARKHER_Mute","ARKHER_Report2","ARKHER_Appeal","ARKHER_Trust","ARKHER_Verified"}) do
...
print("[V1.69 10x] Kids/Family/Parental/Safe/Block/Mute/Report2/Appeal/Trust/Verified 326/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_69",{"Source":(T_STRING, FUNC69)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch68(roots)
    roots=patch69(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_69.rbxl")
    write(out, serialize(roots))
    print(f"V1.69 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_69.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.69 done")
