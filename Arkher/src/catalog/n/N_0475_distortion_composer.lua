-- ARKHER SYSTEM N.0475 :: Distortion Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0475"
	S.key = "arkher.vfx.distortion.effect_composition"
	S.name = "Distortion Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Distortion"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.distortion.effect_graph" }
	S.tags = { "n", "distortion", "composer", "vfx" }
	S.description = "Distortion Effect Composition: blending of several effect signals into one output for the Distortion subsystem."
	S.params = {
		backlogLimit = 10,
		baseRadius = 400,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.distortion.effect_composition", mode = "max" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.distortion.*", function(payload) inst.lastSignal = payload end)
			end
