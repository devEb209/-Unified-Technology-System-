-- ARKHER SYSTEM VC.0484 :: Lighting Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0484"
	S.key = "arkher.contneural.lightcont.multi-scale_fidelity"
	S.name = "Lighting Continuum Multi-Scale Fidelity"
	S.category = "VC"
	S.family = "CONTINUUM — NEURAL"
	S.area = "Lighting Continuum"
	S.aspect = "Multi-Scale Fidelity"
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.lightcont.sparse_residency" }
	S.tags = { "vc", "lightcont", "multiscale", "contneural" }
	S.description = "Lighting Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Lighting Continuum subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 560,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.lightcont.multi-scale_fidelity", levels = 5, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.lightcont.*", function(payload) inst.lastSignal = payload end)
			end
