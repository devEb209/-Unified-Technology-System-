-- ARKHER SYSTEM K.0335 :: Curiosity Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0335"
	S.key = "arkher.npc.curiosity.thinking_budget"
	S.name = "Curiosity Thinking Budget"
	S.category = "K"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.curiosity.daily_schedule" }
	S.tags = { "k", "curiosity", "budgeter", "npc" }
	S.description = "Curiosity Thinking Budget: allocation of per-frame thinking time across NPCs for the Curiosity subsystem."
	S.params = {
		backlogLimit = 23,
		baseRadius = 600,
		baseWeight = 0.75,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.curiosity.thinking_budget", total = 235, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.curiosity.*", function(payload) inst.lastSignal = payload end)
			end
