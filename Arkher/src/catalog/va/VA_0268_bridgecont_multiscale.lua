-- ARKHER SYSTEM VA.0268 :: Bridge Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0268"
	S.key = "arkher.contworld.bridgecont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.bridgecont.sparse_residency" }
	S.tags = { "va", "bridgecont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.bridgecont.multi-scale_fidelity", levels = 6, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.bridgecont.*", function(payload) inst.lastSignal = payload end)
			end
