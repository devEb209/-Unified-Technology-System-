-- ARKHER SYSTEM R.0171 :: Interpolation Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0171"
	S.key = "arkher.net.interpolation.prediction_and_reconciliation"
	S.name = "Interpolation Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Interpolation"
	S.aspect = "Prediction And Reconciliation"
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.interpolation.time_sync" }
	S.tags = { "r", "interpolation", "prediction", "net" }
	S.description = "Interpolation Prediction And Reconciliation: client simulation, correction and input replay for the Interpolation subsystem."
	S.params = {
		backlogLimit = 29,
		baseRadius = 520,
		baseWeight = 0.71,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.interpolation.prediction_and_reconciliation", errorThreshold = 0.035, smoothing = 9.0, maxInputs = 120 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.interpolation.*", function(payload) inst.lastSignal = payload end)
			end
