-- ARKHER SYSTEM VC.0592 :: Horizon Span Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0592"
	S.key = "arkher.contneural.horizonspan.multi-scale_fidelity"
	S.name = "Horizon Span Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.horizonspan.sparse_residency" }
	S.tags = { "vc", "horizonspan", "multiscale", "contneural" }
	S.description = "Horizon Span Multi-Scale Fidelity: distance-driven multi-scale level selection for the Horizon Span subsystem."
	S.params = {
		backlogLimit = 9,
		baseRadius = 520,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.horizonspan.multi-scale_fidelity", levels = 4, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.horizonspan.*", function(payload) inst.lastSignal = payload end)
			end
