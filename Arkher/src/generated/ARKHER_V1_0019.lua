--!strict
-- Nuke Nodal — ANIMATION Profiler #0019
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Nuke Nodal — ANIMATION Profiler #0019", size=Vector2.new(640, 500), pos=UDim2.fromOffset(347, 143), parent=parent, icon="⬡"})
...
Components.TextField(form, "Name", "Object_"..19, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
