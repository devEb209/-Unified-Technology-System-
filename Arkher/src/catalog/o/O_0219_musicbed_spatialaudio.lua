-- ARKHER SYSTEM O.0219 :: Music Bed Spatialization
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0219"
	S.key = "arkher.audio.musicbed.spatialization"
	S.name = "Music Bed Spatialization"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Music Bed"
	...
	S.kit = "spatialaudio"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.musicbed.mixer_bus" }
	S.tags = { "o", "musicbed", "spatialaudio", "audio" }
	S.description = "Music Bed Spatialization: distance attenuation, panning, occlusion and doppler for the Music Bed subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 160,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("spatialaudio", { id = "arkher.audio.musicbed.spatialization", model = "inverse", refDistance = 3, maxDistance = 80, rolloff = 0.80, maxVoices = 12 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.musicbed.*", function(payload) inst.lastSignal = payload end)
			end
