-- ARKHER SYSTEM G.0027 :: Temporal Reconstruction Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0027"
	S.key = "arkher.neural.temporalrecon.sampling_graph"
	S.name = "Temporal Reconstruction Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Temporal Reconstruction"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.temporalrecon.training_ledger" }
	S.tags = { "g", "temporalrecon", "graph", "neural" }
	S.description = "Temporal Reconstruction Sampling Graph: dependency graph of sampling stages for the Temporal Reconstruction subsystem."
	S.params = {
		backlogLimit = 39,
		baseRadius = 600,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.temporalrecon.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.temporalrecon.*", function(payload) inst.lastSignal = payload end)
			end
