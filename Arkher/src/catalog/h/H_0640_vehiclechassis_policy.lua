-- ARKHER SYSTEM H.0640 :: Vehicle Chassis Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0640"
	S.key = "arkher.physics.vehiclechassis.simulation_policy"
	S.name = "Vehicle Chassis Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Vehicle Chassis"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.vehiclechassis.budget_governor" }
	S.tags = { "h", "vehiclechassis", "policy", "physics" }
	S.description = "Vehicle Chassis Simulation Policy: device tier rules for substeps, iterations and sleep for the Vehicle Chassis subsystem."
	S.params = {
		backlogLimit = 10,
		baseRadius = 240,
		baseWeight = 0.72,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.vehiclechassis.simulation_policy", algorithm = "wfq", starvationGuard = 18 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.vehiclechassis.*", function(payload) inst.lastSignal = payload end)
			end
