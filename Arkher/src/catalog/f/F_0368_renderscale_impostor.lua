-- ARKHER SYSTEM F.0368 :: Render Scale Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0368"
	S.key = "arkher.render.renderscale.geometry_virtualization"
	S.name = "Render Scale Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Render Scale"
	...
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.renderscale.visibility_set" }
	S.tags = { "f", "renderscale", "impostor", "render" }
	S.description = "Render Scale Geometry Virtualization: impostor and HLOD synthesis under a budget for the Render Scale subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 240,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.renderscale.geometry_virtualization", atlasSlots = 64, viewCount = 8, errorThreshold = 3.00 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.renderscale.*", function(payload) inst.lastSignal = payload end)
			end
