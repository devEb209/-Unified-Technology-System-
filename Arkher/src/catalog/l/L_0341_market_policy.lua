-- ARKHER SYSTEM L.0341 :: Market Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0341"
	S.key = "arkher.sim.market.fidelity_policy"
	S.name = "Market Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.market.streaming" }
	S.tags = { "l", "market", "policy", "sim" }
	S.description = "Market Fidelity Policy: rules that pick full, cohort or statistical simulation for the Market subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.market.fidelity_policy", algorithm = "roundrobin", starvationGuard = 48 })
		inst.system = S
		inst.ctx = ctx
