-- ARKHER SYSTEM F.0038 :: Opaque Pass Render Policy
...
return function(A)
	...
	local S = {}
	S.id = "F.0038"
	S.key = "arkher.render.opaquepass.render_policy"
	S.name = "Opaque Pass Render Policy"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Opaque Pass"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.render.opaquepass.draw_budget" }
	S.tags = { "f", "opaquepass", "policy", "render" }
	S.description = "Opaque Pass Render Policy: device tier rules for the frame for the Opaque Pass subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 560,
		baseWeight = 0.68,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.render.opaquepass.render_policy", algorithm = "wfq", starvationGuard = 50 })
		inst.system = S
		inst.ctx = ctx
