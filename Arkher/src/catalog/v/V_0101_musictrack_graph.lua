-- ARKHER SYSTEM V.0101 :: Music Track Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0101"
	S.key = "arkher.asset.musictrack.dependency_graph"
	S.name = "Music Track Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.musictrack.asset_registry" }
	S.tags = { "v", "musictrack", "graph", "asset" }
	S.description = "Music Track Dependency Graph: what references what, and in which order it must be built for the Music Track subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 280,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.musictrack.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.musictrack.*", function(payload) inst.lastSignal = payload end)
			end
