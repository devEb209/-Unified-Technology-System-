-- ARKHER SYSTEM T.0581 :: Build Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0581"
	S.key = "arkher.ai.buildreasoning.decision_policy"
	S.name = "Build Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.buildreasoning.reasoning_graph" }
	S.tags = { "t", "buildreasoning", "policy", "ai" }
	S.description = "Build Reasoning Decision Policy: ordered rules deciding what the agent does next for the Build Reasoning subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 320,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.buildreasoning.decision_policy", algorithm = "roundrobin", starvationGuard = 44 })
		inst.system = S
		inst.ctx = ctx
