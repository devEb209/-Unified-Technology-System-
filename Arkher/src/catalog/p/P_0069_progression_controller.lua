-- ARKHER SYSTEM P.0069 :: Progression Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0069"
	S.key = "arkher.play.progression.difficulty_controller"
	S.name = "Progression Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Progression"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.progression.balance_analysis" }
	S.tags = { "p", "progression", "controller", "play" }
	S.description = "Progression Difficulty Controller: closed-loop tuning toward a target challenge for the Progression subsystem."
	S.params = {
		backlogLimit = 10,
		baseRadius = 400,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.progression.difficulty_controller", mode = "pid", target = 0.520, kp = 0.370, ki = 0.020, kd = 0.030, min = 0.05, max = 1.0, initial = 0.42, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.progression.*", function(payload) inst.lastSignal = payload end)
			end
