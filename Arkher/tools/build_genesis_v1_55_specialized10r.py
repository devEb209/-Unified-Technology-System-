#!/usr/bin/env python3
"""GENESIS V1.55 SPECIALIZED 10r — +10 únicos 18/33 (186/332) sem parar"""
...
def patch55(roots):
    items=[
        ("ARKHER_Raid","RAID — Wing  •  Boss",0xB71C1C,"Wing  •  Boss  •  Lockout","Wing Boss Lockout  •  20 players"),
        ("ARKHER_Dungeon","DUNGEON — Key  •  Affix",0x4A148C,"Key  •  Affix  •  Timer","Key Affix Timer  •  Mythic+"),
        ("ARKHER_PvP","PVP — Arena  •  Rating",0xAD1457,"Arena  •  Rating  •  Season","Arena Rating Season  •  2v2 3v3"),
        ("ARKHER_Battleground","BATTLEGROUND — Flag  •  Node",0x1B5E20,"Flag  •  Node  •  Score","Flag Node Score  •  15v15"),
        ("ARKHER_WorldBoss","WORLDBOSS — Spawn  •  Loot",0xBF360C,"Spawn  •  Loot  •  Tap","Spawn Loot Tap  •  40 players"),
        ("ARKHER_Quest2","QUEST2 — Chain  •  Daily",0xFF6F00,"Chain  •  Daily  •  Weekly","Chain Daily Weekly  •  25 quests"),
        ("ARKHER_Achieve2","ACHIEVE2 — Hidden  •  Progress",0xFFD600,"Hidden  •  Progress  •  Title","Hidden Progress Title  •  500 pts"),
        ("ARKHER_Reputation","REPUTATION — Bar  •  Reward",0x2E7D32,"Bar  •  Reward  •  Paragon","Bar Reward Paragon  •  Exalted"),
        ("ARKHER_Currency","CURRENCY — Gold  •  Token",0xFFD54F,"Gold  •  Token  •  Cap","Gold Token Cap  •  9999"),
        ("ARKHER_Shop2","SHOP2 — Catalog  •  Discount",0x01579B,"Catalog  •  Discount  •  Bundle","Catalog Discount Bundle  •  20% off"),
    ]
...
for _,n in ipairs({"ARKHER_Raid","ARKHER_Dungeon","ARKHER_PvP","ARKHER_Battleground","ARKHER_WorldBoss","ARKHER_Quest2","ARKHER_Achieve2","ARKHER_Reputation","ARKHER_Currency","ARKHER_Shop2"}) do
...
print("[V1.55 10x] Raid/Dungeon/PvP/Battleground/WorldBoss/Quest2/Achieve2/Reputation/Currency/Shop2 186/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_55",{"Source":(T_STRING, FUNC55)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch54(roots)
    roots=patch55(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_55.rbxl")
    write(out, serialize(roots))
    print(f"V1.55 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_55.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.55 done")
