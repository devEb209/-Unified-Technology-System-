-- ARKHER SYSTEM R.0147 :: Delta Compression Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0147"
	S.key = "arkher.net.deltacompression.prediction_and_reconciliation"
	S.name = "Delta Compression Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Delta Compression"
	S.aspect = "Prediction And Reconciliation"
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.deltacompression.time_sync" }
	S.tags = { "r", "deltacompression", "prediction", "net" }
	S.description = "Delta Compression Prediction And Reconciliation: client simulation, correction and input replay for the Delta Compression subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 520,
		baseWeight = 0.85,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.deltacompression.prediction_and_reconciliation", errorThreshold = 0.055, smoothing = 13.0, maxInputs = 120 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.deltacompression.*", function(payload) inst.lastSignal = payload end)
			end
