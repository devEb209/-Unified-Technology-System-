-- ARKHER SYSTEM I.0110 :: Clip Events Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0110"
	S.key = "arkher.anim.clipevents.retarget_graph"
	S.name = "Clip Events Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Clip Events"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.clipevents.physical_blend" }
	S.tags = { "i", "clipevents", "graph", "anim" }
	S.description = "Clip Events Retarget Graph: bone mapping and retarget path resolution for the Clip Events subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 480,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.clipevents.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.clipevents.*", function(payload) inst.lastSignal = payload end)
			end
