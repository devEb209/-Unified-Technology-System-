--!strict
-- 3dsMax Stack — ANIMATION Profiler #0049
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="3dsMax Stack — ANIMATION Profiler #0049", size=Vector2.new(640, 500), pos=UDim2.fromOffset(337, 133), parent=parent, icon="◨"})
...
Components.TextField(form, "Name", "Object_"..49, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
