-- ARKHER SYSTEM O.0375 :: Mix Snapshot Spatialization
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0375"
	S.key = "arkher.audio.mixsnapshot.spatialization"
	S.name = "Mix Snapshot Spatialization"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Mix Snapshot"
	...
	S.kit = "spatialaudio"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.mixsnapshot.mixer_bus" }
	S.tags = { "o", "mixsnapshot", "spatialaudio", "audio" }
	S.description = "Mix Snapshot Spatialization: distance attenuation, panning, occlusion and doppler for the Mix Snapshot subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 480,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("spatialaudio", { id = "arkher.audio.mixsnapshot.spatialization", model = "exponential", refDistance = 5, maxDistance = 120, rolloff = 0.92, maxVoices = 20 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.mixsnapshot.*", function(payload) inst.lastSignal = payload end)
			end
