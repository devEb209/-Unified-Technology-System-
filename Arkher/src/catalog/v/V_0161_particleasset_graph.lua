-- ARKHER SYSTEM V.0161 :: Particle Asset Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0161"
	S.key = "arkher.asset.particleasset.dependency_graph"
	S.name = "Particle Asset Dependency Graph"
	S.category = "V"
	S.family = "ASSET PIPELINE"
	S.area = "Particle Asset"
	S.aspect = "Dependency Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.particleasset.asset_registry" }
	S.tags = { "v", "particleasset", "graph", "asset" }
	S.description = "Particle Asset Dependency Graph: what references what, and in which order it must be built for the Particle Asset subsystem."
	S.params = {
		backlogLimit = 34,
		baseRadius = 560,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.particleasset.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.particleasset.*", function(payload) inst.lastSignal = payload end)
			end
