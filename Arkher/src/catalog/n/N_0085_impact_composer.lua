-- ARKHER SYSTEM N.0085 :: Impact Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0085"
	S.key = "arkher.vfx.impact.effect_composition"
	S.name = "Impact Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Impact"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.impact.effect_graph" }
	S.tags = { "n", "impact", "composer", "vfx" }
	S.description = "Impact Effect Composition: blending of several effect signals into one output for the Impact subsystem."
	S.params = {
		backlogLimit = 34,
		baseRadius = 400,
		baseWeight = 0.86,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.impact.effect_composition", mode = "max" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.impact.*", function(payload) inst.lastSignal = payload end)
			end
