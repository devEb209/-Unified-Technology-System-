-- ARKHER SYSTEM VA.0292 :: Climate Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0292"
	S.key = "arkher.contworld.climatecont.multi-scale_fidelity"
	S.name = "Climate Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.climatecont.sparse_residency" }
	S.tags = { "va", "climatecont", "multiscale", "contworld" }
	S.description = "Climate Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Climate Continuum subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 240,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.climatecont.multi-scale_fidelity", levels = 6, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.climatecont.*", function(payload) inst.lastSignal = payload end)
			end
