-- ARKHER SYSTEM V.0317 :: Texture Atlas Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0317"
	S.key = "arkher.asset.atlas.dependency_graph"
	S.name = "Texture Atlas Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.atlas.asset_registry" }
	S.tags = { "v", "atlas", "graph", "asset" }
	S.description = "Texture Atlas Dependency Graph: what references what, and in which order it must be built for the Texture Atlas subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 600,
		baseWeight = 0.85,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.atlas.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.atlas.*", function(payload) inst.lastSignal = payload end)
			end
