#!/usr/bin/env python3
"""GENESIS V1.48 SPECIALIZED 10k — +10 únicos 11/33 (116/332) sem parar"""
...
def patch48(roots):
    items=[
        ("ARKHER_Glass","GLASS — IOR  •  Frost",0xB2EBF2,"IOR  •  Frost  •  Thin","IOR Frost Thin  •  Dispersion"),
        ("ARKHER_Wood","WOOD — Grain  •  Ring",0x5D4037,"Grain  •  Ring  •  Knot","Grain Ring Knot  •  Veneer"),
        ("ARKHER_Fabric","FABRIC — Weave  •  Fuzz",0x6A1B9A,"Weave  •  Fuzz  •  Sheen","Weave Fuzz Sheen  •  Subsurface"),
        ("ARKHER_Leather","LEATHER — Grain  •  Crack",0x3E2723,"Grain  •  Crack  •  Coating","Grain Crack Coating  •  Bumps"),
        ("ARKHER_Plastic","PLASTIC — ClearCoat  •  Rough",0x607D8B,"ClearCoat  •  Rough  •  Subsurface","ClearCoat Rough Subsurface"),
        ("ARKHER_Concrete","CONCRETE — Aggregate  •  Crack",0x757575,"Aggregate  •  Crack  •  Dust","Aggregate Crack Dust  •  PBR"),
        ("ARKHER_Brick","BRICK — Mortar  •  Offset",0xBF360C,"Brick  •  Mortar  •  Offset","Brick Mortar Offset  •  Weathering"),
        ("ARKHER_Tile","TILE — Grout  •  Gloss",0x00838F,"Tile  •  Grout  •  Gloss","Tile Grout Gloss  •  Reflect"),
        ("ARKHER_Paint","PAINT — Layer  •  Peel",0xAD1457,"Layer  •  Peel  •  Flake","Layer Peel Flake  •  Metallic"),
        ("ARKHER_Carbon","CARBON — Fiber  •  Weave",0x212121,"Fiber  •  Weave  •  Resin","Fiber Weave Resin  •  Aniso"),
    ]
...
for _,n in ipairs({"ARKHER_Glass","ARKHER_Wood","ARKHER_Fabric","ARKHER_Leather","ARKHER_Plastic","ARKHER_Concrete","ARKHER_Brick","ARKHER_Tile","ARKHER_Paint","ARKHER_Carbon"}) do
...
print("[V1.48 10x] Glass/Wood/Fabric/Leather/Plastic/Concrete/Brick/Tile/Paint/Carbon 116/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_48",{"Source":(T_STRING, FUNC48)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch47(roots)
    roots=patch48(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_48.rbxl")
    write(out, serialize(roots))
    print(f"V1.48 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_48.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.48 done")
