-- ARKHER SYSTEM K.0587 :: Idle Behaviour Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0587"
	S.key = "arkher.npc.idlebehaviour.thinking_budget"
	S.name = "Idle Behaviour Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Idle Behaviour"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.idlebehaviour.daily_schedule" }
	S.tags = { "k", "idlebehaviour", "budgeter", "npc" }
	S.description = "Idle Behaviour Thinking Budget: allocation of per-frame thinking time across NPCs for the Idle Behaviour subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 600,
		baseWeight = 0.63,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.idlebehaviour.thinking_budget", total = 123, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.idlebehaviour.*", function(payload) inst.lastSignal = payload end)
			end
