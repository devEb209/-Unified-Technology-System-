-- ARKHER SYSTEM VC.0340 :: Society Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0340"
	S.key = "arkher.contneural.societycont.multi-scale_fidelity"
	S.name = "Society Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.societycont.sparse_residency" }
	S.tags = { "vc", "societycont", "multiscale", "contneural" }
	S.description = "Society Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Society Continuum subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 360,
		baseWeight = 0.87,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.societycont.multi-scale_fidelity", levels = 6, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.societycont.*", function(payload) inst.lastSignal = payload end)
			end
