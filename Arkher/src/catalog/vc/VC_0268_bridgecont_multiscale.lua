-- ARKHER SYSTEM VC.0268 :: Bridge Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0268"
	S.key = "arkher.contneural.bridgecont.multi-scale_fidelity"
	S.name = "Bridge Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.bridgecont.sparse_residency" }
	S.tags = { "vc", "bridgecont", "multiscale", "contneural" }
	S.description = "Bridge Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Bridge Continuum subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 360,
		baseWeight = 0.79,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.bridgecont.multi-scale_fidelity", levels = 6, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.bridgecont.*", function(payload) inst.lastSignal = payload end)
			end
