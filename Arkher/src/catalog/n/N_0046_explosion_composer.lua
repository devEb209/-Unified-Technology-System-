-- ARKHER SYSTEM N.0046 :: Explosion Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0046"
	S.key = "arkher.vfx.explosion.effect_composition"
	S.name = "Explosion Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Explosion"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.explosion.effect_graph" }
	S.tags = { "n", "explosion", "composer", "vfx" }
	S.description = "Explosion Effect Composition: blending of several effect signals into one output for the Explosion subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 240,
		baseWeight = 0.64,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.explosion.effect_composition", mode = "max" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.explosion.*", function(payload) inst.lastSignal = payload end)
			end
