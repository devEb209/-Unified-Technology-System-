-- ARKHER SYSTEM F.0172 :: Atmosphere Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0172"
	S.key = "arkher.render.atmosphere.geometry_virtualization"
	S.name = "Atmosphere Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Atmosphere"
	...
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.atmosphere.visibility_set" }
	S.tags = { "f", "atmosphere", "impostor", "render" }
	S.description = "Atmosphere Geometry Virtualization: impostor and HLOD synthesis under a budget for the Atmosphere subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 280,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.atmosphere.geometry_virtualization", atlasSlots = 80, viewCount = 8, errorThreshold = 1.30 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.atmosphere.*", function(payload) inst.lastSignal = payload end)
			end
