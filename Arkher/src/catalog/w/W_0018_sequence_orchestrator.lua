-- ARKHER SYSTEM W.0018 :: Sequence Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0018"
	S.key = "arkher.cine.sequence.playback_orchestrator"
	S.name = "Sequence Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Sequence"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.sequence.cut_policy" }
	S.tags = { "w", "sequence", "orchestrator", "cine" }
	S.description = "Sequence Playback Orchestrator: the state machine that drives a cinematic from start to end for the Sequence subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 560,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.sequence.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
