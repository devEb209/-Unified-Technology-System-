-- ARKHER SYSTEM P.0249 :: Objective Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0249"
	S.key = "arkher.play.objective.difficulty_controller"
	S.name = "Objective Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Objective"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.objective.balance_analysis" }
	S.tags = { "p", "objective", "controller", "play" }
	S.description = "Objective Difficulty Controller: closed-loop tuning toward a target challenge for the Objective subsystem."
	S.params = {
		backlogLimit = 19,
		baseRadius = 280,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.objective.difficulty_controller", mode = "bangbang", target = 0.610, kp = 0.460, ki = 0.050, kd = 0.020, min = 0.05, max = 1.0, initial = 0.51, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.objective.*", function(payload) inst.lastSignal = payload end)
			end
