-- ARKHER SYSTEM VD.0292 :: Climate Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0292"
	S.key = "arkher.contsim.climatecont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.climatecont.sparse_residency" }
	S.tags = { "vd", "climatecont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.climatecont.multi-scale_fidelity", levels = 4, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.climatecont.*", function(payload) inst.lastSignal = payload end)
			end
