-- ARKHER SYSTEM VB.0412 :: Behaviour Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0412"
	S.key = "arkher.conttime.behaviourcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.behaviourcont.sparse_residency" }
	S.tags = { "vb", "behaviourcont", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.behaviourcont.multi-scale_fidelity", levels = 5, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.behaviourcont.*", function(payload) inst.lastSignal = payload end)
			end
