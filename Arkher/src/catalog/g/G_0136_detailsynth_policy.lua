-- ARKHER SYSTEM G.0136 :: Detail Synthesis Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0136"
	S.key = "arkher.neural.detailsynth.heuristic_policy"
	S.name = "Detail Synthesis Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.detailsynth.result_cache" }
	S.tags = { "g", "detailsynth", "policy", "neural" }
	S.description = "Detail Synthesis Heuristic Policy: rule fallback when the model is unavailable for the Detail Synthesis subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 400,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.detailsynth.heuristic_policy", algorithm = "wfq", starvationGuard = 46 })
		inst.system = S
		inst.ctx = ctx
