-- ARKHER SYSTEM K.0489 :: Follow Behaviour Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0489"
	S.key = "arkher.npc.followbehaviour.thinking_budget"
	S.name = "Follow Behaviour Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Follow Behaviour"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.followbehaviour.daily_schedule" }
	S.tags = { "k", "followbehaviour", "budgeter", "npc" }
	S.description = "Follow Behaviour Thinking Budget: allocation of per-frame thinking time across NPCs for the Follow Behaviour subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 200,
		baseWeight = 0.85,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.followbehaviour.thinking_budget", total = 145, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.followbehaviour.*", function(payload) inst.lastSignal = payload end)
			end
