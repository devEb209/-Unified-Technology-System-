-- ARKHER SYSTEM VB.0160 :: Coherence Seam Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0160"
	S.key = "arkher.conttime.coherenceseam.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.coherenceseam.sparse_residency" }
	S.tags = { "vb", "coherenceseam", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.coherenceseam.multi-scale_fidelity", levels = 4, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.coherenceseam.*", function(payload) inst.lastSignal = payload end)
			end
