-- ARKHER SYSTEM VA.0604 :: Seam Lattice Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0604"
	S.key = "arkher.contworld.seamlattice.multi-scale_fidelity"
	S.name = "Seam Lattice Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.seamlattice.sparse_residency" }
	S.tags = { "va", "seamlattice", "multiscale", "contworld" }
	S.description = "Seam Lattice Multi-Scale Fidelity: distance-driven multi-scale level selection for the Seam Lattice subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 480,
		baseWeight = 0.82,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.seamlattice.multi-scale_fidelity", levels = 6, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.seamlattice.*", function(payload) inst.lastSignal = payload end)
			end
