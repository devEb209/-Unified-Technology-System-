-- ARKHER SYSTEM K.0111 :: Memory Consolidation Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0111"
	S.key = "arkher.npc.consolidation.thinking_budget"
	S.name = "Memory Consolidation Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Memory Consolidation"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.consolidation.daily_schedule" }
	S.tags = { "k", "consolidation", "budgeter", "npc" }
	S.description = "Memory Consolidation Thinking Budget: allocation of per-frame thinking time across NPCs for the Memory Consolidation subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 320,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.consolidation.thinking_budget", total = 168, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.consolidation.*", function(payload) inst.lastSignal = payload end)
			end
