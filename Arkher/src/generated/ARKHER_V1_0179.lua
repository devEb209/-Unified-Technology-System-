--!strict
-- Google Maps — ANIMATION Profiler #0179
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Google Maps — ANIMATION Profiler #0179", size=Vector2.new(680, 500), pos=UDim2.fromOffset(427, 263), parent=parent, icon="◆"})
    win.Root.Name="ARKHER_V1_0179"
    win.Root.Visible=false
    local content=win.Content
    content.BackgroundColor3=Theme.tokens.void
    -- accent line per UI
    local accent=Instance.new("Frame"); accent.Size=UDim2.new(1,0,0,2); accent.BackgroundColor3=Theme.categoryColor("Google M"); accent.BorderSizePixel=0; accent.Parent=content
