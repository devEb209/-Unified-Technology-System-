-- ARKHER SYSTEM VA.0412 :: Behaviour Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0412"
	S.key = "arkher.contworld.behaviourcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.behaviourcont.sparse_residency" }
	S.tags = { "va", "behaviourcont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.behaviourcont.multi-scale_fidelity", levels = 4, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.behaviourcont.*", function(payload) inst.lastSignal = payload end)
			end
