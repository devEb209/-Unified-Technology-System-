-- ARKHER SYSTEM J.0183 :: Accessory Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0183"
	S.key = "arkher.character.accessory.appearance_composer"
	S.name = "Accessory Appearance Composer"
	S.category = "J"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.accessory.locomotion" }
	S.tags = { "j", "accessory", "composer", "character" }
	S.description = "Accessory Appearance Composer: composition of appearance slots into a look for the Accessory subsystem."
	S.params = {
		backlogLimit = 37,
		baseRadius = 200,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.accessory.appearance_composer", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.accessory.*", function(payload) inst.lastSignal = payload end)
			end
