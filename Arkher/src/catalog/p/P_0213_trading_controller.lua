-- ARKHER SYSTEM P.0213 :: Trading Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0213"
	S.key = "arkher.play.trading.difficulty_controller"
	S.name = "Trading Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Trading"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.trading.balance_analysis" }
	S.tags = { "p", "trading", "controller", "play" }
	S.description = "Trading Difficulty Controller: closed-loop tuning toward a target challenge for the Trading subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.trading.difficulty_controller", mode = "pid", target = 0.800, kp = 0.250, ki = 0.020, kd = 0.010, min = 0.05, max = 1.0, initial = 0.70, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.trading.*", function(payload) inst.lastSignal = payload end)
			end
