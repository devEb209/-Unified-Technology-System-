-- ARKHER SYSTEM K.0041 :: Learning Loop Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0041"
	S.key = "arkher.npc.learningloop.thinking_budget"
	S.name = "Learning Loop Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Learning Loop"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.learningloop.daily_schedule" }
	S.tags = { "k", "learningloop", "budgeter", "npc" }
	S.description = "Learning Loop Thinking Budget: allocation of per-frame thinking time across NPCs for the Learning Loop subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 480,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.learningloop.thinking_budget", total = 408, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.learningloop.*", function(payload) inst.lastSignal = payload end)
			end
