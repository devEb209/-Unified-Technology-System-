-- ARKHER SYSTEM G.0391 :: Deterministic Seeding Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0391"
	S.key = "arkher.neural.detseed.sampling_graph"
	S.name = "Deterministic Seeding Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Deterministic Seeding"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.detseed.training_ledger" }
	S.tags = { "g", "detseed", "graph", "neural" }
	S.description = "Deterministic Seeding Sampling Graph: dependency graph of sampling stages for the Deterministic Seeding subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 200,
		baseWeight = 0.85,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.detseed.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.detseed.*", function(payload) inst.lastSignal = payload end)
			end
