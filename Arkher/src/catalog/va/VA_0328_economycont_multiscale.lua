-- ARKHER SYSTEM VA.0328 :: Economy Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0328"
	S.key = "arkher.contworld.economycont.multi-scale_fidelity"
	S.name = "Economy Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.economycont.sparse_residency" }
	S.tags = { "va", "economycont", "multiscale", "contworld" }
	S.description = "Economy Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Economy Continuum subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 200,
		baseWeight = 0.87,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.economycont.multi-scale_fidelity", levels = 5, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.economycont.*", function(payload) inst.lastSignal = payload end)
			end
