-- ARKHER SYSTEM F.0144 :: Light Clustering Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0144"
	S.key = "arkher.render.lightcluster.geometry_virtualization"
	S.name = "Light Clustering Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Light Clustering"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.lightcluster.visibility_set" }
	S.tags = { "f", "lightcluster", "impostor", "render" }
	S.description = "Light Clustering Geometry Virtualization: impostor and HLOD synthesis under a budget for the Light Clustering subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 280,
		baseWeight = 0.53,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.lightcluster.geometry_virtualization", atlasSlots = 80, viewCount = 8, errorThreshold = 1.30 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.lightcluster.*", function(payload) inst.lastSignal = payload end)
			end
