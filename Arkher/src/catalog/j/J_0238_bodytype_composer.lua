-- ARKHER SYSTEM J.0238 :: Body Type Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0238"
	S.key = "arkher.character.bodytype.appearance_composer"
	S.name = "Body Type Appearance Composer"
	S.category = "J"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.bodytype.locomotion" }
	S.tags = { "j", "bodytype", "composer", "character" }
	S.description = "Body Type Appearance Composer: composition of appearance slots into a look for the Body Type subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 440,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.bodytype.appearance_composer", mode = "overlay" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.bodytype.*", function(payload) inst.lastSignal = payload end)
			end
