-- ARKHER SYSTEM N.0033 :: Fire Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0033"
	S.key = "arkher.vfx.fire.effect_composition"
	S.name = "Fire Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Fire"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.fire.effect_graph" }
	S.tags = { "n", "fire", "composer", "vfx" }
	S.description = "Fire Effect Composition: blending of several effect signals into one output for the Fire subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 480,
		baseWeight = 0.66,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.fire.effect_composition", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.fire.*", function(payload) inst.lastSignal = payload end)
			end
