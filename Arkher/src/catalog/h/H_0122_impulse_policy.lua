-- ARKHER SYSTEM H.0122 :: Impulse Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0122"
	S.key = "arkher.physics.impulse.simulation_policy"
	S.name = "Impulse Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Impulse"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.impulse.budget_governor" }
	S.tags = { "h", "impulse", "policy", "physics" }
	S.description = "Impulse Simulation Policy: device tier rules for substeps, iterations and sleep for the Impulse subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 320,
		baseWeight = 0.52,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.impulse.simulation_policy", algorithm = "roundrobin", starvationGuard = 44 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.impulse.*", function(payload) inst.lastSignal = payload end)
			end
