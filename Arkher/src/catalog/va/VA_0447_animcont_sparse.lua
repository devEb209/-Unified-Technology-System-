-- ARKHER SYSTEM VA.0447 :: Animation Continuum Sparse Residency
...
return function(A)
	...
	local S = {}
	S.id = "VA.0447"
	S.key = "arkher.contworld.animcont.sparse_residency"
	...
	S.kit = "sparse"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.animcont.epoch_clock" }
	S.tags = { "va", "animcont", "sparse", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("sparse", { id = "arkher.contworld.animcont.sparse_residency", capacity = 512, pageBytes = 5120 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.animcont.*", function(payload) inst.lastSignal = payload end)
			end
