-- ARKHER SYSTEM V.0113 :: Voice Line Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0113"
	S.key = "arkher.asset.voiceline.dependency_graph"
	S.name = "Voice Line Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.voiceline.asset_registry" }
	S.tags = { "v", "voiceline", "graph", "asset" }
	S.description = "Voice Line Dependency Graph: what references what, and in which order it must be built for the Voice Line subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 520,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.voiceline.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.voiceline.*", function(payload) inst.lastSignal = payload end)
			end
