-- ARKHER SYSTEM G.0486 :: Evaluation Harness Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0486"
	S.key = "arkher.neural.evalharness.heuristic_policy"
	S.name = "Evaluation Harness Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.evalharness.result_cache" }
	S.tags = { "g", "evalharness", "policy", "neural" }
	S.description = "Evaluation Harness Heuristic Policy: rule fallback when the model is unavailable for the Evaluation Harness subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 280,
		baseWeight = 0.53,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.evalharness.heuristic_policy", algorithm = "lottery", starvationGuard = 43 })
		inst.system = S
		inst.ctx = ctx
