-- ARKHER SYSTEM O.0044 :: Impact Sound Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0044"
	S.key = "arkher.audio.impactsound.compression_codec"
	S.name = "Impact Sound Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Impact Sound"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.impactsound.asset_registry" }
	S.tags = { "o", "impactsound", "codec", "audio" }
	S.description = "Impact Sound Compression Codec: payload encoding of audio state and events for the Impact Sound subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 320,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.impactsound.compression_codec", format = "binary", quantBits = 12 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.impactsound.*", function(payload) inst.lastSignal = payload end)
			end
