-- ARKHER SYSTEM O.0368 :: Magic Audio Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0368"
	S.key = "arkher.audio.magicaudio.compression_codec"
	S.name = "Magic Audio Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Magic Audio"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.magicaudio.asset_registry" }
	S.tags = { "o", "magicaudio", "codec", "audio" }
	S.description = "Magic Audio Compression Codec: payload encoding of audio state and events for the Magic Audio subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 480,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.magicaudio.compression_codec", format = "binary", quantBits = 12 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.magicaudio.*", function(payload) inst.lastSignal = payload end)
			end
