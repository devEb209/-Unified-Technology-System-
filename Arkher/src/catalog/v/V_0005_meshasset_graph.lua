-- ARKHER SYSTEM V.0005 :: Mesh Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0005"
	S.key = "arkher.asset.meshasset.dependency_graph"
	S.name = "Mesh Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.meshasset.asset_registry" }
	S.tags = { "v", "meshasset", "graph", "asset" }
	S.description = "Mesh Dependency Graph: what references what, and in which order it must be built for the Mesh subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 560,
		baseWeight = 0.9,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.meshasset.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.meshasset.*", function(payload) inst.lastSignal = payload end)
			end
