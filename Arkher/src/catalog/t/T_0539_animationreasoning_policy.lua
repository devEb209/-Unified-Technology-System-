-- ARKHER SYSTEM T.0539 :: Animation Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0539"
	S.key = "arkher.ai.animationreasoning.decision_policy"
	S.name = "Animation Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.animationreasoning.reasoning_graph" }
	S.tags = { "t", "animationreasoning", "policy", "ai" }
	S.description = "Animation Reasoning Decision Policy: ordered rules deciding what the agent does next for the Animation Reasoning subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 400,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.animationreasoning.decision_policy", algorithm = "wfq", starvationGuard = 58 })
		inst.system = S
		inst.ctx = ctx
