-- ARKHER SYSTEM W.0090 :: Crane Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0090"
	S.key = "arkher.cine.crane.playback_orchestrator"
	S.name = "Crane Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Crane"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.crane.cut_policy" }
	S.tags = { "w", "crane", "orchestrator", "cine" }
	S.description = "Crane Playback Orchestrator: the state machine that drives a cinematic from start to end for the Crane subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 160,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.crane.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
