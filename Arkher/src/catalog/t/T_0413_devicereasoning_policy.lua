-- ARKHER SYSTEM T.0413 :: Device Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0413"
	S.key = "arkher.ai.devicereasoning.decision_policy"
	S.name = "Device Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.devicereasoning.reasoning_graph" }
	S.tags = { "t", "devicereasoning", "policy", "ai" }
	S.description = "Device Reasoning Decision Policy: ordered rules deciding what the agent does next for the Device Reasoning subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 600,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.devicereasoning.decision_policy", algorithm = "lottery", starvationGuard = 27 })
		inst.system = S
		inst.ctx = ctx
