-- ARKHER SYSTEM VA.0004 :: World Grid Cell Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0004"
	S.key = "arkher.contworld.worldcell.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.worldcell.sparse_residency" }
	S.tags = { "va", "worldcell", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.worldcell.multi-scale_fidelity", levels = 4, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.worldcell.*", function(payload) inst.lastSignal = payload end)
			end
