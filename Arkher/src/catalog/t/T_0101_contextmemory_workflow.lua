-- ARKHER SYSTEM T.0101 :: Context Memory Plan Workflow
...
return function(A)
	...
	local S = {}
	S.id = "T.0101"
	S.key = "arkher.ai.contextmemory.plan_workflow"
	S.name = "Context Memory Plan Workflow"
	S.category = "T"
	...
	S.kit = "workflow"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.contextmemory.semantic_knowledge" }
	S.tags = { "t", "contextmemory", "workflow", "ai" }
	S.description = "Context Memory Plan Workflow: dependency-ordered plans that verify, retry and roll back for the Context Memory subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 400,
		baseWeight = 0.64,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("workflow", { id = "arkher.ai.contextmemory.plan_workflow", maxRetries = 1, budget = 100 })
		inst.system = S
		inst.ctx = ctx
