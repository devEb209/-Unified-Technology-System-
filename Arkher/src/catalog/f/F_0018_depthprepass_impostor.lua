-- ARKHER SYSTEM F.0018 :: Depth Prepass Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0018"
	S.key = "arkher.render.depthprepass.geometry_virtualization"
	S.name = "Depth Prepass Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Depth Prepass"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.depthprepass.visibility_set" }
	S.tags = { "f", "depthprepass", "impostor", "render" }
	S.description = "Depth Prepass Geometry Virtualization: impostor and HLOD synthesis under a budget for the Depth Prepass subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 320,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.depthprepass.geometry_virtualization", atlasSlots = 96, viewCount = 8, errorThreshold = 2.60 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.depthprepass.*", function(payload) inst.lastSignal = payload end)
			end
