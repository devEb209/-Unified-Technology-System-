-- ARKHER SYSTEM VA.0520 :: Asset Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0520"
	S.key = "arkher.contworld.assetcont.multi-scale_fidelity"
	S.name = "Asset Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.assetcont.sparse_residency" }
	S.tags = { "va", "assetcont", "multiscale", "contworld" }
	S.description = "Asset Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Asset Continuum subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 160,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.assetcont.multi-scale_fidelity", levels = 4, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.assetcont.*", function(payload) inst.lastSignal = payload end)
			end
