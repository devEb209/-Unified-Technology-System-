--!strict
-- Source2 Hammer — CORE Stack #0270
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Source2 Hammer — CORE Stack #0270", size=Vector2.new(600, 420), pos=UDim2.fromOffset(410, 250), parent=parent, icon="♪"})
...
Components.Slider(props, "Strength", 0,1,0.5, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_S"..270, v) end end)
Components.Checkbox(props, "Enabled", true, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_E"..270, v) end end)
