-- ARKHER SYSTEM G.0010 :: Neural Upscaling Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0010"
	S.key = "arkher.neural.neuralupscale.heuristic_policy"
	S.name = "Neural Upscaling Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.neuralupscale.result_cache" }
	S.tags = { "g", "neuralupscale", "policy", "neural" }
	S.description = "Neural Upscaling Heuristic Policy: rule fallback when the model is unavailable for the Neural Upscaling subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 360,
		baseWeight = 0.75,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.neuralupscale.heuristic_policy", algorithm = "edf", starvationGuard = 57 })
		inst.system = S
		inst.ctx = ctx
