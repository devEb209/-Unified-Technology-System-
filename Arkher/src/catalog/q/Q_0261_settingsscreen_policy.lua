-- ARKHER SYSTEM Q.0261 :: Settings Screen Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0261"
	S.key = "arkher.ui.settingsscreen.accessibility_policy"
	S.name = "Settings Screen Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Settings Screen"
	S.aspect = "Accessibility Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.settingsscreen.interaction_ledger" }
	S.tags = { "q", "settingsscreen", "policy", "ui" }
	S.description = "Settings Screen Accessibility Policy: rules enforcing contrast, target size and motion limits for the Settings Screen subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 520,
		baseWeight = 0.73,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.settingsscreen.accessibility_policy", algorithm = "edf", starvationGuard = 37 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.settingsscreen.*", function(payload) inst.lastSignal = payload end)
			end
