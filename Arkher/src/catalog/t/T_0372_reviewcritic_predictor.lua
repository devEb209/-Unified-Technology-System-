-- ARKHER SYSTEM T.0372 :: Review Critic Outcome Prediction
...
return function(A)
	...
	local S = {}
	S.id = "T.0372"
	S.key = "arkher.ai.reviewcritic.outcome_prediction"
	S.name = "Review Critic Outcome Prediction"
	S.category = "T"
	...
	S.kit = "predictor"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.reviewcritic.decision_policy" }
	S.tags = { "t", "reviewcritic", "predictor", "ai" }
	S.description = "Review Critic Outcome Prediction: forecasting the result before the work is done for the Review Critic subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.64,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("predictor", { id = "arkher.ai.reviewcritic.outcome_prediction", method = "markov", alpha = 0.34, window = 88 })
		inst.system = S
		inst.ctx = ctx
