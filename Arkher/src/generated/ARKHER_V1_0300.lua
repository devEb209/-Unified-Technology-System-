--!strict
-- ESA WorldCover — CORE Stack #0300
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="ESA WorldCover — CORE Stack #0300", size=Vector2.new(600, 420), pos=UDim2.fromOffset(400, 240), parent=parent, icon="⛰"})
    win.Root.Name="ARKHER_V1_0300"
    win.Root.Visible=false
    local content=win.Content
    content.BackgroundColor3=Theme.tokens.void
    -- accent line per UI
    local accent=Instance.new("Frame"); accent.Size=UDim2.new(1,0,0,2); accent.BackgroundColor3=Theme.categoryColor("ESA Worl"); accent.BorderSizePixel=0; accent.Parent=content
...
Components.Slider(props, "Strength", 0,1,0.5, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_S"..300, v) end end)
Components.Checkbox(props, "Enabled", true, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_E"..300, v) end end)
