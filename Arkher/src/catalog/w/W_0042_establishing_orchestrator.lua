-- ARKHER SYSTEM W.0042 :: Establishing Shot Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0042"
	S.key = "arkher.cine.establishing.playback_orchestrator"
	S.name = "Establishing Shot Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Establishing Shot"
	S.aspect = "Playback Orchestrator"
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.establishing.cut_policy" }
	S.tags = { "w", "establishing", "orchestrator", "cine" }
	S.description = "Establishing Shot Playback Orchestrator: the state machine that drives a cinematic from start to end for the Establishing Shot subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 360,
		baseWeight = 0.63,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.establishing.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.establishing.*", function(payload) inst.lastSignal = payload end)
			end
