-- ARKHER SYSTEM J.0073 :: Teeth Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0073"
	S.key = "arkher.character.teeth.appearance_composer"
	S.name = "Teeth Appearance Composer"
	S.category = "J"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.teeth.locomotion" }
	S.tags = { "j", "teeth", "composer", "character" }
	S.description = "Teeth Appearance Composer: composition of appearance slots into a look for the Teeth subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 520,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.teeth.appearance_composer", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.teeth.*", function(payload) inst.lastSignal = payload end)
			end
