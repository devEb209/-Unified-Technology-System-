-- ARKHER SYSTEM T.0367 :: Review Critic Plan Workflow
...
return function(A)
	...
	local S = {}
	S.id = "T.0367"
	S.key = "arkher.ai.reviewcritic.plan_workflow"
	S.name = "Review Critic Plan Workflow"
	S.category = "T"
	...
	S.kit = "workflow"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.reviewcritic.semantic_knowledge" }
	S.tags = { "t", "reviewcritic", "workflow", "ai" }
	S.description = "Review Critic Plan Workflow: dependency-ordered plans that verify, retry and roll back for the Review Critic subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 320,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("workflow", { id = "arkher.ai.reviewcritic.plan_workflow", maxRetries = 2, budget = 80 })
		inst.system = S
		inst.ctx = ctx
