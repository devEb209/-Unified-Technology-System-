-- ARKHER SYSTEM L.0011 :: Living World Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0011"
	S.key = "arkher.sim.livingworld.fidelity_policy"
	S.name = "Living World Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.livingworld.streaming" }
	S.tags = { "l", "livingworld", "policy", "sim" }
	S.description = "Living World Fidelity Policy: rules that pick full, cohort or statistical simulation for the Living World subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 360,
		baseWeight = 0.73,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.livingworld.fidelity_policy", algorithm = "edf", starvationGuard = 45 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.livingworld.*", function(payload) inst.lastSignal = payload end)
			end
