-- ARKHER SYSTEM K.0237 :: Personality Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0237"
	S.key = "arkher.npc.personality.thinking_budget"
	S.name = "Personality Thinking Budget"
	S.category = "K"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.personality.daily_schedule" }
	S.tags = { "k", "personality", "budgeter", "npc" }
	S.description = "Personality Thinking Budget: allocation of per-frame thinking time across NPCs for the Personality subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 240,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.personality.thinking_budget", total = 394, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.personality.*", function(payload) inst.lastSignal = payload end)
			end
