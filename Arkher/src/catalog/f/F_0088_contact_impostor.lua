-- ARKHER SYSTEM F.0088 :: Contact Shadows Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0088"
	S.key = "arkher.render.contact.geometry_virtualization"
	S.name = "Contact Shadows Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Contact Shadows"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.contact.visibility_set" }
	S.tags = { "f", "contact", "impostor", "render" }
	S.description = "Contact Shadows Geometry Virtualization: impostor and HLOD synthesis under a budget for the Contact Shadows subsystem."
	S.params = {
		backlogLimit = 14,
		baseRadius = 240,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.contact.geometry_virtualization", atlasSlots = 64, viewCount = 8, errorThreshold = 3.60 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.contact.*", function(payload) inst.lastSignal = payload end)
			end
