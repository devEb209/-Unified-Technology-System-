-- ARKHER SYSTEM L.0131 :: Village Life Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0131"
	S.key = "arkher.sim.villagelife.fidelity_policy"
	S.name = "Village Life Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.villagelife.streaming" }
	S.tags = { "l", "villagelife", "policy", "sim" }
	S.description = "Village Life Fidelity Policy: rules that pick full, cohort or statistical simulation for the Village Life subsystem."
	S.params = {
		backlogLimit = 15,
		baseRadius = 440,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.villagelife.fidelity_policy", algorithm = "lottery", starvationGuard = 47 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.villagelife.*", function(payload) inst.lastSignal = payload end)
			end
