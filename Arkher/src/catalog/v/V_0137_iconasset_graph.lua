-- ARKHER SYSTEM V.0137 :: Icon Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0137"
	S.key = "arkher.asset.iconasset.dependency_graph"
	S.name = "Icon Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.iconasset.asset_registry" }
	S.tags = { "v", "iconasset", "graph", "asset" }
	S.description = "Icon Dependency Graph: what references what, and in which order it must be built for the Icon subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 280,
		baseWeight = 0.53,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.iconasset.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.iconasset.*", function(payload) inst.lastSignal = payload end)
			end
