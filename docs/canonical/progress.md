...
## % ATUAL: **85%** (após R11 — 5 sistemas de uma vez: quaternions+ragdoll, pinna+reverb, deltas, névoa por altura, galeria; 84.98 exato)
...
| 5 | **Usabilidade para todos** — demo browser completo **COM TOUR GUIADO + GALERIA DE MUNDOS (4 presets reais: alvorada com névoa, arquipélago com vento no mar, vale em chamas, cidade viva — via ?world=)**, render + IA + anexos + áudio + save/load + HUD, CLI, docs, Termux | 10 | 90% | 9.00 | falta: empacotamento 1-comando |
| 6 | **Física como realidade** — translação, impactos causais, **energia ½mv² + deformação persistente por MATERIAL (rocha/madeira/gelo/carne)**, rotação+torque, **ROTAÇÃO LIVRE 3D POR QUATERNION (q̇=½ω⊗q, yaw derivado — uma verdade)**, **RAGDOLL (7 segmentos, 6 juntas PBD, sobrevive save/load)**, juntas PBD, raycast, persistência | 8 | 77% | 6.16 | falta: fluidos, colisão corpo-a-corpo do ragdoll |
| 7 | **Áudio como fenômeno** — synth, binaural ITD+ILD, música adaptativa, stream, devices, WAV, **ACÚSTICA REAL (atraso d/343, sombra de terreno, absorção do ar)**, **MUFFLE = FILTRO REAL**, **PINNA (a concha filtra por elevação 5.5–10.5kHz + sombra do torso: frente vs fundo)**, **REVERB POR AMBIENTE (Schroeder: sala/vale/cânion com RT60 real)**, trovão longe = rumble grave | 8 | 88% | 7.04 | falta: HRTF medida (base de dados), absorção por material |
| 8 | **Streaming + Persistência** — residência LOD, histerese, UTS-DB, autosave, fenômenos persistem, **STREAMING ASSÍNCRONO (worker_threads byte-idêntico)**, **DELTAS POR-ENTIDADE (componentes carimbados __v; deltaSince/applyDelta — o clone acompanha o original só com o que mudou)** | 8 | 92% | 7.36 | falta: compressão do tail, transporte de rede real |
