-- ARKHER SYSTEM G.0234 :: Animation Compression Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0234"
	S.key = "arkher.neural.animcompress.heuristic_policy"
	S.name = "Animation Compression Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.animcompress.result_cache" }
	S.tags = { "g", "animcompress", "policy", "neural" }
	S.description = "Animation Compression Heuristic Policy: rule fallback when the model is unavailable for the Animation Compression subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 440,
		baseWeight = 0.75,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.animcompress.heuristic_policy", algorithm = "lottery", starvationGuard = 59 })
		inst.system = S
		inst.ctx = ctx
