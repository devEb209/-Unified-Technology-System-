-- ARKHER SYSTEM VD.0364 :: Quest Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0364"
	S.key = "arkher.contsim.questcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.questcont.sparse_residency" }
	S.tags = { "vd", "questcont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.questcont.multi-scale_fidelity", levels = 4, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.questcont.*", function(payload) inst.lastSignal = payload end)
			end
