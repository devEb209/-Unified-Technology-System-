-- ARKHER SYSTEM VC.0160 :: Coherence Seam Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0160"
	S.key = "arkher.contneural.coherenceseam.multi-scale_fidelity"
	S.name = "Coherence Seam Multi-Scale Fidelity"
	S.category = "VC"
	S.family = "CONTINUUM — NEURAL"
	S.area = "Coherence Seam"
	S.aspect = "Multi-Scale Fidelity"
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.coherenceseam.sparse_residency" }
	S.tags = { "vc", "coherenceseam", "multiscale", "contneural" }
	S.description = "Coherence Seam Multi-Scale Fidelity: distance-driven multi-scale level selection for the Coherence Seam subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 480,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.coherenceseam.multi-scale_fidelity", levels = 6, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.coherenceseam.*", function(payload) inst.lastSignal = payload end)
			end
