-- ARKHER SYSTEM I.0344 :: Physical Animation Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0344"
	S.key = "arkher.anim.physanim.retarget_graph"
	S.name = "Physical Animation Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Physical Animation"
	S.aspect = "Retarget Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.physanim.physical_blend" }
	S.tags = { "i", "physanim", "graph", "anim" }
	S.description = "Physical Animation Retarget Graph: bone mapping and retarget path resolution for the Physical Animation subsystem."
	S.params = {
		backlogLimit = 15,
		baseRadius = 600,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.physanim.retarget_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.physanim.*", function(payload) inst.lastSignal = payload end)
			end
