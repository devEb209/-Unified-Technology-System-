--!strict
-- Creation2 — ANIMATION Profiler #0109
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Creation2 — ANIMATION Profiler #0109", size=Vector2.new(640, 500), pos=UDim2.fromOffset(317, 113), parent=parent, icon="≡"})
...
Components.TextField(form, "Name", "Object_"..109, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
