-- ARKHER SYSTEM K.0559 :: Trade Behaviour Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0559"
	S.key = "arkher.npc.tradebehaviour.thinking_budget"
	S.name = "Trade Behaviour Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Trade Behaviour"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.tradebehaviour.daily_schedule" }
	S.tags = { "k", "tradebehaviour", "budgeter", "npc" }
	S.description = "Trade Behaviour Thinking Budget: allocation of per-frame thinking time across NPCs for the Trade Behaviour subsystem."
	S.params = {
		backlogLimit = 39,
		baseRadius = 440,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.tradebehaviour.thinking_budget", total = 171, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.tradebehaviour.*", function(payload) inst.lastSignal = payload end)
			end
