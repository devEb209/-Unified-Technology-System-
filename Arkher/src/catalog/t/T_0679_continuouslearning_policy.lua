-- ARKHER SYSTEM T.0679 :: Continuous Learning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0679"
	S.key = "arkher.ai.continuouslearning.decision_policy"
	S.name = "Continuous Learning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.continuouslearning.reasoning_graph" }
	S.tags = { "t", "continuouslearning", "policy", "ai" }
	S.description = "Continuous Learning Decision Policy: ordered rules deciding what the agent does next for the Continuous Learning subsystem."
	S.params = {
		backlogLimit = 9,
		baseRadius = 200,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.continuouslearning.decision_policy", algorithm = "edf", starvationGuard = 41 })
		inst.system = S
		inst.ctx = ctx
