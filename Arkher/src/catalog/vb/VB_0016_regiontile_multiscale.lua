-- ARKHER SYSTEM VB.0016 :: Region Tile Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0016"
	S.key = "arkher.conttime.regiontile.multi-scale_fidelity"
	S.name = "Region Tile Multi-Scale Fidelity"
	S.category = "VB"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.regiontile.sparse_residency" }
	S.tags = { "vb", "regiontile", "multiscale", "conttime" }
	S.description = "Region Tile Multi-Scale Fidelity: distance-driven multi-scale level selection for the Region Tile subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 320,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.regiontile.multi-scale_fidelity", levels = 5, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.regiontile.*", function(payload) inst.lastSignal = payload end)
			end
