-- ARKHER SYSTEM T.0391 :: Refactoring Learning Analysis
...
return function(A)
	...
	local S = {}
	S.id = "T.0391"
	S.key = "arkher.ai.refactoring.learning_analysis"
	S.name = "Refactoring Learning Analysis"
	S.category = "T"
	...
	S.kit = "analyzer"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.refactoring.model_inference" }
	S.tags = { "t", "refactoring", "analyzer", "ai" }
	S.description = "Refactoring Learning Analysis: trend, anomaly and improvement analysis of the agent for the Refactoring subsystem."
	S.params = {
		backlogLimit = 37,
		baseRadius = 360,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("analyzer", { id = "arkher.ai.refactoring.learning_analysis", window = 237, buckets = 13, threshold = 4.90 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.summary()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ai.refactoring.*", function(payload) inst.lastSignal = payload end)
			end
