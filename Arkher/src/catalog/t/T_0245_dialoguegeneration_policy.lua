-- ARKHER SYSTEM T.0245 :: Dialogue Generation Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0245"
	S.key = "arkher.ai.dialoguegeneration.decision_policy"
	S.name = "Dialogue Generation Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.dialoguegeneration.reasoning_graph" }
	S.tags = { "t", "dialoguegeneration", "policy", "ai" }
	S.description = "Dialogue Generation Decision Policy: ordered rules deciding what the agent does next for the Dialogue Generation subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 480,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.dialoguegeneration.decision_policy", algorithm = "roundrobin", starvationGuard = 60 })
		inst.system = S
		inst.ctx = ctx
