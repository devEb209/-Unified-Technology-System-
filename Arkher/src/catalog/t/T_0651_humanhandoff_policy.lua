-- ARKHER SYSTEM T.0651 :: Human Handoff Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0651"
	S.key = "arkher.ai.humanhandoff.decision_policy"
	S.name = "Human Handoff Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.humanhandoff.reasoning_graph" }
	S.tags = { "t", "humanhandoff", "policy", "ai" }
	S.description = "Human Handoff Decision Policy: ordered rules deciding what the agent does next for the Human Handoff subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 240,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.humanhandoff.decision_policy", algorithm = "wfq", starvationGuard = 54 })
		inst.system = S
		inst.ctx = ctx
