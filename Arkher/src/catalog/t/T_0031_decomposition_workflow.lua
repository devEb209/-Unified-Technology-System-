-- ARKHER SYSTEM T.0031 :: Task Decomposition Plan Workflow
...
return function(A)
	...
	local S = {}
	S.id = "T.0031"
	S.key = "arkher.ai.decomposition.plan_workflow"
	S.name = "Task Decomposition Plan Workflow"
	S.category = "T"
	...
	S.kit = "workflow"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.decomposition.semantic_knowledge" }
	S.tags = { "t", "decomposition", "workflow", "ai" }
	S.description = "Task Decomposition Plan Workflow: dependency-ordered plans that verify, retry and roll back for the Task Decomposition subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 240,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("workflow", { id = "arkher.ai.decomposition.plan_workflow", maxRetries = 3, budget = 60 })
		inst.system = S
		inst.ctx = ctx
