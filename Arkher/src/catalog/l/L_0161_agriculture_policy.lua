-- ARKHER SYSTEM L.0161 :: Agriculture Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0161"
	S.key = "arkher.sim.agriculture.fidelity_policy"
	S.name = "Agriculture Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.agriculture.streaming" }
	S.tags = { "l", "agriculture", "policy", "sim" }
	S.description = "Agriculture Fidelity Policy: rules that pick full, cohort or statistical simulation for the Agriculture subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 320,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.agriculture.fidelity_policy", algorithm = "roundrobin", starvationGuard = 32 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.agriculture.*", function(payload) inst.lastSignal = payload end)
			end
