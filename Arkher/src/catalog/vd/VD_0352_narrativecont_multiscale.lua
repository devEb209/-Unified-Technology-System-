-- ARKHER SYSTEM VD.0352 :: Narrative Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0352"
	S.key = "arkher.contsim.narrativecont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.narrativecont.sparse_residency" }
	S.tags = { "vd", "narrativecont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.narrativecont.multi-scale_fidelity", levels = 5, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.narrativecont.*", function(payload) inst.lastSignal = payload end)
			end
