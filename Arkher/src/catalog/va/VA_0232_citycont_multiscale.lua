-- ARKHER SYSTEM VA.0232 :: City Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0232"
	S.key = "arkher.contworld.citycont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.citycont.sparse_residency" }
	S.tags = { "va", "citycont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.citycont.multi-scale_fidelity", levels = 5, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.citycont.*", function(payload) inst.lastSignal = payload end)
			end
