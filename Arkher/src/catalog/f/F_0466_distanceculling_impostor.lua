-- ARKHER SYSTEM F.0466 :: Distance Culling Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0466"
	S.key = "arkher.render.distanceculling.geometry_virtualization"
	S.name = "Distance Culling Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Distance Culling"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.distanceculling.visibility_set" }
	S.tags = { "f", "distanceculling", "impostor", "render" }
	S.description = "Distance Culling Geometry Virtualization: impostor and HLOD synthesis under a budget for the Distance Culling subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 320,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.distanceculling.geometry_virtualization", atlasSlots = 96, viewCount = 8, errorThreshold = 1.40 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.distanceculling.*", function(payload) inst.lastSignal = payload end)
			end
