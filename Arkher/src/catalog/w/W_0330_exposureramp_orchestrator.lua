-- ARKHER SYSTEM W.0330 :: Exposure Ramp Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0330"
	S.key = "arkher.cine.exposureramp.playback_orchestrator"
	S.name = "Exposure Ramp Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Exposure Ramp"
	S.aspect = "Playback Orchestrator"
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.exposureramp.cut_policy" }
	S.tags = { "w", "exposureramp", "orchestrator", "cine" }
	S.description = "Exposure Ramp Playback Orchestrator: the state machine that drives a cinematic from start to end for the Exposure Ramp subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 200,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.exposureramp.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.exposureramp.*", function(payload) inst.lastSignal = payload end)
			end
