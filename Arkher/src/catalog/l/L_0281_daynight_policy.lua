-- ARKHER SYSTEM L.0281 :: Day Night Cycle Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0281"
	S.key = "arkher.sim.daynight.fidelity_policy"
	S.name = "Day Night Cycle Fidelity Policy"
	S.category = "L"
	S.family = "WORLD SIMULATION"
	S.area = "Day Night Cycle"
	S.aspect = "Fidelity Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.daynight.streaming" }
	S.tags = { "l", "daynight", "policy", "sim" }
	S.description = "Day Night Cycle Fidelity Policy: rules that pick full, cohort or statistical simulation for the Day Night Cycle subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 160,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.daynight.fidelity_policy", algorithm = "roundrobin", starvationGuard = 28 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.daynight.*", function(payload) inst.lastSignal = payload end)
			end
