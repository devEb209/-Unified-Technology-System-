-- ARKHER SYSTEM Q.0033 :: Button Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0033"
	S.key = "arkher.ui.button.accessibility_policy"
	S.name = "Button Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Button"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.button.interaction_ledger" }
	S.tags = { "q", "button", "policy", "ui" }
	S.description = "Button Accessibility Policy: rules enforcing contrast, target size and motion limits for the Button subsystem."
	S.params = {
		backlogLimit = 15,
		baseRadius = 600,
		baseWeight = 0.57,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.button.accessibility_policy", algorithm = "lottery", starvationGuard = 63 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.button.*", function(payload) inst.lastSignal = payload end)
			end
