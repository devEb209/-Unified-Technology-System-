#!/usr/bin/env python3
"""GENESIS V1.49 SPECIALIZED 10l — +10 únicos 12/33 (126/332) sem parar — Singularity"""
...
def patch49(roots):
    items=[
        ("ARKHER_SingCore","SINGULARITY — Core  •  AGI",0x7C4DFF,"Routing  •  Models  •  Tools","Routing Models Tools  •  AGI Unaware"),
        ("ARKHER_SingMem","SING MEMORY — Context  •  Recall",0x651FFF,"Memory  •  Context  •  Embed","Memory Context Embed  •  128k"),
        ("ARKHER_SingVerify","SING VERIFY — Fallback  •  Check",0xFF6F00,"Verify  •  Fallback  •  Score","Verify Fallback Score  •  99.9%"),
        ("ARKHER_SingMulti","SING MULTI — Vision  •  Audio",0x00ACC1,"Vision  •  Audio  •  Text","Vision Audio Text  •  Multimodal"),
        ("ARKHER_SingFront","SING FRONT — Chat  •  UI",0x0288D1,"Chat  •  UI  •  Stream","Chat UI Stream  •  Independente"),
        ("ARKHER_D_O15","D-O15 — Opt  •  Mobile AAA",0x1B5E20,"620 Sys  •  9300 Feat  •  Mobile","620 Systems 9300 Features  •  Mobile AAA"),
        ("ARKHER_Neural","NEURAL — DLSS  •  FSR",0xAD1457,"DLSS  •  FSR  •  XeSS","DLSS FSR XeSS  •  Neural Rendering"),
        ("ARKHER_MobileAAA","MOBILE AAA — Perf  •  Battery",0x33691E,"Perf  •  Battery  •  Therm","Perf Battery Therm  •  60FPS"),
        ("ARKHER_Streaming2","STREAM2 — World  •  Texture",0x01579B,"World  •  Texture  •  Mesh","World Texture Mesh  •  Virtual"),
        ("ARKHER_VirtualGeom","VIRTUAL GEOM — Nanite  •  Cluster",0x37474F,"Cluster  •  DAG  •  Culling","Cluster DAG Culling  •  Nanite"),
    ]
...
for _,n in ipairs({"ARKHER_SingCore","ARKHER_SingMem","ARKHER_SingVerify","ARKHER_SingMulti","ARKHER_SingFront","ARKHER_D_O15","ARKHER_Neural","ARKHER_MobileAAA","ARKHER_Streaming2","ARKHER_VirtualGeom"}) do
...
print("[V1.49 10x] SingCore/SingMem/SingVerify/SingMulti/SingFront/D-O15/Neural/MobileAAA/Stream2/VirtualGeom 126/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_49",{"Source":(T_STRING, FUNC49)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch48(roots)
    roots=patch49(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_49.rbxl")
    write(out, serialize(roots))
    print(f"V1.49 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_49.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.49 done")
