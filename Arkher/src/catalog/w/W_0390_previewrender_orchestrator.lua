-- ARKHER SYSTEM W.0390 :: Preview Render Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0390"
	S.key = "arkher.cine.previewrender.playback_orchestrator"
	S.name = "Preview Render Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Preview Render"
	S.aspect = "Playback Orchestrator"
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.previewrender.cut_policy" }
	S.tags = { "w", "previewrender", "orchestrator", "cine" }
	S.description = "Preview Render Playback Orchestrator: the state machine that drives a cinematic from start to end for the Preview Render subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 240,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.previewrender.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.previewrender.*", function(payload) inst.lastSignal = payload end)
			end
