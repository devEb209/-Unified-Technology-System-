-- ARKHER SYSTEM N.0319 :: Trail Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0319"
	S.key = "arkher.vfx.trail.effect_composition"
	S.name = "Trail Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Trail"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.trail.effect_graph" }
	S.tags = { "n", "trail", "composer", "vfx" }
	S.description = "Trail Effect Composition: blending of several effect signals into one output for the Trail subsystem."
	S.params = {
		backlogLimit = 47,
		baseRadius = 280,
		baseWeight = 0.69,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.trail.effect_composition", mode = "overlay" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.trail.*", function(payload) inst.lastSignal = payload end)
			end
