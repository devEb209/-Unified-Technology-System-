-- ARKHER SYSTEM N.0332 :: Ribbon Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0332"
	S.key = "arkher.vfx.ribbonfx.effect_composition"
	S.name = "Ribbon Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Ribbon"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.ribbonfx.effect_graph" }
	S.tags = { "n", "ribbonfx", "composer", "vfx" }
	S.description = "Ribbon Effect Composition: blending of several effect signals into one output for the Ribbon subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 600,
		baseWeight = 0.53,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.ribbonfx.effect_composition", mode = "overlay" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.ribbonfx.*", function(payload) inst.lastSignal = payload end)
			end
