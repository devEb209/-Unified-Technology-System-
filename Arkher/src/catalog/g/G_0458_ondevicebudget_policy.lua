-- ARKHER SYSTEM G.0458 :: On Device Budget Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0458"
	S.key = "arkher.neural.ondevicebudget.heuristic_policy"
	S.name = "On Device Budget Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.ondevicebudget.result_cache" }
	S.tags = { "g", "ondevicebudget", "policy", "neural" }
	S.description = "On Device Budget Heuristic Policy: rule fallback when the model is unavailable for the On Device Budget subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 320,
		baseWeight = 0.9,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.ondevicebudget.heuristic_policy", algorithm = "roundrobin", starvationGuard = 44 })
		inst.system = S
		inst.ctx = ctx
