-- ARKHER SYSTEM J.0249 :: Aging Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0249"
	S.key = "arkher.character.aging.appearance_composer"
	S.name = "Aging Appearance Composer"
	S.category = "J"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.aging.locomotion" }
	S.tags = { "j", "aging", "composer", "character" }
	S.description = "Aging Appearance Composer: composition of appearance slots into a look for the Aging subsystem."
	S.params = {
		backlogLimit = 29,
		baseRadius = 360,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.aging.appearance_composer", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.aging.*", function(payload) inst.lastSignal = payload end)
			end
