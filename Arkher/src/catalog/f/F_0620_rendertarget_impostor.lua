-- ARKHER SYSTEM F.0620 :: Render Target Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0620"
	S.key = "arkher.render.rendertarget.geometry_virtualization"
	S.name = "Render Target Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Render Target"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.rendertarget.visibility_set" }
	S.tags = { "f", "rendertarget", "impostor", "render" }
	S.description = "Render Target Geometry Virtualization: impostor and HLOD synthesis under a budget for the Render Target subsystem."
	S.params = {
		backlogLimit = 34,
		baseRadius = 560,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.rendertarget.geometry_virtualization", atlasSlots = 96, viewCount = 8, errorThreshold = 2.60 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.rendertarget.*", function(payload) inst.lastSignal = payload end)
			end
