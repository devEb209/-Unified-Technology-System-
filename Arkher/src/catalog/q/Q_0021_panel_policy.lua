-- ARKHER SYSTEM Q.0021 :: Panel Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0021"
	S.key = "arkher.ui.panel.accessibility_policy"
	S.name = "Panel Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Panel"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.panel.interaction_ledger" }
	S.tags = { "q", "panel", "policy", "ui" }
	S.description = "Panel Accessibility Policy: rules enforcing contrast, target size and motion limits for the Panel subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.panel.accessibility_policy", algorithm = "roundrobin", starvationGuard = 32 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.panel.*", function(payload) inst.lastSignal = payload end)
			end
