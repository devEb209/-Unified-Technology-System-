#!/usr/bin/env python3
"""GENESIS V1.65 SPECIALIZED 10ab — +10 únicos 28/33 (286/332) sem parar"""
...
def patch65(roots):
    items=[
        ("ARKHER_Legal","LEGAL — EULA  •  DMCA",0x3E2723,"EULA  •  DMCA  •  Trademark","EULA DMCA Trademark  •  Legal"),
        ("ARKHER_Copyright","COPYRIGHT — Claim  •  Takedown",0x5D4037,"Claim  •  Takedown  •  Counter","Claim Takedown Counter  •  DMCA"),
        ("ARKHER_Payment","PAYMENT — Method  •  Invoice",0x1B5E20,"Method  •  Invoice  •  Tax","Method Invoice Tax  •  Visa"),
        ("ARKHER_Billing","BILLING — Cycle  •  Receipt",0x2E7D32,"Cycle  •  Receipt  •  Refund","Cycle Receipt Refund  •  Monthly"),
        ("ARKHER_Invoice2","INVOICE2 — Number  •  Due",0x33691E,"Number  •  Due  •  Paid","Number Due Paid  •  INV-2026"),
        ("ARKHER_Refund","REFUND — Reason  •  Status",0xAD1457,"Reason  •  Status  •  Amount","Reason Status Amount  •  $9.99"),
        ("ARKHER_Subscription","SUBSCRIPTION — Plan  •  Renew",0x7C4DFF,"Plan  •  Renew  •  Cancel","Plan Renew Cancel  •  Pro $19"),
        ("ARKHER_Trial","TRIAL — Days  •  Convert",0x651FFF,"Days  •  Convert  •  Reminder","Days Convert Reminder  •  14 days"),
        ("ARKHER_Coupon2","COUPON2 — Apply  •  Stack",0xFF6F00,"Apply  •  Stack  •  Limit","Apply Stack Limit  •  1 per user"),
        ("ARKHER_Discount","DISCOUNT — Percent  •  Fixed",0xFFD600,"Percent  •  Fixed  •  Cap","Percent Fixed Cap  •  20%"),
    ]
...
for _,n in ipairs({"ARKHER_Legal","ARKHER_Copyright","ARKHER_Payment","ARKHER_Billing","ARKHER_Invoice2","ARKHER_Refund","ARKHER_Subscription","ARKHER_Trial","ARKHER_Coupon2","ARKHER_Discount"}) do
...
print("[V1.65 10x] Legal/Copyright/Payment/Billing/Invoice2/Refund/Subscription/Trial/Coupon2/Discount 286/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_65",{"Source":(T_STRING, FUNC65)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch64(roots)
    roots=patch65(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_65.rbxl")
    write(out, serialize(roots))
    print(f"V1.65 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_65.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.65 done")
