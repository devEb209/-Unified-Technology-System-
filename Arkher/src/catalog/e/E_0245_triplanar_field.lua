-- ARKHER SYSTEM E.0245 :: Triplanar Projection Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0245"
	S.key = "arkher.material.triplanar.procedural_detail"
	S.name = "Triplanar Projection Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Triplanar Projection"
	S.aspect = "Procedural Detail"
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.triplanar.texture_sampling" }
	S.tags = { "e", "triplanar", "field", "material" }
	S.description = "Triplanar Projection Procedural Detail: coherent noise detail driving the surface for the Triplanar Projection subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 360,
		baseWeight = 0.69,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.triplanar.procedural_detail", seed = 40169, frequency = 0.0063, octaves = 4, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.triplanar.*", function(payload) inst.lastSignal = payload end)
			end
