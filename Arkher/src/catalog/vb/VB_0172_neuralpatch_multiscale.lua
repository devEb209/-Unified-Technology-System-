-- ARKHER SYSTEM VB.0172 :: Neural Patch Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0172"
	S.key = "arkher.conttime.neuralpatch.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.neuralpatch.sparse_residency" }
	S.tags = { "vb", "neuralpatch", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.neuralpatch.multi-scale_fidelity", levels = 5, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.neuralpatch.*", function(payload) inst.lastSignal = payload end)
			end
