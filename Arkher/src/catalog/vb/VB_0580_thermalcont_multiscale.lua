-- ARKHER SYSTEM VB.0580 :: Thermal Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0580"
	S.key = "arkher.conttime.thermalcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.thermalcont.sparse_residency" }
	S.tags = { "vb", "thermalcont", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.thermalcont.multi-scale_fidelity", levels = 4, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.thermalcont.*", function(payload) inst.lastSignal = payload end)
			end
