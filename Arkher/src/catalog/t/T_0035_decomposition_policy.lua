-- ARKHER SYSTEM T.0035 :: Task Decomposition Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0035"
	S.key = "arkher.ai.decomposition.decision_policy"
	S.name = "Task Decomposition Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.decomposition.reasoning_graph" }
	S.tags = { "t", "decomposition", "policy", "ai" }
	S.description = "Task Decomposition Decision Policy: ordered rules deciding what the agent does next for the Task Decomposition subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.decomposition.decision_policy", algorithm = "wfq", starvationGuard = 46 })
		inst.system = S
		inst.ctx = ctx
