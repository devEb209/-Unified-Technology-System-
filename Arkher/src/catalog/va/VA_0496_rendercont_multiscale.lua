-- ARKHER SYSTEM VA.0496 :: Rendering Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0496"
	S.key = "arkher.contworld.rendercont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.rendercont.sparse_residency" }
	S.tags = { "va", "rendercont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.rendercont.multi-scale_fidelity", levels = 5, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.rendercont.*", function(payload) inst.lastSignal = payload end)
			end
