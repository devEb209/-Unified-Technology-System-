-- ARKHER SYSTEM VB.0220 :: Terrain Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0220"
	S.key = "arkher.conttime.terraincont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.terraincont.sparse_residency" }
	S.tags = { "vb", "terraincont", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.terraincont.multi-scale_fidelity", levels = 6, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.terraincont.*", function(payload) inst.lastSignal = payload end)
			end
