-- ARKHER SYSTEM H.0696 :: Overlap Query Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0696"
	S.key = "arkher.physics.overlap.simulation_policy"
	S.name = "Overlap Query Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Overlap Query"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.overlap.budget_governor" }
	S.tags = { "h", "overlap", "policy", "physics" }
	S.description = "Overlap Query Simulation Policy: device tier rules for substeps, iterations and sleep for the Overlap Query subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 160,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.overlap.simulation_policy", algorithm = "roundrobin", starvationGuard = 52 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.overlap.*", function(payload) inst.lastSignal = payload end)
			end
