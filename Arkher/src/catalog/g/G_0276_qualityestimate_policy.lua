-- ARKHER SYSTEM G.0276 :: Quality Estimation Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0276"
	S.key = "arkher.neural.qualityestimate.heuristic_policy"
	S.name = "Quality Estimation Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.qualityestimate.result_cache" }
	S.tags = { "g", "qualityestimate", "policy", "neural" }
	S.description = "Quality Estimation Heuristic Policy: rule fallback when the model is unavailable for the Quality Estimation subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.qualityestimate.heuristic_policy", algorithm = "wfq", starvationGuard = 22 })
		inst.system = S
		inst.ctx = ctx
