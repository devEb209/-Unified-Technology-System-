-- ARKHER SYSTEM T.0609 :: Safety Alignment Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0609"
	S.key = "arkher.ai.safetyalignment.decision_policy"
	S.name = "Safety Alignment Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.safetyalignment.reasoning_graph" }
	S.tags = { "t", "safetyalignment", "policy", "ai" }
	S.description = "Safety Alignment Decision Policy: ordered rules deciding what the agent does next for the Safety Alignment subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 520,
		baseWeight = 0.53,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.safetyalignment.decision_policy", algorithm = "edf", starvationGuard = 61 })
		inst.system = S
		inst.ctx = ctx
