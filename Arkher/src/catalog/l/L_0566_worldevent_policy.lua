-- ARKHER SYSTEM L.0566 :: World Event Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0566"
	S.key = "arkher.sim.worldevent.fidelity_policy"
	S.name = "World Event Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.worldevent.streaming" }
	S.tags = { "l", "worldevent", "policy", "sim" }
	S.description = "World Event Fidelity Policy: rules that pick full, cohort or statistical simulation for the World Event subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 480,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.worldevent.fidelity_policy", algorithm = "roundrobin", starvationGuard = 48 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.worldevent.*", function(payload) inst.lastSignal = payload end)
			end
