-- ARKHER SYSTEM N.0358 :: Laser Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0358"
	S.key = "arkher.vfx.laser.effect_composition"
	S.name = "Laser Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Laser"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.laser.effect_graph" }
	S.tags = { "n", "laser", "composer", "vfx" }
	S.description = "Laser Effect Composition: blending of several effect signals into one output for the Laser subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.laser.effect_composition", mode = "max" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.laser.*", function(payload) inst.lastSignal = payload end)
			end
