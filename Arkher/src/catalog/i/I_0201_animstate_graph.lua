-- ARKHER SYSTEM I.0201 :: State Machine Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0201"
	S.key = "arkher.anim.animstate.retarget_graph"
	S.name = "State Machine Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "State Machine"
	S.aspect = "Retarget Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.animstate.physical_blend" }
	S.tags = { "i", "animstate", "graph", "anim" }
	S.description = "State Machine Retarget Graph: bone mapping and retarget path resolution for the State Machine subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 280,
		baseWeight = 0.87,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.animstate.retarget_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.animstate.*", function(payload) inst.lastSignal = payload end)
			end
