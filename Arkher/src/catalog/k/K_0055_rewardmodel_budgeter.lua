-- ARKHER SYSTEM K.0055 :: Reward Model Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0055"
	S.key = "arkher.npc.rewardmodel.thinking_budget"
	S.name = "Reward Model Thinking Budget"
	S.category = "K"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.rewardmodel.daily_schedule" }
	S.tags = { "k", "rewardmodel", "budgeter", "npc" }
	S.description = "Reward Model Thinking Budget: allocation of per-frame thinking time across NPCs for the Reward Model subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 600,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.rewardmodel.thinking_budget", total = 203, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.rewardmodel.*", function(payload) inst.lastSignal = payload end)
			end
