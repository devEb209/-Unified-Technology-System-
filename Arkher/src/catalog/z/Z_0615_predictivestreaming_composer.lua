-- ARKHER SYSTEM Z.0615 :: Predictive Streaming Signal Composition
...
return function(A)
	...
	local S = {}
	S.id = "Z.0615"
	S.key = "arkher.origin.predictivestreaming.signal_composition"
	S.name = "Predictive Streaming Signal Composition"
	S.category = "Z"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.origin.predictivestreaming.telemetry_ledger" }
	S.tags = { "z", "predictivestreaming", "composer", "origin" }
	S.description = "Predictive Streaming Signal Composition: blending of the technology's input signals into one output for the Predictive Streaming subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 400,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.origin.predictivestreaming.signal_composition", mode = "max" })
		inst.system = S
		inst.ctx = ctx
