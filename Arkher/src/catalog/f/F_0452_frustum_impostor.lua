-- ARKHER SYSTEM F.0452 :: Frustum Culling Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0452"
	S.key = "arkher.render.frustum.geometry_virtualization"
	S.name = "Frustum Culling Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Frustum Culling"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.frustum.visibility_set" }
	S.tags = { "f", "frustum", "impostor", "render" }
	S.description = "Frustum Culling Geometry Virtualization: impostor and HLOD synthesis under a budget for the Frustum Culling subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 600,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.frustum.geometry_virtualization", atlasSlots = 112, viewCount = 8, errorThreshold = 1.50 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.frustum.*", function(payload) inst.lastSignal = payload end)
			end
