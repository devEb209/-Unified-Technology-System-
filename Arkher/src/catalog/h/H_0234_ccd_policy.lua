-- ARKHER SYSTEM H.0234 :: Continuous Detection Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0234"
	S.key = "arkher.physics.ccd.simulation_policy"
	S.name = "Continuous Detection Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Continuous Detection"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.ccd.budget_governor" }
	S.tags = { "h", "ccd", "policy", "physics" }
	S.description = "Continuous Detection Simulation Policy: device tier rules for substeps, iterations and sleep for the Continuous Detection subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 280,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.ccd.simulation_policy", algorithm = "lottery", starvationGuard = 19 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.ccd.*", function(payload) inst.lastSignal = payload end)
			end
