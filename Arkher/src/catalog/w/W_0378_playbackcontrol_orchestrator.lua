-- ARKHER SYSTEM W.0378 :: Playback Control Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0378"
	S.key = "arkher.cine.playbackcontrol.playback_orchestrator"
	S.name = "Playback Control Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Playback Control"
	S.aspect = "Playback Orchestrator"
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.playbackcontrol.cut_policy" }
	S.tags = { "w", "playbackcontrol", "orchestrator", "cine" }
	S.description = "Playback Control Playback Orchestrator: the state machine that drives a cinematic from start to end for the Playback Control subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 480,
		baseWeight = 0.66,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.playbackcontrol.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.playbackcontrol.*", function(payload) inst.lastSignal = payload end)
			end
