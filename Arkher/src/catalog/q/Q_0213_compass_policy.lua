-- ARKHER SYSTEM Q.0213 :: Compass Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0213"
	S.key = "arkher.ui.compass.accessibility_policy"
	S.name = "Compass Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Compass"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.compass.interaction_ledger" }
	S.tags = { "q", "compass", "policy", "ui" }
	S.description = "Compass Accessibility Policy: rules enforcing contrast, target size and motion limits for the Compass subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 400,
		baseWeight = 0.72,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.compass.accessibility_policy", algorithm = "wfq", starvationGuard = 46 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.compass.*", function(payload) inst.lastSignal = payload end)
			end
