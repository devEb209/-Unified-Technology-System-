-- ARKHER SYSTEM E.0020 :: Surface Definition Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0020"
	S.key = "arkher.material.surfacedef.procedural_detail"
	S.name = "Surface Definition Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Surface Definition"
	S.aspect = "Procedural Detail"
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.surfacedef.texture_sampling" }
	S.tags = { "e", "surfacedef", "field", "material" }
	S.description = "Surface Definition Procedural Detail: coherent noise detail driving the surface for the Surface Definition subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 360,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.surfacedef.procedural_detail", seed = 75209, frequency = 0.0063, octaves = 4, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.surfacedef.*", function(payload) inst.lastSignal = payload end)
			end
