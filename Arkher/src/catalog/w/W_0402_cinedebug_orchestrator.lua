-- ARKHER SYSTEM W.0402 :: Cinematic Debug Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0402"
	S.key = "arkher.cine.cinedebug.playback_orchestrator"
	S.name = "Cinematic Debug Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Cinematic Debug"
	S.aspect = "Playback Orchestrator"
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.cinedebug.cut_policy" }
	S.tags = { "w", "cinedebug", "orchestrator", "cine" }
	S.description = "Cinematic Debug Playback Orchestrator: the state machine that drives a cinematic from start to end for the Cinematic Debug subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 280,
		baseWeight = 0.57,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.cinedebug.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.cinedebug.*", function(payload) inst.lastSignal = payload end)
			end
