...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Godot4 — CORE Profiler #0034", size=Vector2.new(640, 500), pos=UDim2.fromOffset(142, 138), parent=parent, icon="◐"})
...
Components.TextField(form, "Name", "Object_"..34, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
