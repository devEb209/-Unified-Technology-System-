-- ARKHER SYSTEM T.0595 :: Deployment Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0595"
	S.key = "arkher.ai.deploymentreasoning.decision_policy"
	S.name = "Deployment Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.deploymentreasoning.reasoning_graph" }
	S.tags = { "t", "deploymentreasoning", "policy", "ai" }
	S.description = "Deployment Reasoning Decision Policy: ordered rules deciding what the agent does next for the Deployment Reasoning subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 360,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.deploymentreasoning.decision_policy", algorithm = "edf", starvationGuard = 21 })
		inst.system = S
		inst.ctx = ctx
