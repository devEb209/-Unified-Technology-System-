-- ARKHER SYSTEM VD.0196 :: Architect District Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0196"
	S.key = "arkher.contsim.architectdistrict.multi-scale_fidelity"
	S.name = "Architect District Multi-Scale Fidelity"
	S.category = "VD"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.architectdistrict.sparse_residency" }
	S.tags = { "vd", "architectdistrict", "multiscale", "contsim" }
	S.description = "Architect District Multi-Scale Fidelity: distance-driven multi-scale level selection for the Architect District subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 440,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.architectdistrict.multi-scale_fidelity", levels = 5, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.architectdistrict.*", function(payload) inst.lastSignal = payload end)
			end
