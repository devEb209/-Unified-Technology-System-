-- ARKHER SYSTEM P.0129 :: Loot Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0129"
	S.key = "arkher.play.loot.difficulty_controller"
	S.name = "Loot Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Loot"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.loot.balance_analysis" }
	S.tags = { "p", "loot", "controller", "play" }
	S.description = "Loot Difficulty Controller: closed-loop tuning toward a target challenge for the Loot subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 160,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.loot.difficulty_controller", mode = "pid", target = 0.660, kp = 0.310, ki = 0.050, kd = 0.020, min = 0.05, max = 1.0, initial = 0.56, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.loot.*", function(payload) inst.lastSignal = payload end)
			end
