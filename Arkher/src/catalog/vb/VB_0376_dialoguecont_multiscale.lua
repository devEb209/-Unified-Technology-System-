-- ARKHER SYSTEM VB.0376 :: Dialogue Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0376"
	S.key = "arkher.conttime.dialoguecont.multi-scale_fidelity"
	S.name = "Dialogue Continuum Multi-Scale Fidelity"
	S.category = "VB"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.dialoguecont.sparse_residency" }
	S.tags = { "vb", "dialoguecont", "multiscale", "conttime" }
	S.description = "Dialogue Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Dialogue Continuum subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.dialoguecont.multi-scale_fidelity", levels = 4, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.dialoguecont.*", function(payload) inst.lastSignal = payload end)
			end
