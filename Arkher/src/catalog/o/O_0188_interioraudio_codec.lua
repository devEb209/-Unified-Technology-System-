-- ARKHER SYSTEM O.0188 :: Interior Audio Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0188"
	S.key = "arkher.audio.interioraudio.compression_codec"
	S.name = "Interior Audio Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Interior Audio"
	S.aspect = "Compression Codec"
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.interioraudio.asset_registry" }
	S.tags = { "o", "interioraudio", "codec", "audio" }
	S.description = "Interior Audio Compression Codec: payload encoding of audio state and events for the Interior Audio subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 440,
		baseWeight = 0.69,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.interioraudio.compression_codec", format = "json", quantBits = 11 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.interioraudio.*", function(payload) inst.lastSignal = payload end)
			end
