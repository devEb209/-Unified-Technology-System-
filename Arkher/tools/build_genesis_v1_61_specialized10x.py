#!/usr/bin/env python3
"""GENESIS V1.61 SPECIALIZED 10x — +10 únicos 24/33 (246/332) sem parar"""
...
def patch61(roots):
    items=[
        ("ARKHER_Docs","DOCS — Markdown  •  Search",0x33691E,"Markdown  •  Search  •  Version","Markdown Search Version  •  v1.60"),
        ("ARKHER_Tutorial3","TUTORIAL3 — Interactive  •  Check",0xFF6F00,"Interactive  •  Check  •  Reward","Interactive Check Reward  •  XP"),
        ("ARKHER_Sample","SAMPLE — Project  •  Fork",0x1B5E20,"Project  •  Fork  •  Star","Project Fork Star  •  1k stars"),
        ("ARKHER_Template","TEMPLATE — New  •  Category",0x2E7D32,"New  •  Category  •  Preview","New Category Preview  •  FPS"),
        ("ARKHER_Market","MARKET — Browse  •  Buy",0x01579B,"Browse  •  Buy  •  Review","Browse Buy Review  •  4.8 stars"),
        ("ARKHER_Library","LIBRARY — Owned  •  Update",0x0288D1,"Owned  •  Update  •  Install","Owned Update Install  •  50 assets"),
        ("ARKHER_Collection","COLLECTION — Tag  •  Filter",0x00838F,"Tag  •  Filter  •  Sort","Tag Filter Sort  •  Recent"),
        ("ARKHER_History","HISTORY — Undo  •  Redo",0x37474F,"Undo  •  Redo  •  Branch","Undo Redo Branch  •  100 steps"),
        ("ARKHER_Snapshot","SNAPSHOT — Save  •  Restore",0x5D4037,"Save  •  Restore  •  Diff","Save Restore Diff  •  Snapshot 5"),
        ("ARKHER_Backup","BACKUP — Auto  •  Cloud",0x311B92,"Auto  •  Cloud  •  Restore","Auto Cloud Restore  •  Daily"),
    ]
...
for _,n in ipairs({"ARKHER_Docs","ARKHER_Tutorial3","ARKHER_Sample","ARKHER_Template","ARKHER_Market","ARKHER_Library","ARKHER_Collection","ARKHER_History","ARKHER_Snapshot","ARKHER_Backup"}) do
...
print("[V1.61 10x] Docs/Tutorial3/Sample/Template/Market/Library/Collection/History/Snapshot/Backup 246/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_61",{"Source":(T_STRING, FUNC61)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch60(roots)
    roots=patch61(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_61.rbxl")
    write(out, serialize(roots))
    print(f"V1.61 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_61.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.61 done")
