-- ARKHER SYSTEM W.0246 :: Dissolve Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0246"
	S.key = "arkher.cine.dissolve.playback_orchestrator"
	S.name = "Dissolve Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Dissolve"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.dissolve.cut_policy" }
	S.tags = { "w", "dissolve", "orchestrator", "cine" }
	S.description = "Dissolve Playback Orchestrator: the state machine that drives a cinematic from start to end for the Dissolve subsystem."
	S.params = {
		backlogLimit = 29,
		baseRadius = 360,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.dissolve.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.dissolve.*", function(payload) inst.lastSignal = payload end)
			end
