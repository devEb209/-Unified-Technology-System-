-- ARKHER SYSTEM K.0615 :: Faction Membership Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0615"
	S.key = "arkher.npc.factionmembership.thinking_budget"
	S.name = "Faction Membership Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Faction Membership"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.factionmembership.daily_schedule" }
	S.tags = { "k", "factionmembership", "budgeter", "npc" }
	S.description = "Faction Membership Thinking Budget: allocation of per-frame thinking time across NPCs for the Faction Membership subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 520,
		baseWeight = 0.75,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.factionmembership.thinking_budget", total = 385, strategy = "proportional" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.factionmembership.*", function(payload) inst.lastSignal = payload end)
			end
