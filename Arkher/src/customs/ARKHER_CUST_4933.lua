--!strict
-- Substance Sampler CUSTOM 4933 — INFINITY+
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Substance Sampler CUSTOM 4933 — INFINITY+", size=Vector2.new(649, 541), pos=UDim2.fromOffset(91, 89), parent=parent, icon="⬣"})
    win.Root.Name="ARKHER_CUST_4933"
...
local hdr=Instance.new("TextLabel"); hdr.Size=UDim2.new(1,0,0,22); hdr.BackgroundTransparency=1; hdr.Text="Substance Sampler CUSTOM 4933 — INFINITY+"; hdr.Font=Enum.Font.GothamBold; hdr.TextSize=11; hdr.TextColor3=Theme.tokens.arkherGlow; hdr.TextXAlignment=Enum.TextXAlignment.Left; hdr.Parent=content
...
Components.Dropdown(content, "Mode", {"A","B","C"}, "A", function() end)
    ...
    local img=Instance.new("ImageLabel"); img.Name="Thumb_4933"; img.Size=UDim2.fromOffset(80,80); img.Position=UDim2.fromOffset(12,12); img.BackgroundColor3=Theme.tokens.slate800; img.Image="rbxassetid://0"; img.ScaleType=Enum.ScaleType.Crop; img.BorderSizePixel=0; img.Parent=body; local ic=Instance.new("UICorner"); ic.CornerRadius=UDim.new(0,8); ic.Parent=img
    local idLbl=Instance.new("TextLabel"); idLbl.Size=UDim2.new(1,0,0,12); idLbl.Position=UDim2.new(0,0,1,-12); idLbl.BackgroundTransparency=1; idLbl.Text="ID "..(2000000+4933*137); idLbl.Font=Enum.Font.Code; idLbl.TextSize=8; idLbl.TextColor3=Theme.tokens.slate400; idLbl.Parent=img
