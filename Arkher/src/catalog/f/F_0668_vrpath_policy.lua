-- ARKHER SYSTEM F.0668 :: VR Stereo Path Render Policy
...
return function(A)
	...
	local S = {}
	S.id = "F.0668"
	S.key = "arkher.render.vrpath.render_policy"
	S.name = "VR Stereo Path Render Policy"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "VR Stereo Path"
	S.aspect = "Render Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.render.vrpath.draw_budget" }
	S.tags = { "f", "vrpath", "policy", "render" }
	S.description = "VR Stereo Path Render Policy: device tier rules for the frame for the VR Stereo Path subsystem."
	S.params = {
		backlogLimit = 41,
		baseRadius = 520,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.render.vrpath.render_policy", algorithm = "edf", starvationGuard = 25 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.vrpath.*", function(payload) inst.lastSignal = payload end)
			end
