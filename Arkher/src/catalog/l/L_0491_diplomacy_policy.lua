-- ARKHER SYSTEM L.0491 :: Diplomacy Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0491"
	S.key = "arkher.sim.diplomacy.fidelity_policy"
	S.name = "Diplomacy Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.diplomacy.streaming" }
	S.tags = { "l", "diplomacy", "policy", "sim" }
	S.description = "Diplomacy Fidelity Policy: rules that pick full, cohort or statistical simulation for the Diplomacy subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.94,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.diplomacy.fidelity_policy", algorithm = "roundrobin", starvationGuard = 48 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.diplomacy.*", function(payload) inst.lastSignal = payload end)
			end
