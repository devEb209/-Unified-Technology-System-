-- ARKHER SYSTEM T.0629 :: Explainability Learning Analysis
...
return function(A)
	...
	local S = {}
	S.id = "T.0629"
	S.key = "arkher.ai.explainability.learning_analysis"
	S.name = "Explainability Learning Analysis"
	S.category = "T"
	S.family = "SINGULARITY AI"
	S.area = "Explainability"
	S.aspect = "Learning Analysis"
	S.kit = "analyzer"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.explainability.model_inference" }
	S.tags = { "t", "explainability", "analyzer", "ai" }
	S.description = "Explainability Learning Analysis: trend, anomaly and improvement analysis of the agent for the Explainability subsystem."
	S.params = {
		backlogLimit = 34,
		baseRadius = 400,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("analyzer", { id = "arkher.ai.explainability.learning_analysis", window = 194, buckets = 14, threshold = 2.60 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.summary()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ai.explainability.*", function(payload) inst.lastSignal = payload end)
			end
