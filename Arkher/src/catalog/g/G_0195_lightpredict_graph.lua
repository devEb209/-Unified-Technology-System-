-- ARKHER SYSTEM G.0195 :: Light Transport Prediction Sampling Graph
...
-- Kit: graph (dependency graph of sampling stages)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "G.0195"
	S.key = "arkher.neural.lightpredict.sampling_graph"
	S.name = "Light Transport Prediction Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Light Transport Prediction"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.lightpredict.training_ledger" }
	S.tags = { "g", "lightpredict", "graph", "neural" }
	S.description = "Light Transport Prediction Sampling Graph: dependency graph of sampling stages for the Light Transport Prediction subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 520,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.lightpredict.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.lightpredict.*", function(payload) inst.lastSignal = payload end)
			end
