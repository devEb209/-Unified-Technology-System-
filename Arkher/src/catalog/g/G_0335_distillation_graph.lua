-- ARKHER SYSTEM G.0335 :: Model Distillation Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0335"
	S.key = "arkher.neural.distillation.sampling_graph"
	S.name = "Model Distillation Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Model Distillation"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.distillation.training_ledger" }
	S.tags = { "g", "distillation", "graph", "neural" }
	S.description = "Model Distillation Sampling Graph: dependency graph of sampling stages for the Model Distillation subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 360,
		baseWeight = 0.63,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.distillation.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.distillation.*", function(payload) inst.lastSignal = payload end)
			end
