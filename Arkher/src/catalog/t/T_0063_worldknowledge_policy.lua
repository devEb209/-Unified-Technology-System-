-- ARKHER SYSTEM T.0063 :: World Knowledge Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0063"
	S.key = "arkher.ai.worldknowledge.decision_policy"
	S.name = "World Knowledge Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.worldknowledge.reasoning_graph" }
	S.tags = { "t", "worldknowledge", "policy", "ai" }
	S.description = "World Knowledge Decision Policy: ordered rules deciding what the agent does next for the World Knowledge subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 160,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.worldknowledge.decision_policy", algorithm = "roundrobin", starvationGuard = 16 })
		inst.system = S
		inst.ctx = ctx
