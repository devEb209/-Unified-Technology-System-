-- ARKHER SYSTEM Q.0381 :: Localization Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0381"
	S.key = "arkher.ui.localization.accessibility_policy"
	S.name = "Localization Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Localization"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.localization.interaction_ledger" }
	S.tags = { "q", "localization", "policy", "ui" }
	S.description = "Localization Accessibility Policy: rules enforcing contrast, target size and motion limits for the Localization subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.localization.accessibility_policy", algorithm = "roundrobin", starvationGuard = 32 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.localization.*", function(payload) inst.lastSignal = payload end)
			end
