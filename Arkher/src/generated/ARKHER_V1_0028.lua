...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Decima Workshop — CORE Map #0028", size=Vector2.new(640, 480), pos=UDim2.fromOffset(464, 296), parent=parent, icon="✥"})
...
Components.Slider(props, "Value", -5,5,0, function(v) local s=game:GetService("Selection"):Get()[1]; if s and s:IsA("BasePart") then s:SetAttribute("ARKHER_T"..28, v) end end)
