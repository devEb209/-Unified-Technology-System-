-- ARKHER SYSTEM K.0405 :: Plan Execution Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0405"
	S.key = "arkher.npc.planexecution.thinking_budget"
	S.name = "Plan Execution Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Plan Execution"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.planexecution.daily_schedule" }
	S.tags = { "k", "planexecution", "budgeter", "npc" }
	S.description = "Plan Execution Thinking Budget: allocation of per-frame thinking time across NPCs for the Plan Execution subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 440,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.planexecution.thinking_budget", total = 87, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.planexecution.*", function(payload) inst.lastSignal = payload end)
			end
