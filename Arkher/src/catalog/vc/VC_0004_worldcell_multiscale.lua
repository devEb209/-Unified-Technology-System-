-- ARKHER SYSTEM VC.0004 :: World Grid Cell Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0004"
	S.key = "arkher.contneural.worldcell.multi-scale_fidelity"
	S.name = "World Grid Cell Multi-Scale Fidelity"
	S.category = "VC"
	S.family = "CONTINUUM — NEURAL"
	S.area = "World Grid Cell"
	S.aspect = "Multi-Scale Fidelity"
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.worldcell.sparse_residency" }
	S.tags = { "vc", "worldcell", "multiscale", "contneural" }
	S.description = "World Grid Cell Multi-Scale Fidelity: distance-driven multi-scale level selection for the World Grid Cell subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.74,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.worldcell.multi-scale_fidelity", levels = 6, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.worldcell.*", function(payload) inst.lastSignal = payload end)
			end
