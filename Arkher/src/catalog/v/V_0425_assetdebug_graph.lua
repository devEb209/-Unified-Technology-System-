-- ARKHER SYSTEM V.0425 :: Asset Debug Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0425"
	S.key = "arkher.asset.assetdebug.dependency_graph"
	S.name = "Asset Debug Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.assetdebug.asset_registry" }
	S.tags = { "v", "assetdebug", "graph", "asset" }
	S.description = "Asset Debug Dependency Graph: what references what, and in which order it must be built for the Asset Debug subsystem."
	S.params = {
		backlogLimit = 40,
		baseRadius = 320,
		baseWeight = 0.52,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.assetdebug.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.assetdebug.*", function(payload) inst.lastSignal = payload end)
			end
