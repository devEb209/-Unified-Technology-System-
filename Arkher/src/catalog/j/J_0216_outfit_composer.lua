-- ARKHER SYSTEM J.0216 :: Outfit Composition Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0216"
	S.key = "arkher.character.outfit.appearance_composer"
	S.name = "Outfit Composition Appearance Composer"
	S.category = "J"
	S.family = "CHARACTERS / DIGITAL HUMANS"
	S.area = "Outfit Composition"
	S.aspect = "Appearance Composer"
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.outfit.locomotion" }
	S.tags = { "j", "outfit", "composer", "character" }
	S.description = "Outfit Composition Appearance Composer: composition of appearance slots into a look for the Outfit Composition subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 200,
		baseWeight = 0.73,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.outfit.appearance_composer", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.outfit.*", function(payload) inst.lastSignal = payload end)
			end
