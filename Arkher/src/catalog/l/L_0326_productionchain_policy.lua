-- ARKHER SYSTEM L.0326 :: Production Chain Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0326"
	S.key = "arkher.sim.productionchain.fidelity_policy"
	S.name = "Production Chain Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.productionchain.streaming" }
	S.tags = { "l", "productionchain", "policy", "sim" }
	S.description = "Production Chain Fidelity Policy: rules that pick full, cohort or statistical simulation for the Production Chain subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 160,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.productionchain.fidelity_policy", algorithm = "roundrobin", starvationGuard = 16 })
		inst.system = S
		inst.ctx = ctx
