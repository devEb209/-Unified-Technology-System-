-- ARKHER SYSTEM T.0497 :: Physics Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0497"
	S.key = "arkher.ai.physicsreasoning.decision_policy"
	S.name = "Physics Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.physicsreasoning.reasoning_graph" }
	S.tags = { "t", "physicsreasoning", "policy", "ai" }
	S.description = "Physics Reasoning Decision Policy: ordered rules deciding what the agent does next for the Physics Reasoning subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.physicsreasoning.decision_policy", algorithm = "roundrobin", starvationGuard = 56 })
		inst.system = S
		inst.ctx = ctx
