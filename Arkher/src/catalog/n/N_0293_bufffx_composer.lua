-- ARKHER SYSTEM N.0293 :: Buff Effect Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0293"
	S.key = "arkher.vfx.bufffx.effect_composition"
	S.name = "Buff Effect Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Buff Effect"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.bufffx.effect_graph" }
	S.tags = { "n", "bufffx", "composer", "vfx" }
	S.description = "Buff Effect Effect Composition: blending of several effect signals into one output for the Buff Effect subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 600,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.bufffx.effect_composition", mode = "overlay" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.bufffx.*", function(payload) inst.lastSignal = payload end)
			end
