-- ARKHER SYSTEM P.0165 :: Gathering Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0165"
	S.key = "arkher.play.gathering.difficulty_controller"
	S.name = "Gathering Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Gathering"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.gathering.balance_analysis" }
	S.tags = { "p", "gathering", "controller", "play" }
	S.description = "Gathering Difficulty Controller: closed-loop tuning toward a target challenge for the Gathering subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 440,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.gathering.difficulty_controller", mode = "bangbang", target = 0.690, kp = 0.440, ki = 0.090, kd = 0.050, min = 0.05, max = 1.0, initial = 0.59, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.gathering.*", function(payload) inst.lastSignal = payload end)
			end
