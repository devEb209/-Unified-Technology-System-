-- ARKHER SYSTEM F.0480 :: Impostor Rendering Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0480"
	S.key = "arkher.render.impostorrender.geometry_virtualization"
	S.name = "Impostor Rendering Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Impostor Rendering"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.impostorrender.visibility_set" }
	S.tags = { "f", "impostorrender", "impostor", "render" }
	S.description = "Impostor Rendering Geometry Virtualization: impostor and HLOD synthesis under a budget for the Impostor Rendering subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 320,
		baseWeight = 0.72,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.impostorrender.geometry_virtualization", atlasSlots = 96, viewCount = 8, errorThreshold = 3.20 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.impostorrender.*", function(payload) inst.lastSignal = payload end)
			end
