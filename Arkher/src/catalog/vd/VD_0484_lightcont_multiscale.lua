-- ARKHER SYSTEM VD.0484 :: Lighting Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0484"
	S.key = "arkher.contsim.lightcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.lightcont.sparse_residency" }
	S.tags = { "vd", "lightcont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.lightcont.multi-scale_fidelity", levels = 4, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.lightcont.*", function(payload) inst.lastSignal = payload end)
			end
