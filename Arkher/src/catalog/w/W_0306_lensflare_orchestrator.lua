-- ARKHER SYSTEM W.0306 :: Lens Flare Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0306"
	S.key = "arkher.cine.lensflare.playback_orchestrator"
	S.name = "Lens Flare Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Lens Flare"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.lensflare.cut_policy" }
	S.tags = { "w", "lensflare", "orchestrator", "cine" }
	S.description = "Lens Flare Playback Orchestrator: the state machine that drives a cinematic from start to end for the Lens Flare subsystem."
	S.params = {
		backlogLimit = 23,
		baseRadius = 600,
		baseWeight = 0.55,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.lensflare.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.lensflare.*", function(payload) inst.lastSignal = payload end)
			end
