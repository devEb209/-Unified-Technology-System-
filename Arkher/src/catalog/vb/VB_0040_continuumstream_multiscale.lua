-- ARKHER SYSTEM VB.0040 :: Continuum Stream Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0040"
	S.key = "arkher.conttime.continuumstream.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.continuumstream.sparse_residency" }
	S.tags = { "vb", "continuumstream", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.continuumstream.multi-scale_fidelity", levels = 6, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.continuumstream.*", function(payload) inst.lastSignal = payload end)
			end
