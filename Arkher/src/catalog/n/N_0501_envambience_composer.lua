-- ARKHER SYSTEM N.0501 :: Environment Ambience Effect Composition
-- Category N - VFX
-- ARKHER VFX Framework capability: emission, simulation, forces and trails inside one shared budget.
-- Kit: composer (blending of several effect signals into one output)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "N.0501"
	S.key = "arkher.vfx.envambience.effect_composition"
	S.name = "Environment Ambience Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Environment Ambience"
	S.aspect = "Effect Composition"
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.envambience.effect_graph" }
	S.tags = { "n", "envambience", "composer", "vfx" }
	S.description = "Environment Ambience Effect Composition: blending of several effect signals into one output for the Environment Ambience subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 200,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.envambience.effect_composition", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.envambience.*", function(payload) inst.lastSignal = payload end)
			end
