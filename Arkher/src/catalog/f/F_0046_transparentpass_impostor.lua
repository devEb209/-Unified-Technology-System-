-- ARKHER SYSTEM F.0046 :: Transparent Pass Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0046"
	S.key = "arkher.render.transparentpass.geometry_virtualization"
	S.name = "Transparent Pass Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Transparent Pass"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.transparentpass.visibility_set" }
	S.tags = { "f", "transparentpass", "impostor", "render" }
	S.description = "Transparent Pass Geometry Virtualization: impostor and HLOD synthesis under a budget for the Transparent Pass subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 560,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.transparentpass.geometry_virtualization", atlasSlots = 96, viewCount = 8, errorThreshold = 3.80 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.transparentpass.*", function(payload) inst.lastSignal = payload end)
			end
