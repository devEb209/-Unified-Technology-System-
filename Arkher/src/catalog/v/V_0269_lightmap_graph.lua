-- ARKHER SYSTEM V.0269 :: Lightmap Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0269"
	S.key = "arkher.asset.lightmap.dependency_graph"
	S.name = "Lightmap Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.lightmap.asset_registry" }
	S.tags = { "v", "lightmap", "graph", "asset" }
	S.description = "Lightmap Dependency Graph: what references what, and in which order it must be built for the Lightmap subsystem."
	S.params = {
		backlogLimit = 41,
		baseRadius = 520,
		baseWeight = 0.63,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.lightmap.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.lightmap.*", function(payload) inst.lastSignal = payload end)
			end
