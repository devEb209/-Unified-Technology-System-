-- ARKHER SYSTEM F.0312 :: Depth Of Field Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0312"
	S.key = "arkher.render.dof.geometry_virtualization"
	S.name = "Depth Of Field Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	...
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.dof.visibility_set" }
	S.tags = { "f", "dof", "impostor", "render" }
	S.description = "Depth Of Field Geometry Virtualization: impostor and HLOD synthesis under a budget for the Depth Of Field subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.94,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.dof.geometry_virtualization", atlasSlots = 64, viewCount = 8, errorThreshold = 2.40 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.dof.*", function(payload) inst.lastSignal = payload end)
			end
