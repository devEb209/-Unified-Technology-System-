-- ARKHER SYSTEM R.0351 :: Network Debug Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0351"
	S.key = "arkher.net.networkdebug.prediction_and_reconciliation"
	S.name = "Network Debug Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Network Debug"
	S.aspect = "Prediction And Reconciliation"
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.networkdebug.time_sync" }
	S.tags = { "r", "networkdebug", "prediction", "net" }
	S.description = "Network Debug Prediction And Reconciliation: client simulation, correction and input replay for the Network Debug subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 480,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.networkdebug.prediction_and_reconciliation", errorThreshold = 0.030, smoothing = 8.0, maxInputs = 100 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.networkdebug.*", function(payload) inst.lastSignal = payload end)
			end
