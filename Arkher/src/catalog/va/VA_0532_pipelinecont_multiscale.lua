-- ARKHER SYSTEM VA.0532 :: Pipeline Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0532"
	S.key = "arkher.contworld.pipelinecont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.pipelinecont.sparse_residency" }
	S.tags = { "va", "pipelinecont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.pipelinecont.multi-scale_fidelity", levels = 6, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.pipelinecont.*", function(payload) inst.lastSignal = payload end)
			end
