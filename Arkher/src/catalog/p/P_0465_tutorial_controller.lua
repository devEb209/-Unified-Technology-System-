-- ARKHER SYSTEM P.0465 :: Tutorial Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0465"
	S.key = "arkher.play.tutorial.difficulty_controller"
	S.name = "Tutorial Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Tutorial"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.tutorial.balance_analysis" }
	S.tags = { "p", "tutorial", "controller", "play" }
	S.description = "Tutorial Difficulty Controller: closed-loop tuning toward a target challenge for the Tutorial subsystem."
	S.params = {
		backlogLimit = 29,
		baseRadius = 360,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.tutorial.difficulty_controller", mode = "hysteresis", target = 0.710, kp = 0.360, ki = 0.070, kd = 0.020, min = 0.05, max = 1.0, initial = 0.61, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.tutorial.*", function(payload) inst.lastSignal = payload end)
			end
