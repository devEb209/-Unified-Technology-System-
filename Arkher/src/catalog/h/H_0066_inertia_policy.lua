-- ARKHER SYSTEM H.0066 :: Inertia Tensor Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0066"
	S.key = "arkher.physics.inertia.simulation_policy"
	S.name = "Inertia Tensor Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Inertia Tensor"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.inertia.budget_governor" }
	S.tags = { "h", "inertia", "policy", "physics" }
	S.description = "Inertia Tensor Simulation Policy: device tier rules for substeps, iterations and sleep for the Inertia Tensor subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 200,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.inertia.simulation_policy", algorithm = "edf", starvationGuard = 29 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.inertia.*", function(payload) inst.lastSignal = payload end)
			end
