-- ARKHER SYSTEM E.0590 :: Material Migration Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0590"
	S.key = "arkher.material.migration.procedural_detail"
	S.name = "Material Migration Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Material Migration"
	S.aspect = "Procedural Detail"
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.migration.texture_sampling" }
	S.tags = { "e", "migration", "field", "material" }
	S.description = "Material Migration Procedural Detail: coherent noise detail driving the surface for the Material Migration subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 200,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.migration.procedural_detail", seed = 7465, frequency = 0.0103, octaves = 4, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.migration.*", function(payload) inst.lastSignal = payload end)
			end
