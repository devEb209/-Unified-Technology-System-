-- ARKHER SYSTEM G.0360 :: Batch Inference Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0360"
	S.key = "arkher.neural.batchinference.heuristic_policy"
	S.name = "Batch Inference Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.batchinference.result_cache" }
	S.tags = { "g", "batchinference", "policy", "neural" }
	S.description = "Batch Inference Heuristic Policy: rule fallback when the model is unavailable for the Batch Inference subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 560,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.batchinference.heuristic_policy", algorithm = "wfq", starvationGuard = 50 })
		inst.system = S
		inst.ctx = ctx
