-- ARKHER SYSTEM Q.0321 :: Gamepad Focus Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0321"
	S.key = "arkher.ui.gamepadfocus.accessibility_policy"
	S.name = "Gamepad Focus Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Gamepad Focus"
	S.aspect = "Accessibility Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.gamepadfocus.interaction_ledger" }
	S.tags = { "q", "gamepadfocus", "policy", "ui" }
	S.description = "Gamepad Focus Accessibility Policy: rules enforcing contrast, target size and motion limits for the Gamepad Focus subsystem."
	S.params = {
		backlogLimit = 23,
		baseRadius = 280,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.gamepadfocus.accessibility_policy", algorithm = "lottery", starvationGuard = 55 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.gamepadfocus.*", function(payload) inst.lastSignal = payload end)
			end
