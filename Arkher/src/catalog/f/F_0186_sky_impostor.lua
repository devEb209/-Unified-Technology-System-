-- ARKHER SYSTEM F.0186 :: Sky Model Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0186"
	S.key = "arkher.render.sky.geometry_virtualization"
	S.name = "Sky Model Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Sky Model"
	...
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.sky.visibility_set" }
	S.tags = { "f", "sky", "impostor", "render" }
	S.description = "Sky Model Geometry Virtualization: impostor and HLOD synthesis under a budget for the Sky Model subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 240,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.sky.geometry_virtualization", atlasSlots = 64, viewCount = 8, errorThreshold = 1.80 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.sky.*", function(payload) inst.lastSignal = payload end)
			end
