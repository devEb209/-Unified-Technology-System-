-- ARKHER SYSTEM Z.0416 :: Battery Aware Pipeline Governance Policy
...
return function(A)
	...
	local S = {}
	S.id = "Z.0416"
	S.key = "arkher.origin.batteryaware.governance_policy"
	S.name = "Battery Aware Pipeline Governance Policy"
	S.category = "Z"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.origin.batteryaware.signal_composition" }
	S.tags = { "z", "batteryaware", "policy", "origin" }
	S.description = "Battery Aware Pipeline Governance Policy: ordered rules governing when the technology may act for the Battery Aware Pipeline subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 320,
		baseWeight = 0.94,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.origin.batteryaware.governance_policy", algorithm = "roundrobin", starvationGuard = 20 })
		inst.system = S
		inst.ctx = ctx
