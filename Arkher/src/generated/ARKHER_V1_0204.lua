--!strict
-- Substance Layers — CORE Profiler #0204
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Substance Layers — CORE Profiler #0204", size=Vector2.new(600, 500), pos=UDim2.fromOffset(352, 168), parent=parent, icon="♪"})
...
Components.TextField(right, "File", "input.png", function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_File"..204, v) end end)
