-- ARKHER SYSTEM VC.0232 :: City Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0232"
	S.key = "arkher.contneural.citycont.multi-scale_fidelity"
	S.name = "City Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.citycont.sparse_residency" }
	S.tags = { "vc", "citycont", "multiscale", "contneural" }
	S.description = "City Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the City Continuum subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 320,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.citycont.multi-scale_fidelity", levels = 5, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.citycont.*", function(payload) inst.lastSignal = payload end)
			end
