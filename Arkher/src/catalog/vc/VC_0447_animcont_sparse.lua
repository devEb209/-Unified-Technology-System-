-- ARKHER SYSTEM VC.0447 :: Animation Continuum Sparse Residency
...
return function(A)
	...
	local S = {}
	S.id = "VC.0447"
	S.key = "arkher.contneural.animcont.sparse_residency"
	S.name = "Animation Continuum Sparse Residency"
	S.category = "VC"
	...
	S.kit = "sparse"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.animcont.epoch_clock" }
	S.tags = { "vc", "animcont", "sparse", "contneural" }
	S.description = "Animation Continuum Sparse Residency: sparse page residency with eviction and compression for the Animation Continuum subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 480,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("sparse", { id = "arkher.contneural.animcont.sparse_residency", capacity = 640, pageBytes = 6144 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.animcont.*", function(payload) inst.lastSignal = payload end)
			end
