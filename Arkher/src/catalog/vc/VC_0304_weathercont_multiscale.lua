-- ARKHER SYSTEM VC.0304 :: Weather Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0304"
	S.key = "arkher.contneural.weathercont.multi-scale_fidelity"
	S.name = "Weather Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.weathercont.sparse_residency" }
	S.tags = { "vc", "weathercont", "multiscale", "contneural" }
	S.description = "Weather Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Weather Continuum subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 200,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.weathercont.multi-scale_fidelity", levels = 5, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.weathercont.*", function(payload) inst.lastSignal = payload end)
			end
