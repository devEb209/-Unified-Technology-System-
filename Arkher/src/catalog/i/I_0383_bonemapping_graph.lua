-- ARKHER SYSTEM I.0383 :: Bone Mapping Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0383"
	S.key = "arkher.anim.bonemapping.retarget_graph"
	S.name = "Bone Mapping Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Bone Mapping"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.bonemapping.physical_blend" }
	S.tags = { "i", "bonemapping", "graph", "anim" }
	S.description = "Bone Mapping Retarget Graph: bone mapping and retarget path resolution for the Bone Mapping subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 440,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.bonemapping.retarget_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.bonemapping.*", function(payload) inst.lastSignal = payload end)
			end
