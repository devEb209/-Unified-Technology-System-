-- ARKHER SYSTEM T.0050 :: Goal Reasoning Outcome Prediction
...
return function(A)
	...
	local S = {}
	S.id = "T.0050"
	S.key = "arkher.ai.goalreasoning.outcome_prediction"
	S.name = "Goal Reasoning Outcome Prediction"
	S.category = "T"
	S.family = "SINGULARITY AI"
	S.area = "Goal Reasoning"
	S.aspect = "Outcome Prediction"
	S.kit = "predictor"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.goalreasoning.decision_policy" }
	S.tags = { "t", "goalreasoning", "predictor", "ai" }
	S.description = "Goal Reasoning Outcome Prediction: forecasting the result before the work is done for the Goal Reasoning subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 440,
		baseWeight = 0.67,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("predictor", { id = "arkher.ai.goalreasoning.outcome_prediction", method = "linear", alpha = 0.37, window = 99 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.trustworthy() return inst.confidence() >= S.params.minConfidence end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ai.goalreasoning.*", function(payload) inst.lastSignal = payload end)
			end
