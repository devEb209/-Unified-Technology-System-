-- ARKHER SYSTEM O.0020 :: Foley Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0020"
	S.key = "arkher.audio.foley.compression_codec"
	S.name = "Foley Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Foley"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.foley.asset_registry" }
	S.tags = { "o", "foley", "codec", "audio" }
	S.description = "Foley Compression Codec: payload encoding of audio state and events for the Foley subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 320,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.foley.compression_codec", format = "binary", quantBits = 12 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.foley.*", function(payload) inst.lastSignal = payload end)
			end
