-- ARKHER SYSTEM T.0623 :: Explainability Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0623"
	S.key = "arkher.ai.explainability.decision_policy"
	S.name = "Explainability Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.explainability.reasoning_graph" }
	S.tags = { "t", "explainability", "policy", "ai" }
	S.description = "Explainability Decision Policy: ordered rules deciding what the agent does next for the Explainability subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 160,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.explainability.decision_policy", algorithm = "roundrobin", starvationGuard = 28 })
		inst.system = S
		inst.ctx = ctx
