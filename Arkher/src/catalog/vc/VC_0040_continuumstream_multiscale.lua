-- ARKHER SYSTEM VC.0040 :: Continuum Stream Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0040"
	S.key = "arkher.contneural.continuumstream.multi-scale_fidelity"
	S.name = "Continuum Stream Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.continuumstream.sparse_residency" }
	S.tags = { "vc", "continuumstream", "multiscale", "contneural" }
	S.description = "Continuum Stream Multi-Scale Fidelity: distance-driven multi-scale level selection for the Continuum Stream subsystem."
	S.params = {
		backlogLimit = 29,
		baseRadius = 360,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.continuumstream.multi-scale_fidelity", levels = 6, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.continuumstream.*", function(payload) inst.lastSignal = payload end)
			end
