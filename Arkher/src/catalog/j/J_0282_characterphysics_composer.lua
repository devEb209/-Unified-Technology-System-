-- ARKHER SYSTEM J.0282 :: Character Physics Appearance Composer
...
-- Kit: composer (composition of appearance slots into a look)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "J.0282"
	S.key = "arkher.character.characterphysics.appearance_composer"
	S.name = "Character Physics Appearance Composer"
	S.category = "J"
	S.family = "CHARACTERS / DIGITAL HUMANS"
	S.area = "Character Physics"
	S.aspect = "Appearance Composer"
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.characterphysics.locomotion" }
	S.tags = { "j", "characterphysics", "composer", "character" }
	S.description = "Character Physics Appearance Composer: composition of appearance slots into a look for the Character Physics subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 480,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.characterphysics.appearance_composer", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.characterphysics.*", function(payload) inst.lastSignal = payload end)
			end
