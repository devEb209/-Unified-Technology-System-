-- ARKHER SYSTEM VA.0352 :: Narrative Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0352"
	S.key = "arkher.contworld.narrativecont.multi-scale_fidelity"
	S.name = "Narrative Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.narrativecont.sparse_residency" }
	S.tags = { "va", "narrativecont", "multiscale", "contworld" }
	S.description = "Narrative Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Narrative Continuum subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.narrativecont.multi-scale_fidelity", levels = 4, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.narrativecont.*", function(payload) inst.lastSignal = payload end)
			end
