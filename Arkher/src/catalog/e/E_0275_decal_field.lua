-- ARKHER SYSTEM E.0275 :: Decal Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0275"
	S.key = "arkher.material.decal.procedural_detail"
	S.name = "Decal Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Decal"
	...
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.decal.texture_sampling" }
	S.tags = { "e", "decal", "field", "material" }
	S.description = "Decal Procedural Detail: coherent noise detail driving the surface for the Decal subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 320,
		baseWeight = 0.74,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.decal.procedural_detail", seed = 34924, frequency = 0.0050, octaves = 3, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.decal.*", function(payload) inst.lastSignal = payload end)
			end
