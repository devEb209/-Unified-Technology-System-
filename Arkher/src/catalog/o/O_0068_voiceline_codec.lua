-- ARKHER SYSTEM O.0068 :: Voice Line Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0068"
	S.key = "arkher.audio.voiceline.compression_codec"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.voiceline.asset_registry" }
	S.tags = { "o", "voiceline", "codec", "audio" }
	S.description = "Voice Line Compression Codec: payload encoding of audio state and events for the Voice Line subsystem."
	S.params = {
		backlogLimit = 9,
		baseRadius = 200,
		baseWeight = 0.71,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.voiceline.compression_codec", format = "quantized", quantBits = 9 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.voiceline.*", function(payload) inst.lastSignal = payload end)
			end
