-- ARKHER SYSTEM O.0272 :: Stinger Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0272"
	S.key = "arkher.audio.stinger.compression_codec"
	S.name = "Stinger Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Stinger"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.stinger.asset_registry" }
	S.tags = { "o", "stinger", "codec", "audio" }
	S.description = "Stinger Compression Codec: payload encoding of audio state and events for the Stinger subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 480,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.stinger.compression_codec", format = "binary", quantBits = 8 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.stinger.*", function(payload) inst.lastSignal = payload end)
			end
