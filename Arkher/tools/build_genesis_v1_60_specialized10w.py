#!/usr/bin/env python3
"""GENESIS V1.60 SPECIALIZED 10w — +10 únicos 23/33 (236/332) sem parar"""
...
def patch60(roots):
    items=[
        ("ARKHER_Package","PACKAGE — Zip  •  Sign",0x33691E,"Zip  •  Sign  •  Verify","Zip Sign Verify  •  500MB"),
        ("ARKHER_Sign","SIGN — Certificate  •  Hash",0x1B5E20,"Certificate  •  Hash  •  Expire","Certificate Hash Expire  •  SHA256"),
        ("ARKHER_CDN","CDN — Edge  •  Cache",0x0277BD,"Edge  •  Cache  •  Purge","Edge Cache Purge  •  CloudFront"),
        ("ARKHER_Staged","STAGED — Ring  •  Percent",0x01579B,"Ring  •  Percent  •  Rollback","Ring Percent Rollback  •  10% 50% 100%"),
        ("ARKHER_Monitor","MONITOR — Metric  •  Alert",0xAD1457,"Metric  •  Alert  •  Dashboard","Metric Alert Dashboard  •  PagerDuty"),
        ("ARKHER_Alert","ALERT — Rule  •  Notify",0xB71C1C,"Rule  •  Notify  •  Silence","Rule Notify Silence  •  Critical"),
        ("ARKHER_Incident","INCIDENT — Timeline  •  Postmortem",0x37474F,"Timeline  •  Postmortem  •  Action","Timeline Postmortem Action  •  2h"),
        ("ARKHER_OnCall","ONCALL — Schedule  •  Escalate",0x4A148C,"Schedule  •  Escalate  •  Handoff","Schedule Escalate Handoff  •  Weekly"),
        ("ARKHER_Runbook","RUNBOOK — Step  •  Command",0x5D4037,"Step  •  Command  •  Verify","Step Command Verify  •  10 steps"),
        ("ARKHER_SRE","SRE — SLO  •  ErrorBudget",0x00838F,"SLO  •  ErrorBudget  •  Burn","SLO ErrorBudget Burn  •  99.9%"),
    ]
...
for _,n in ipairs({"ARKHER_Package","ARKHER_Sign","ARKHER_CDN","ARKHER_Staged","ARKHER_Monitor","ARKHER_Alert","ARKHER_Incident","ARKHER_OnCall","ARKHER_Runbook","ARKHER_SRE"}) do
...
print("[V1.60 10x] Package/Sign/CDN/Staged/Monitor/Alert/Incident/OnCall/Runbook/SRE 236/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_60",{"Source":(T_STRING, FUNC60)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch59(roots)
    roots=patch60(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_60.rbxl")
    write(out, serialize(roots))
    print(f"V1.60 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_60.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.60 done")
