-- ARKHER SYSTEM W.0006 :: Shot Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0006"
	S.key = "arkher.cine.shot.playback_orchestrator"
	S.name = "Shot Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Shot"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.shot.cut_policy" }
	S.tags = { "w", "shot", "orchestrator", "cine" }
	S.description = "Shot Playback Orchestrator: the state machine that drives a cinematic from start to end for the Shot subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 360,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.shot.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.shot.*", function(payload) inst.lastSignal = payload end)
			end
