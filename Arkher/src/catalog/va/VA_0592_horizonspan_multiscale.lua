-- ARKHER SYSTEM VA.0592 :: Horizon Span Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0592"
	S.key = "arkher.contworld.horizonspan.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.horizonspan.sparse_residency" }
	S.tags = { "va", "horizonspan", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.horizonspan.multi-scale_fidelity", levels = 6, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.horizonspan.*", function(payload) inst.lastSignal = payload end)
			end
