#!/usr/bin/env python3
"""GENESIS V1.68 SPECIALIZED 10ae — +10 únicos 31/33 (316/332) sem parar"""
...
def patch68(roots):
    items=[
        ("ARKHER_Xbox","XBOX — Live  •  Achieve",0x107C10,"Live  •  Achieve  •  Store","Live Achieve Store  •  Xbox"),
        ("ARKHER_PlayStation","PLAYSTATION — PSN  •  Trophy",0x003791,"PSN  •  Trophy  •  Store","PSN Trophy Store  •  PS5"),
        ("ARKHER_Switch","SWITCH — Online  •  eShop",0xE60012,"Online  •  eShop  •  JoyCon","Online eShop JoyCon  •  Switch"),
        ("ARKHER_MobileStore","MOBILESTORE — IAP  •  Review",0x4285F4,"IAP  •  Review  •  Install","IAP Review Install  •  Mobile"),
        ("ARKHER_Desktop","DESKTOP — Win  •  Mac",0x0078D4,"Win  •  Mac  •  Linux","Win Mac Linux  •  Desktop"),
        ("ARKHER_Web","WEB — Browser  •  PWA",0x4285F4,"Browser  •  PWA  •  Offline","Browser PWA Offline  •  Web"),
        ("ARKHER_VR2","VR2 — Quest  •  Index",0x01579B,"Quest  •  Index  •  Hand","Quest Index Hand  •  VR"),
        ("ARKHER_CloudGame","CLOUDGAME — Stream  •  Queue",0xFF6F00,"Stream  •  Queue  •  Latency","Stream Queue Latency  •  Cloud"),
        ("ARKHER_CrossPlay","CROSSPLAY — Friend  •  Party",0x7C4DFF,"Friend  •  Party  •  Voice","Friend Party Voice  •  Cross"),
        ("ARKHER_CrossSave","CROSSSAVE — Slot  •  Sync",0x651FFF,"Slot  •  Sync  •  Conflict","Slot Sync Conflict  •  Cloud Save"),
    ]
...
for _,n in ipairs({"ARKHER_Xbox","ARKHER_PlayStation","ARKHER_Switch","ARKHER_MobileStore","ARKHER_Desktop","ARKHER_Web","ARKHER_VR2","ARKHER_CloudGame","ARKHER_CrossPlay","ARKHER_CrossSave"}) do
...
print("[V1.68 10x] Xbox/PlayStation/Switch/MobileStore/Desktop/Web/VR2/CloudGame/CrossPlay/CrossSave 316/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_68",{"Source":(T_STRING, FUNC68)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch67(roots)
    roots=patch68(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_68.rbxl")
    write(out, serialize(roots))
    print(f"V1.68 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_68.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.68 done")
