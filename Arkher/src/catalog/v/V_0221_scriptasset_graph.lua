-- ARKHER SYSTEM V.0221 :: Script Asset Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0221"
	S.key = "arkher.asset.scriptasset.dependency_graph"
	S.name = "Script Asset Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.scriptasset.asset_registry" }
	S.tags = { "v", "scriptasset", "graph", "asset" }
	S.description = "Script Asset Dependency Graph: what references what, and in which order it must be built for the Script Asset subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 520,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.scriptasset.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.scriptasset.*", function(payload) inst.lastSignal = payload end)
			end
