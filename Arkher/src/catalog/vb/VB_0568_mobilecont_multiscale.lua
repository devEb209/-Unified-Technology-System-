-- ARKHER SYSTEM VB.0568 :: Mobile Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0568"
	S.key = "arkher.conttime.mobilecont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.mobilecont.sparse_residency" }
	S.tags = { "vb", "mobilecont", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.mobilecont.multi-scale_fidelity", levels = 5, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.mobilecont.*", function(payload) inst.lastSignal = payload end)
			end
