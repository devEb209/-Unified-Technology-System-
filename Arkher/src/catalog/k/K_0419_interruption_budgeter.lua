-- ARKHER SYSTEM K.0419 :: Interruption Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0419"
	S.key = "arkher.npc.interruption.thinking_budget"
	S.name = "Interruption Thinking Budget"
	S.category = "K"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.interruption.daily_schedule" }
	S.tags = { "k", "interruption", "budgeter", "npc" }
	S.description = "Interruption Thinking Budget: allocation of per-frame thinking time across NPCs for the Interruption subsystem."
	S.params = {
		backlogLimit = 29,
		baseRadius = 520,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.interruption.thinking_budget", total = 361, strategy = "proportional" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.interruption.*", function(payload) inst.lastSignal = payload end)
			end
