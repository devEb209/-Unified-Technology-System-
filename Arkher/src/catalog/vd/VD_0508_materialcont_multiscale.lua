-- ARKHER SYSTEM VD.0508 :: Material Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0508"
	S.key = "arkher.contsim.materialcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.materialcont.sparse_residency" }
	S.tags = { "vd", "materialcont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.materialcont.multi-scale_fidelity", levels = 6, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.materialcont.*", function(payload) inst.lastSignal = payload end)
			end
