#!/usr/bin/env python3
"""GENESIS V1.66 SPECIALIZED 10ac — +10 únicos 29/33 (296/332) sem parar"""
...
def patch66(roots):
    items=[
        ("ARKHER_Tax","TAX — Region  •  Rate",0x1B5E20,"Region  •  Rate  •  Inclusive","Region Rate Inclusive  •  19% DE"),
        ("ARKHER_Shipping","SHIPPING — Method  •  Cost",0x01579B,"Method  •  Cost  •  Track","Method Cost Track  •  Free > $50"),
        ("ARKHER_Inventory2","INVENTORY2 — Stock  •  Low",0x33691E,"Stock  •  Low  •  Reorder","Stock Low Reorder  •  5 left"),
        ("ARKHER_Warehouse","WAREHOUSE — Bin  •  Pick",0x5D4037,"Bin  •  Pick  •  Pack","Bin Pick Pack  •  A-12"),
        ("ARKHER_Fulfill","FULFILL — Order  •  Ship",0x37474F,"Order  •  Ship  •  Track","Order Ship Track  •  Shipped"),
        ("ARKHER_Return","RETURN — Reason  •  Refund",0xAD1457,"Reason  •  Refund  •  Inspect","Reason Refund Inspect  •  30 days"),
        ("ARKHER_Warranty","WARRANTY — Period  •  Claim",0x00838F,"Period  •  Claim  •  Status","Period Claim Status  •  2 years"),
        ("ARKHER_Support2","SUPPORT2 — Chat  •  Ticket",0x0288D1,"Chat  •  Ticket  •  Bot","Chat Ticket Bot  •  Live"),
        ("ARKHER_KB","KB — Article  •  Search",0x4A148C,"Article  •  Search  •  Vote","Article Search Vote  •  Helpful"),
        ("ARKHER_Community","COMMUNITY — Forum  •  Post",0x311B92,"Forum  •  Post  •  Like","Forum Post Like  •  10k posts"),
    ]
...
for _,n in ipairs({"ARKHER_Tax","ARKHER_Shipping","ARKHER_Inventory2","ARKHER_Warehouse","ARKHER_Fulfill","ARKHER_Return","ARKHER_Warranty","ARKHER_Support2","ARKHER_KB","ARKHER_Community"}) do
...
print("[V1.66 10x] Tax/Shipping/Inventory2/Warehouse/Fulfill/Return/Warranty/Support2/KB/Community 296/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_66",{"Source":(T_STRING, FUNC66)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch65(roots)
    roots=patch66(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_66.rbxl")
    write(out, serialize(roots))
    print(f"V1.66 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_66.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.66 done")
