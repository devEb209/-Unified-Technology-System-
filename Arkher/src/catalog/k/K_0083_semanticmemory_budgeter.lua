-- ARKHER SYSTEM K.0083 :: Semantic Memory Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0083"
	S.key = "arkher.npc.semanticmemory.thinking_budget"
	S.name = "Semantic Memory Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Semantic Memory"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.semanticmemory.daily_schedule" }
	S.tags = { "k", "semanticmemory", "budgeter", "npc" }
	S.description = "Semantic Memory Thinking Budget: allocation of per-frame thinking time across NPCs for the Semantic Memory subsystem."
	S.params = {
		backlogLimit = 39,
		baseRadius = 600,
		baseWeight = 0.91,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.semanticmemory.thinking_budget", total = 451, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.semanticmemory.*", function(payload) inst.lastSignal = payload end)
			end
