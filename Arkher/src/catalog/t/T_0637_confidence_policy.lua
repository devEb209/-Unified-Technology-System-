-- ARKHER SYSTEM T.0637 :: Confidence Estimation Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0637"
	S.key = "arkher.ai.confidence.decision_policy"
	S.name = "Confidence Estimation Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.confidence.reasoning_graph" }
	S.tags = { "t", "confidence", "policy", "ai" }
	S.description = "Confidence Estimation Decision Policy: ordered rules deciding what the agent does next for the Confidence Estimation subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 160,
		baseWeight = 0.68,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.confidence.decision_policy", algorithm = "roundrobin", starvationGuard = 28 })
		inst.system = S
		inst.ctx = ctx
