-- ARKHER SYSTEM VD.0004 :: World Grid Cell Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0004"
	S.key = "arkher.contsim.worldcell.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.worldcell.sparse_residency" }
	S.tags = { "vd", "worldcell", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.worldcell.multi-scale_fidelity", levels = 6, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.worldcell.*", function(payload) inst.lastSignal = payload end)
			end
