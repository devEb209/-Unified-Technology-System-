-- ARKHER SYSTEM T.0554 :: Scene Reasoning Outcome Prediction
...
return function(A)
	...
	local S = {}
	S.id = "T.0554"
	S.key = "arkher.ai.scenereasoning.outcome_prediction"
	S.name = "Scene Reasoning Outcome Prediction"
	S.category = "T"
	...
	S.kit = "predictor"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.scenereasoning.decision_policy" }
	S.tags = { "t", "scenereasoning", "predictor", "ai" }
	S.description = "Scene Reasoning Outcome Prediction: forecasting the result before the work is done for the Scene Reasoning subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 320,
		baseWeight = 0.82,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("predictor", { id = "arkher.ai.scenereasoning.outcome_prediction", method = "linear", alpha = 0.22, window = 60 })
		inst.system = S
		inst.ctx = ctx
