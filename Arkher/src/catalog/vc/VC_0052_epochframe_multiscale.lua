-- ARKHER SYSTEM VC.0052 :: Epoch Frame Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0052"
	S.key = "arkher.contneural.epochframe.multi-scale_fidelity"
	S.name = "Epoch Frame Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.epochframe.sparse_residency" }
	S.tags = { "vc", "epochframe", "multiscale", "contneural" }
	S.description = "Epoch Frame Multi-Scale Fidelity: distance-driven multi-scale level selection for the Epoch Frame subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 320,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.epochframe.multi-scale_fidelity", levels = 5, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.epochframe.*", function(payload) inst.lastSignal = payload end)
			end
