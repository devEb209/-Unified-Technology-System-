-- ARKHER SYSTEM P.0105 :: Inventory Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0105"
	S.key = "arkher.play.inventorysys.difficulty_controller"
	S.name = "Inventory Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Inventory"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.inventorysys.balance_analysis" }
	S.tags = { "p", "inventorysys", "controller", "play" }
	S.description = "Inventory Difficulty Controller: closed-loop tuning toward a target challenge for the Inventory subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 360,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.inventorysys.difficulty_controller", mode = "hysteresis", target = 0.590, kp = 0.540, ki = 0.100, kd = 0.050, min = 0.05, max = 1.0, initial = 0.49, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.inventorysys.*", function(payload) inst.lastSignal = payload end)
			end
