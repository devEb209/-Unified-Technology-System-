-- ARKHER SYSTEM VD.0496 :: Rendering Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0496"
	S.key = "arkher.contsim.rendercont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.rendercont.sparse_residency" }
	S.tags = { "vd", "rendercont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.rendercont.multi-scale_fidelity", levels = 4, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.rendercont.*", function(payload) inst.lastSignal = payload end)
			end
