-- ARKHER SYSTEM G.0332 :: Model Distillation Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0332"
	S.key = "arkher.neural.distillation.heuristic_policy"
	S.name = "Model Distillation Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.distillation.result_cache" }
	S.tags = { "g", "distillation", "policy", "neural" }
	S.description = "Model Distillation Heuristic Policy: rule fallback when the model is unavailable for the Model Distillation subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 480,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.distillation.heuristic_policy", algorithm = "roundrobin", starvationGuard = 36 })
		inst.system = S
		inst.ctx = ctx
