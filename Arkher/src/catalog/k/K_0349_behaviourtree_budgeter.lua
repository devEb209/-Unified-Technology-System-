-- ARKHER SYSTEM K.0349 :: Behaviour Tree Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0349"
	S.key = "arkher.npc.behaviourtree.thinking_budget"
	S.name = "Behaviour Tree Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Behaviour Tree"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.behaviourtree.daily_schedule" }
	S.tags = { "k", "behaviourtree", "budgeter", "npc" }
	S.description = "Behaviour Tree Thinking Budget: allocation of per-frame thinking time across NPCs for the Behaviour Tree subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 360,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.behaviourtree.thinking_budget", total = 157, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.behaviourtree.*", function(payload) inst.lastSignal = payload end)
			end
