-- ARKHER SYSTEM N.0423 :: Foliage Motion Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0423"
	S.key = "arkher.vfx.foliagemotion.effect_composition"
	S.name = "Foliage Motion Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Foliage Motion"
	S.aspect = "Effect Composition"
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.foliagemotion.effect_graph" }
	S.tags = { "n", "foliagemotion", "composer", "vfx" }
	S.description = "Foliage Motion Effect Composition: blending of several effect signals into one output for the Foliage Motion subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.9,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.foliagemotion.effect_composition", mode = "max" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.foliagemotion.*", function(payload) inst.lastSignal = payload end)
			end
