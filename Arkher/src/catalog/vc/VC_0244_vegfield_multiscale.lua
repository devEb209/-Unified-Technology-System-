-- ARKHER SYSTEM VC.0244 :: Vegetation Field Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0244"
	S.key = "arkher.contneural.vegfield.multi-scale_fidelity"
	S.name = "Vegetation Field Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.vegfield.sparse_residency" }
	S.tags = { "vc", "vegfield", "multiscale", "contneural" }
	S.description = "Vegetation Field Multi-Scale Fidelity: distance-driven multi-scale level selection for the Vegetation Field subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 280,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.vegfield.multi-scale_fidelity", levels = 4, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.vegfield.*", function(payload) inst.lastSignal = payload end)
			end
