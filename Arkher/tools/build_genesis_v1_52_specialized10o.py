#!/usr/bin/env python3
"""GENESIS V1.52 SPECIALIZED 10o — +10 únicos 15/33 (156/332) sem parar"""
...
def patch52(roots):
    items=[
        ("ARKHER_Achieve","ACHIEVE — Trophy  •  Progress",0xFFD600,"Trophy  •  Progress  •  Hidden","Trophy Progress Hidden  •  100%"),
        ("ARKHER_Trophy","TROPHY — Platinum  •  Gold",0xFFB800,"Platinum  •  Gold  •  Silver","Platinum Gold Silver  •  Collect"),
        ("ARKHER_Challenge","CHALLENGE — Weekly  •  Daily",0xFF6F00,"Weekly  •  Daily  •  Reward","Weekly Daily Reward  •  Reset"),
        ("ARKHER_Leader2","LEADER2 — Global  •  Friends",0x0288D1,"Global  •  Friends  •  Season","Global Friends Season  •  Rank"),
        ("ARKHER_Social","SOCIAL — Feed  •  Like",0x01579B,"Feed  •  Like  •  Share","Feed Like Share  •  Follow"),
        ("ARKHER_Chat2","CHAT — Channel  •  Filter",0x4A148C,"Channel  •  Filter  •  Mute","Channel Filter Mute  •  Profanity"),
        ("ARKHER_Emote","EMOTE — Wheel  •  Dance",0xAD1457,"Wheel  •  Dance  •  Sync","Wheel Dance Sync  •  8 slots"),
        ("ARKHER_Cosmetic","COSMETIC — Skin  •  Attach",0x6A1B9A,"Skin  •  Attach  •  Rarity","Skin Attach Rarity  •  Preview"),
        ("ARKHER_Skin","SKIN — Material  •  Pattern",0x5D4037,"Material  •  Pattern  •  Wear","Material Pattern Wear  •  0-1"),
        ("ARKHER_Pet","PET — Follow  •  Ability",0x2E7D32,"Follow  •  Ability  •  Evolve","Follow Ability Evolve  •  Level 50"),
    ]
...
for _,n in ipairs({"ARKHER_Achieve","ARKHER_Trophy","ARKHER_Challenge","ARKHER_Leader2","ARKHER_Social","ARKHER_Chat2","ARKHER_Emote","ARKHER_Cosmetic","ARKHER_Skin","ARKHER_Pet"}) do
...
print("[V1.52 10x] Achieve/Trophy/Challenge/Leader2/Social/Chat2/Emote/Cosmetic/Skin/Pet 156/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_52",{"Source":(T_STRING, FUNC52)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch51(roots)
    roots=patch52(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_52.rbxl")
    write(out, serialize(roots))
    print(f"V1.52 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_52.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.52 done")
