-- ARKHER SYSTEM G.0237 :: Animation Compression Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0237"
	S.key = "arkher.neural.animcompress.sampling_graph"
	S.name = "Animation Compression Sampling Graph"
	S.category = "G"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.animcompress.training_ledger" }
	S.tags = { "g", "animcompress", "graph", "neural" }
	S.description = "Animation Compression Sampling Graph: dependency graph of sampling stages for the Animation Compression subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 160,
		baseWeight = 0.64,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.animcompress.sampling_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.animcompress.*", function(payload) inst.lastSignal = payload end)
			end
