-- ARKHER SYSTEM G.0374 :: Feature Cache Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0374"
	S.key = "arkher.neural.featurecache.heuristic_policy"
	S.name = "Feature Cache Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.featurecache.result_cache" }
	S.tags = { "g", "featurecache", "policy", "neural" }
	S.description = "Feature Cache Heuristic Policy: rule fallback when the model is unavailable for the Feature Cache subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 480,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.featurecache.heuristic_policy", algorithm = "roundrobin", starvationGuard = 60 })
		inst.system = S
		inst.ctx = ctx
