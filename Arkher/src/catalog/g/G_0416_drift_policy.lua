-- ARKHER SYSTEM G.0416 :: Drift Detection Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0416"
	S.key = "arkher.neural.drift.heuristic_policy"
	S.name = "Drift Detection Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.drift.result_cache" }
	S.tags = { "g", "drift", "policy", "neural" }
	S.description = "Drift Detection Heuristic Policy: rule fallback when the model is unavailable for the Drift Detection subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 160,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.drift.heuristic_policy", algorithm = "roundrobin", starvationGuard = 16 })
		inst.system = S
		inst.ctx = ctx
