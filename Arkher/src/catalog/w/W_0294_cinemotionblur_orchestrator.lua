-- ARKHER SYSTEM W.0294 :: Motion Blur Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0294"
	S.key = "arkher.cine.cinemotionblur.playback_orchestrator"
	S.name = "Motion Blur Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Motion Blur"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.cinemotionblur.cut_policy" }
	S.tags = { "w", "cinemotionblur", "orchestrator", "cine" }
	S.description = "Motion Blur Playback Orchestrator: the state machine that drives a cinematic from start to end for the Motion Blur subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 160,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.cinemotionblur.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
