-- ARKHER SYSTEM N.0215 :: Lightning Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0215"
	S.key = "arkher.vfx.lightning.effect_composition"
	S.name = "Lightning Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Lightning"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.lightning.effect_graph" }
	S.tags = { "n", "lightning", "composer", "vfx" }
	S.description = "Lightning Effect Composition: blending of several effect signals into one output for the Lightning subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 200,
		baseWeight = 0.73,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.lightning.effect_composition", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.lightning.*", function(payload) inst.lastSignal = payload end)
			end
