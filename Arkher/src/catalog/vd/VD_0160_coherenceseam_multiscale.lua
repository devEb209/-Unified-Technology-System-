-- ARKHER SYSTEM VD.0160 :: Coherence Seam Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0160"
	S.key = "arkher.contsim.coherenceseam.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.coherenceseam.sparse_residency" }
	S.tags = { "vd", "coherenceseam", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.coherenceseam.multi-scale_fidelity", levels = 6, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.coherenceseam.*", function(payload) inst.lastSignal = payload end)
			end
