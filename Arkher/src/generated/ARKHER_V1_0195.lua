--!strict
-- Unity6 URP — ANIMATION Stack #0195
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Unity6 URP — ANIMATION Stack #0195", size=Vector2.new(600, 420), pos=UDim2.fromOffset(235, 275), parent=parent, icon="⤢"})
...
Components.Slider(props, "Strength", 0,1,0.5, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_S"..195, v) end end)
Components.Checkbox(props, "Enabled", true, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_E"..195, v) end end)
