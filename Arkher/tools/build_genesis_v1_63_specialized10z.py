#!/usr/bin/env python3
"""GENESIS V1.63 SPECIALIZED 10z — +10 únicos 26/33 (266/332) sem parar"""
...
def patch63(roots):
    items=[
        ("ARKHER_Cookie","COOKIE — Consent  •  Prefs",0x33691E,"Consent  •  Prefs  •  Banner","Consent Prefs Banner  •  GDPR"),
        ("ARKHER_Notif","NOTIF — Push  •  InGame",0x01579B,"Push  •  InGame  •  Email","Push InGame Email  •  Opt-in"),
        ("ARKHER_News","NEWS — Article  •  Carousel",0x0288D1,"Article  •  Carousel  •  Pin","Article Carousel Pin  •  3 news"),
        ("ARKHER_PatchNote","PATCHNOTE — Version  •  Highlight",0x37474F,"Version  •  Highlight  •  Date","Version Highlight Date  •  v1.63"),
        ("ARKHER_Event2","EVENT2 — Banner  •  Timer",0xFF6F00,"Banner  •  Timer  •  Reward","Banner Timer Reward  •  2 days"),
        ("ARKHER_Offer","OFFER — Discount  •  Bundle",0xFFD600,"Discount  •  Bundle  •  Timer","Discount Bundle Timer  •  50% off"),
        ("ARKHER_Sale","SALE — Flash  •  Countdown",0xB71C1C,"Flash  •  Countdown  •  Stock","Flash Countdown Stock  •  100 left"),
        ("ARKHER_Coupon","COUPON — Code  •  Redeem",0x4A148C,"Code  •  Redeem  •  Expire","Code Redeem Expire  •  ARKHER10"),
        ("ARKHER_Gift","GIFT — Send  •  Claim",0xAD1457,"Send  •  Claim  •  Expire","Send Claim Expire  •  7 days"),
        ("ARKHER_Referral","REFERRAL — Link  •  Bonus",0x1B5E20,"Link  •  Bonus  •  Tier","Link Bonus Tier  •  10% bonus"),
    ]
...
for _,n in ipairs({"ARKHER_Cookie","ARKHER_Notif","ARKHER_News","ARKHER_PatchNote","ARKHER_Event2","ARKHER_Offer","ARKHER_Sale","ARKHER_Coupon","ARKHER_Gift","ARKHER_Referral"}) do
...
print("[V1.63 10x] Cookie/Notif/News/PatchNote/Event2/Offer/Sale/Coupon/Gift/Referral 266/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_63",{"Source":(T_STRING, FUNC63)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch62(roots)
    roots=patch63(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_63.rbxl")
    write(out, serialize(roots))
    print(f"V1.63 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_63.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.63 done")
