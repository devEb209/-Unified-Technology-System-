-- ARKHER SYSTEM L.0056 :: Catch Up Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0056"
	S.key = "arkher.sim.catchup.fidelity_policy"
	S.name = "Catch Up Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.catchup.streaming" }
	S.tags = { "l", "catchup", "policy", "sim" }
	S.description = "Catch Up Fidelity Policy: rules that pick full, cohort or statistical simulation for the Catch Up subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 560,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.catchup.fidelity_policy", algorithm = "wfq", starvationGuard = 26 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.catchup.*", function(payload) inst.lastSignal = payload end)
			end
