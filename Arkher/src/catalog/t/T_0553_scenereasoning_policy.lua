-- ARKHER SYSTEM T.0553 :: Scene Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0553"
	S.key = "arkher.ai.scenereasoning.decision_policy"
	S.name = "Scene Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.scenereasoning.reasoning_graph" }
	S.tags = { "t", "scenereasoning", "policy", "ai" }
	S.description = "Scene Reasoning Decision Policy: ordered rules deciding what the agent does next for the Scene Reasoning subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 240,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.scenereasoning.decision_policy", algorithm = "wfq", starvationGuard = 18 })
		inst.system = S
		inst.ctx = ctx
