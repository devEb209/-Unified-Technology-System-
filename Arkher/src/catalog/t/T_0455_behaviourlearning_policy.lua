-- ARKHER SYSTEM T.0455 :: Behaviour Learning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0455"
	S.key = "arkher.ai.behaviourlearning.decision_policy"
	S.name = "Behaviour Learning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.behaviourlearning.reasoning_graph" }
	S.tags = { "t", "behaviourlearning", "policy", "ai" }
	S.description = "Behaviour Learning Decision Policy: ordered rules deciding what the agent does next for the Behaviour Learning subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 320,
		baseWeight = 0.94,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.behaviourlearning.decision_policy", algorithm = "roundrobin", starvationGuard = 20 })
		inst.system = S
		inst.ctx = ctx
