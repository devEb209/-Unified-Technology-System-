-- ARKHER SYSTEM Q.0141 :: Menu Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0141"
	S.key = "arkher.ui.menu.accessibility_policy"
	S.name = "Menu Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Menu"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.menu.interaction_ledger" }
	S.tags = { "q", "menu", "policy", "ui" }
	S.description = "Menu Accessibility Policy: rules enforcing contrast, target size and motion limits for the Menu subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 240,
		baseWeight = 0.94,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.menu.accessibility_policy", algorithm = "wfq", starvationGuard = 18 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.menu.*", function(payload) inst.lastSignal = payload end)
			end
