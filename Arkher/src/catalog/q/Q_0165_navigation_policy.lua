-- ARKHER SYSTEM Q.0165 :: Navigation Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0165"
	S.key = "arkher.ui.navigation.accessibility_policy"
	S.name = "Navigation Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Navigation"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.navigation.interaction_ledger" }
	S.tags = { "q", "navigation", "policy", "ui" }
	S.description = "Navigation Accessibility Policy: rules enforcing contrast, target size and motion limits for the Navigation subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 600,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.navigation.accessibility_policy", algorithm = "lottery", starvationGuard = 27 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.navigation.*", function(payload) inst.lastSignal = payload end)
			end
