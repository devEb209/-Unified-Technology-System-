-- ARKHER SYSTEM N.0163 :: Snow Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0163"
	S.key = "arkher.vfx.snow.effect_composition"
	S.name = "Snow Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Snow"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.snow.effect_graph" }
	S.tags = { "n", "snow", "composer", "vfx" }
	S.description = "Snow Effect Composition: blending of several effect signals into one output for the Snow subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 480,
		baseWeight = 0.78,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.snow.effect_composition", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.snow.*", function(payload) inst.lastSignal = payload end)
			end
