-- ARKHER SYSTEM P.0045 :: Ability Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0045"
	S.key = "arkher.play.ability.difficulty_controller"
	S.name = "Ability Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Ability"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.ability.balance_analysis" }
	S.tags = { "p", "ability", "controller", "play" }
	S.description = "Ability Difficulty Controller: closed-loop tuning toward a target challenge for the Ability subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 440,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.ability.difficulty_controller", mode = "bangbang", target = 0.530, kp = 0.380, ki = 0.060, kd = 0.040, min = 0.05, max = 1.0, initial = 0.43, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.ability.*", function(payload) inst.lastSignal = payload end)
			end
