-- ARKHER SYSTEM H.0220 :: Fixed Timestep Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0220"
	S.key = "arkher.physics.fixedstep.simulation_policy"
	S.name = "Fixed Timestep Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Fixed Timestep"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.fixedstep.budget_governor" }
	S.tags = { "h", "fixedstep", "policy", "physics" }
	S.description = "Fixed Timestep Simulation Policy: device tier rules for substeps, iterations and sleep for the Fixed Timestep subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 360,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.fixedstep.simulation_policy", algorithm = "edf", starvationGuard = 45 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.fixedstep.*", function(payload) inst.lastSignal = payload end)
			end
