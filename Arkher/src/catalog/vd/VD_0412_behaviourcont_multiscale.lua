-- ARKHER SYSTEM VD.0412 :: Behaviour Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0412"
	S.key = "arkher.contsim.behaviourcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.behaviourcont.sparse_residency" }
	S.tags = { "vd", "behaviourcont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.behaviourcont.multi-scale_fidelity", levels = 6, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.behaviourcont.*", function(payload) inst.lastSignal = payload end)
			end
