-- ARKHER SYSTEM T.0078 :: Project Knowledge Outcome Prediction
...
return function(A)
	...
	local S = {}
	S.id = "T.0078"
	S.key = "arkher.ai.projectknowledge.outcome_prediction"
	S.name = "Project Knowledge Outcome Prediction"
	S.category = "T"
	S.family = "SINGULARITY AI"
	S.area = "Project Knowledge"
	S.aspect = "Outcome Prediction"
	S.kit = "predictor"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.projectknowledge.decision_policy" }
	S.tags = { "t", "projectknowledge", "predictor", "ai" }
	S.description = "Project Knowledge Outcome Prediction: forecasting the result before the work is done for the Project Knowledge subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 160,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("predictor", { id = "arkher.ai.projectknowledge.outcome_prediction", method = "ema", alpha = 0.10, window = 80 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.trustworthy() return inst.confidence() >= S.params.minConfidence end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ai.projectknowledge.*", function(payload) inst.lastSignal = payload end)
			end
