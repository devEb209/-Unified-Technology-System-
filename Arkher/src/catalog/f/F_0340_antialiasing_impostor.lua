-- ARKHER SYSTEM F.0340 :: Antialiasing Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0340"
	S.key = "arkher.render.antialiasing.geometry_virtualization"
	S.name = "Antialiasing Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Antialiasing"
	...
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.antialiasing.visibility_set" }
	S.tags = { "f", "antialiasing", "impostor", "render" }
	S.description = "Antialiasing Geometry Virtualization: impostor and HLOD synthesis under a budget for the Antialiasing subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 280,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.antialiasing.geometry_virtualization", atlasSlots = 80, viewCount = 8, errorThreshold = 1.90 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.antialiasing.*", function(payload) inst.lastSignal = payload end)
			end
