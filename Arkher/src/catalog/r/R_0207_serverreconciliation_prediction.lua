-- ARKHER SYSTEM R.0207 :: Server Reconciliation Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0207"
	S.key = "arkher.net.serverreconciliation.prediction_and_reconciliation"
	S.name = "Server Reconciliation Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Server Reconciliation"
	S.aspect = "Prediction And Reconciliation"
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.serverreconciliation.time_sync" }
	S.tags = { "r", "serverreconciliation", "prediction", "net" }
	S.description = "Server Reconciliation Prediction And Reconciliation: client simulation, correction and input replay for the Server Reconciliation subsystem."
	S.params = {
		backlogLimit = 37,
		baseRadius = 520,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.serverreconciliation.prediction_and_reconciliation", errorThreshold = 0.075, smoothing = 17.0, maxInputs = 120 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.serverreconciliation.*", function(payload) inst.lastSignal = payload end)
			end
