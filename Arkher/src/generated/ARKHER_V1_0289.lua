--!strict
-- Unity6 URP — ANIMATION Profiler #0289
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Unity6 URP — ANIMATION Profiler #0289", size=Vector2.new(640, 500), pos=UDim2.fromOffset(257, 313), parent=parent, icon="●"})
...
Components.TextField(form, "Name", "Object_"..289, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
