-- ARKHER SYSTEM L.0581 :: World History Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0581"
	S.key = "arkher.sim.worldhistory.fidelity_policy"
	S.name = "World History Fidelity Policy"
	S.category = "L"
	S.family = "WORLD SIMULATION"
	S.area = "World History"
	S.aspect = "Fidelity Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.worldhistory.streaming" }
	S.tags = { "l", "worldhistory", "policy", "sim" }
	S.description = "World History Fidelity Policy: rules that pick full, cohort or statistical simulation for the World History subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 160,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.worldhistory.fidelity_policy", algorithm = "roundrobin", starvationGuard = 28 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.worldhistory.*", function(payload) inst.lastSignal = payload end)
			end
