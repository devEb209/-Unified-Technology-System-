-- ARKHER SYSTEM H.0248 :: Broadphase Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0248"
	S.key = "arkher.physics.broadphase.simulation_policy"
	S.name = "Broadphase Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Broadphase"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.broadphase.budget_governor" }
	S.tags = { "h", "broadphase", "policy", "physics" }
	S.description = "Broadphase Simulation Policy: device tier rules for substeps, iterations and sleep for the Broadphase subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 240,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.broadphase.simulation_policy", algorithm = "wfq", starvationGuard = 54 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.broadphase.*", function(payload) inst.lastSignal = payload end)
			end
