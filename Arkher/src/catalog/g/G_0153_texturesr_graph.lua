-- ARKHER SYSTEM G.0153 :: Texture Super Resolution Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0153"
	S.key = "arkher.neural.texturesr.sampling_graph"
	S.name = "Texture Super Resolution Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Texture Super Resolution"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.texturesr.training_ledger" }
	S.tags = { "g", "texturesr", "graph", "neural" }
	S.description = "Texture Super Resolution Sampling Graph: dependency graph of sampling stages for the Texture Super Resolution subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 440,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.texturesr.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.texturesr.*", function(payload) inst.lastSignal = payload end)
			end
