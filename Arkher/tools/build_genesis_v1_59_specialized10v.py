#!/usr/bin/env python3
"""GENESIS V1.59 SPECIALIZED 10v — +10 únicos 22/33 (226/332) sem parar"""
...
def patch59(roots):
    items=[
        ("ARKHER_Stat","STAT — FPS  •  Draw",0xAD1457,"FPS  •  Draw  •  Tris","FPS Draw Tris  •  1800 draw 4M tris"),
        ("ARKHER_Profile2","PROFILE2 — GPU  •  Pass",0x37474F,"GPU  •  Pass  •  Scope","GPU Pass Scope  •  12ms"),
        ("ARKHER_Mem2","MEM2 — Budget  •  Pool",0x00838F,"Budget  •  Pool  •  Leak","Budget Pool Leak  •  2GB"),
        ("ARKHER_Trace","TRACE — Ray  •  Hit",0xBF360C,"Ray  •  Hit  •  Miss","Ray Hit Miss  •  TLAS"),
        ("ARKHER_Log2","LOG2 — Filter  •  Verbose",0x263238,"Filter  •  Verbose  •  Export","Filter Verbose Export  •  Log"),
        ("ARKHER_Output2","OUTPUT2 — Console  •  Error",0xB71C1C,"Console  •  Error  •  Clear","Console Error Clear  •  0 errors"),
        ("ARKHER_Terminal","TERMINAL — Shell  •  Cmd",0x212121,"Shell  •  Cmd  •  Env","Shell Cmd Env  •  bash"),
        ("ARKHER_Task","TASK — Job  •  Queue",0x1B5E20,"Job  •  Queue  •  Worker","Job Queue Worker  •  8 threads"),
        ("ARKHER_Schedule","SCHEDULE — Cron  •  Trigger",0x33691E,"Cron  •  Trigger  •  Run","Cron Trigger Run  •  Daily 02:00"),
        ("ARKHER_Automation","AUTOMATION — Script  •  Action",0x2E7D32,"Script  •  Action  •  Chain","Script Action Chain  •  Auto"),
    ]
...
for _,n in ipairs({"ARKHER_Stat","ARKHER_Profile2","ARKHER_Mem2","ARKHER_Trace","ARKHER_Log2","ARKHER_Output2","ARKHER_Terminal","ARKHER_Task","ARKHER_Schedule","ARKHER_Automation"}) do
...
print("[V1.59 10x] Stat/Profile2/Mem2/Trace/Log2/Output2/Terminal/Task/Schedule/Automation 226/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_59",{"Source":(T_STRING, FUNC59)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch58(roots)
    roots=patch59(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_59.rbxl")
    write(out, serialize(roots))
    print(f"V1.59 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_59.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.59 done")
