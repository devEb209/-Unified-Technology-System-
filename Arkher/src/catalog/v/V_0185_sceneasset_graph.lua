-- ARKHER SYSTEM V.0185 :: Scene Asset Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0185"
	S.key = "arkher.asset.sceneasset.dependency_graph"
	S.name = "Scene Asset Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.sceneasset.asset_registry" }
	S.tags = { "v", "sceneasset", "graph", "asset" }
	S.description = "Scene Asset Dependency Graph: what references what, and in which order it must be built for the Scene Asset subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 480,
		baseWeight = 0.9,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.sceneasset.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.sceneasset.*", function(payload) inst.lastSignal = payload end)
			end
