--!strict
-- Terragen Sky — CORE Profiler #0004
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Terragen Sky — CORE Profiler #0004", size=Vector2.new(640, 500), pos=UDim2.fromOffset(152, 148), parent=parent, icon="✦"})
    win.Root.Name="ARKHER_V1_0004"
    win.Root.Visible=false
    local content=win.Content
    content.BackgroundColor3=Theme.tokens.void
    -- accent line per UI
    local accent=Instance.new("Frame"); accent.Size=UDim2.new(1,0,0,2); accent.BackgroundColor3=Theme.categoryColor("Terragen"); accent.BorderSizePixel=0; accent.Parent=content
...
Components.TextField(form, "Name", "Object_"..4, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
