-- ARKHER SYSTEM N.0241 :: Spell Cast Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0241"
	S.key = "arkher.vfx.spellcast.effect_composition"
	S.name = "Spell Cast Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Spell Cast"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.spellcast.effect_graph" }
	S.tags = { "n", "spellcast", "composer", "vfx" }
	S.description = "Spell Cast Effect Composition: blending of several effect signals into one output for the Spell Cast subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 160,
		baseWeight = 0.82,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.spellcast.effect_composition", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.spellcast.*", function(payload) inst.lastSignal = payload end)
			end
