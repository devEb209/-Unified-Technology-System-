-- ARKHER SYSTEM I.0461 :: Procedural Animation Retarget Graph
-- Category I - ANIMATION
-- ARKHER Animation Framework capability: skeletons, clips, layers, IK and physical blending.
-- Kit: graph (bone mapping and retarget path resolution)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "I.0461"
	S.key = "arkher.anim.proceduralanim.retarget_graph"
	S.name = "Procedural Animation Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Procedural Animation"
	S.aspect = "Retarget Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.proceduralanim.physical_blend" }
	S.tags = { "i", "proceduralanim", "graph", "anim" }
	S.description = "Procedural Animation Retarget Graph: bone mapping and retarget path resolution for the Procedural Animation subsystem."
	S.params = {
		backlogLimit = 40,
		baseRadius = 480,
		baseWeight = 0.82,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.proceduralanim.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.proceduralanim.*", function(payload) inst.lastSignal = payload end)
			end
