-- ARKHER SYSTEM VB.0028 :: Zone Chunk Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0028"
	S.key = "arkher.conttime.zonechunk.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.zonechunk.sparse_residency" }
	S.tags = { "vb", "zonechunk", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.zonechunk.multi-scale_fidelity", levels = 4, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.zonechunk.*", function(payload) inst.lastSignal = payload end)
			end
