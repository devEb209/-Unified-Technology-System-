...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Fairlight — CORE Profiler #0214", size=Vector2.new(640, 500), pos=UDim2.fromOffset(482, 338), parent=parent, icon="◨"})
...
Components.TextField(form, "Name", "Object_"..214, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
