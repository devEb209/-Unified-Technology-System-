-- ARKHER SYSTEM K.0503 :: Patrol Route Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0503"
	S.key = "arkher.npc.patrolroute.thinking_budget"
	S.name = "Patrol Route Thinking Budget"
	S.category = "K"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.patrolroute.daily_schedule" }
	S.tags = { "k", "patrolroute", "budgeter", "npc" }
	S.description = "Patrol Route Thinking Budget: allocation of per-frame thinking time across NPCs for the Patrol Route subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 400,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.patrolroute.thinking_budget", total = 118, strategy = "proportional" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.patrolroute.*", function(payload) inst.lastSignal = payload end)
			end
