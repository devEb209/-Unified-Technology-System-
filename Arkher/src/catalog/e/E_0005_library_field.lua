-- ARKHER SYSTEM E.0005 :: Material Library Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0005"
	S.key = "arkher.material.library.procedural_detail"
	S.name = "Material Library Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Material Library"
	S.aspect = "Procedural Detail"
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.library.texture_sampling" }
	S.tags = { "e", "library", "field", "material" }
	S.description = "Material Library Procedural Detail: coherent noise detail driving the surface for the Material Library subsystem."
	S.params = {
		backlogLimit = 39,
		baseRadius = 440,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.library.procedural_detail", seed = 40711, frequency = 0.0118, octaves = 6, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.library.*", function(payload) inst.lastSignal = payload end)
			end
