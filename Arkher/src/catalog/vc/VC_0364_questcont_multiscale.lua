-- ARKHER SYSTEM VC.0364 :: Quest Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0364"
	S.key = "arkher.contneural.questcont.multi-scale_fidelity"
	S.name = "Quest Continuum Multi-Scale Fidelity"
	S.category = "VC"
	S.family = "CONTINUUM — NEURAL"
	S.area = "Quest Continuum"
	S.aspect = "Multi-Scale Fidelity"
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.questcont.sparse_residency" }
	S.tags = { "vc", "questcont", "multiscale", "contneural" }
	S.description = "Quest Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Quest Continuum subsystem."
	S.params = {
		backlogLimit = 14,
		baseRadius = 560,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.questcont.multi-scale_fidelity", levels = 5, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.questcont.*", function(payload) inst.lastSignal = payload end)
			end
