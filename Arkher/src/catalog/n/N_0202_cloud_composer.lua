-- ARKHER SYSTEM N.0202 :: Cloud Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0202"
	S.key = "arkher.vfx.cloud.effect_composition"
	S.name = "Cloud Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Cloud"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.cloud.effect_graph" }
	S.tags = { "n", "cloud", "composer", "vfx" }
	S.description = "Cloud Effect Composition: blending of several effect signals into one output for the Cloud subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 560,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.cloud.effect_composition", mode = "max" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.cloud.*", function(payload) inst.lastSignal = payload end)
			end
