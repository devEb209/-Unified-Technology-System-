-- ARKHER SYSTEM F.0158 :: Volumetric Light Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0158"
	S.key = "arkher.render.volumetric.geometry_virtualization"
	S.name = "Volumetric Light Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Volumetric Light"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.volumetric.visibility_set" }
	S.tags = { "f", "volumetric", "impostor", "render" }
	S.description = "Volumetric Light Geometry Virtualization: impostor and HLOD synthesis under a budget for the Volumetric Light subsystem."
	S.params = {
		backlogLimit = 47,
		baseRadius = 440,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.volumetric.geometry_virtualization", atlasSlots = 48, viewCount = 8, errorThreshold = 2.90 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.volumetric.*", function(payload) inst.lastSignal = payload end)
			end
