-- ARKHER SYSTEM T.0189 :: Terrain Generation Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0189"
	S.key = "arkher.ai.terraingeneration.decision_policy"
	S.name = "Terrain Generation Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.terraingeneration.reasoning_graph" }
	S.tags = { "t", "terraingeneration", "policy", "ai" }
	S.description = "Terrain Generation Decision Policy: ordered rules deciding what the agent does next for the Terrain Generation subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 560,
		baseWeight = 0.68,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.terraingeneration.decision_policy", algorithm = "wfq", starvationGuard = 62 })
		inst.system = S
		inst.ctx = ctx
