-- ARKHER SYSTEM P.0117 :: Equipment Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0117"
	S.key = "arkher.play.equipment.difficulty_controller"
	S.name = "Equipment Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Equipment"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.equipment.balance_analysis" }
	S.tags = { "p", "equipment", "controller", "play" }
	S.description = "Equipment Difficulty Controller: closed-loop tuning toward a target challenge for the Equipment subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 400,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.equipment.difficulty_controller", mode = "pid", target = 0.800, kp = 0.250, ki = 0.020, kd = 0.010, min = 0.05, max = 1.0, initial = 0.70, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.equipment.*", function(payload) inst.lastSignal = payload end)
			end
