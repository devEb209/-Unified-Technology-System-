-- ARKHER SYSTEM H.0360 :: Trigger Volume Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0360"
	S.key = "arkher.physics.trigger.simulation_policy"
	S.name = "Trigger Volume Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.trigger.budget_governor" }
	S.tags = { "h", "trigger", "policy", "physics" }
	S.description = "Trigger Volume Simulation Policy: device tier rules for substeps, iterations and sleep for the Trigger Volume subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 480,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.trigger.simulation_policy", algorithm = "roundrobin", starvationGuard = 36 })
		inst.system = S
		inst.ctx = ctx
