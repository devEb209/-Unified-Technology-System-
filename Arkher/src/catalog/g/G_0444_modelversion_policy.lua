-- ARKHER SYSTEM G.0444 :: Model Versioning Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0444"
	S.key = "arkher.neural.modelversion.heuristic_policy"
	S.name = "Model Versioning Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.modelversion.result_cache" }
	S.tags = { "g", "modelversion", "policy", "neural" }
	S.description = "Model Versioning Heuristic Policy: rule fallback when the model is unavailable for the Model Versioning subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 520,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.modelversion.heuristic_policy", algorithm = "edf", starvationGuard = 49 })
		inst.system = S
		inst.ctx = ctx
