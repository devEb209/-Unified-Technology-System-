-- ARKHER SYSTEM L.0026 :: Region Fidelity Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0026"
	S.key = "arkher.sim.regionfidelity.fidelity_policy"
	S.name = "Region Fidelity Fidelity Policy"
	S.category = "L"
	S.family = "WORLD SIMULATION"
	S.area = "Region Fidelity"
	S.aspect = "Fidelity Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.regionfidelity.streaming" }
	S.tags = { "l", "regionfidelity", "policy", "sim" }
	S.description = "Region Fidelity Fidelity Policy: rules that pick full, cohort or statistical simulation for the Region Fidelity subsystem."
	S.params = {
		backlogLimit = 23,
		baseRadius = 280,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.regionfidelity.fidelity_policy", algorithm = "lottery", starvationGuard = 31 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.regionfidelity.*", function(payload) inst.lastSignal = payload end)
			end
