#!/usr/bin/env python3
"""GENESIS V1.47 SPECIALIZED 10j — +10 únicos 10/33 (106/332) sem parar"""
...
def patch47(roots):
    items=[
        ("ARKHER_Building","BUILDING — Modular  •  Facade",0x37474F,"Floor  •  Facade  •  Window","Floor Facade Window  •  HLOD"),
        ("ARKHER_City","CITY — Block  •  District",0x424242,"District  •  Block  •  Zone","District Block Zone  •  8k buildings"),
        ("ARKHER_Interior","INTERIOR — Room  •  Furniture",0x5D4037,"Room  •  Furniture  •  Light","Room Furniture Light  •  Probe"),
        ("ARKHER_Prop","PROP — Scatter  •  Physics",0x3E2723,"Scatter  •  Physics  •  LOD","Scatter Physics LOD  •  HLOD"),
        ("ARKHER_Veget","VEGET — Tree  •  Bush",0x1B5E20,"Tree  •  Bush  •  Grass","Tree Bush Grass  •  Wind"),
        ("ARKHER_Rock","ROCK — Boulder  •  Cliff",0x4E342E,"Boulder  •  Cliff  •  Erode","Boulder Cliff Erode  •  4k verts"),
        ("ARKHER_Sand","SAND — Dune  •  Footprint",0xFFD54F,"Dune  •  Print  •  Wind","Dune Print Wind  •  Displacement"),
        ("ARKHER_Ice","ICE — Frost  •  Crack",0xB2EBF2,"Frost  •  Crack  •  Refract","Frost Crack Refract  •  Translucent"),
        ("ARKHER_Lava","LAVA — Flow  •  Emissive",0xBF360C,"Flow  •  Cool  •  Glow","Flow Cool Glow  •  Emissive 5000"),
        ("ARKHER_Metal","METAL — Rust  •  Scratch",0x607D8B,"Rust  •  Scratch  •  Aniso","Rust Scratch Aniso  •  ClearCoat"),
    ]
...
for _,n in ipairs({"ARKHER_Building","ARKHER_City","ARKHER_Interior","ARKHER_Prop","ARKHER_Veget","ARKHER_Rock","ARKHER_Sand","ARKHER_Ice","ARKHER_Lava","ARKHER_Metal"}) do
...
print("[V1.47 10x] Building/City/Interior/Prop/Veget/Rock/Sand/Ice/Lava/Metal 106/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_47",{"Source":(T_STRING, FUNC47)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch46(roots)
    roots=patch47(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_47.rbxl")
    write(out, serialize(roots))
    print(f"V1.47 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_47.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.47 done")
