-- ARKHER SYSTEM VD.0340 :: Society Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0340"
	S.key = "arkher.contsim.societycont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.societycont.sparse_residency" }
	S.tags = { "vd", "societycont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.societycont.multi-scale_fidelity", levels = 5, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.societycont.*", function(payload) inst.lastSignal = payload end)
			end
