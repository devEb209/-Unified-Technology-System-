-- ARKHER SYSTEM VB.0484 :: Lighting Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0484"
	S.key = "arkher.conttime.lightcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.lightcont.sparse_residency" }
	S.tags = { "vb", "lightcont", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.lightcont.multi-scale_fidelity", levels = 5, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.lightcont.*", function(payload) inst.lastSignal = payload end)
			end
