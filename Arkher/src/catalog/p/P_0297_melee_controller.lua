-- ARKHER SYSTEM P.0297 :: Melee Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0297"
	S.key = "arkher.play.melee.difficulty_controller"
	S.name = "Melee Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Melee"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.melee.balance_analysis" }
	S.tags = { "p", "melee", "controller", "play" }
	S.description = "Melee Difficulty Controller: closed-loop tuning toward a target challenge for the Melee subsystem."
	S.params = {
		backlogLimit = 34,
		baseRadius = 400,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.melee.difficulty_controller", mode = "pid", target = 0.760, kp = 0.310, ki = 0.020, kd = 0.020, min = 0.05, max = 1.0, initial = 0.66, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.melee.*", function(payload) inst.lastSignal = payload end)
			end
