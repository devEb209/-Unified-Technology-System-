...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Cinema4D — ANIMATION Mixer #0217", size=Vector2.new(640, 460), pos=UDim2.fromOffset(121, 129), parent=parent, icon="⬡"})
    win.Root.Name="ARKHER_V1_0217"
    win.Root.Visible=false
    local content=win.Content
    content.BackgroundColor3=Theme.tokens.void
    -- accent line per UI
    local accent=Instance.new("Frame"); accent.Size=UDim2.new(1,0,0,2); accent.BackgroundColor3=Theme.categoryColor("Cinema4D"); accent.BorderSizePixel=0; accent.Parent=content
...
Components.Slider(right, "Rate", 0,3000,600, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_Rate"..217, v) end end)
