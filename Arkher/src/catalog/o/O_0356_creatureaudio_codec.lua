-- ARKHER SYSTEM O.0356 :: Creature Audio Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0356"
	S.key = "arkher.audio.creatureaudio.compression_codec"
	S.name = "Creature Audio Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Creature Audio"
	S.aspect = "Compression Codec"
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.creatureaudio.asset_registry" }
	S.tags = { "o", "creatureaudio", "codec", "audio" }
	S.description = "Creature Audio Compression Codec: payload encoding of audio state and events for the Creature Audio subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 200,
		baseWeight = 0.85,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.creatureaudio.compression_codec", format = "quantized", quantBits = 13 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.creatureaudio.*", function(payload) inst.lastSignal = payload end)
			end
