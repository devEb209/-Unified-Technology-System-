-- ARKHER SYSTEM L.0311 :: Resource Node Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0311"
	S.key = "arkher.sim.resourcenode.fidelity_policy"
	S.name = "Resource Node Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.resourcenode.streaming" }
	S.tags = { "l", "resourcenode", "policy", "sim" }
	S.description = "Resource Node Fidelity Policy: rules that pick full, cohort or statistical simulation for the Resource Node subsystem."
	S.params = {
		backlogLimit = 42,
		baseRadius = 240,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.resourcenode.fidelity_policy", algorithm = "wfq", starvationGuard = 18 })
		inst.system = S
		inst.ctx = ctx
