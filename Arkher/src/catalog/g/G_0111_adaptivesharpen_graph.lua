-- ARKHER SYSTEM G.0111 :: Adaptive Sharpening Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0111"
	S.key = "arkher.neural.adaptivesharpen.sampling_graph"
	S.name = "Adaptive Sharpening Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Adaptive Sharpening"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.adaptivesharpen.training_ledger" }
	S.tags = { "g", "adaptivesharpen", "graph", "neural" }
	S.description = "Adaptive Sharpening Sampling Graph: dependency graph of sampling stages for the Adaptive Sharpening subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 160,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.adaptivesharpen.sampling_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.adaptivesharpen.*", function(payload) inst.lastSignal = payload end)
			end
