-- ARKHER SYSTEM J.0084 :: Facial Rig Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0084"
	S.key = "arkher.character.facialrig.appearance_composer"
	S.name = "Facial Rig Appearance Composer"
	S.category = "J"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.facialrig.locomotion" }
	S.tags = { "j", "facialrig", "composer", "character" }
	S.description = "Facial Rig Appearance Composer: composition of appearance slots into a look for the Facial Rig subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 160,
		baseWeight = 0.66,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.facialrig.appearance_composer", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.facialrig.*", function(payload) inst.lastSignal = payload end)
			end
