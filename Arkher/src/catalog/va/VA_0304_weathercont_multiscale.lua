-- ARKHER SYSTEM VA.0304 :: Weather Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0304"
	S.key = "arkher.contworld.weathercont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.weathercont.sparse_residency" }
	S.tags = { "va", "weathercont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.weathercont.multi-scale_fidelity", levels = 4, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.weathercont.*", function(payload) inst.lastSignal = payload end)
			end
