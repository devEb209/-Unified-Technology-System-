-- ARKHER SYSTEM R.0231 :: Interest Management Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0231"
	S.key = "arkher.net.interestmanagement.prediction_and_reconciliation"
	S.name = "Interest Management Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Interest Management"
	S.aspect = "Prediction And Reconciliation"
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.interestmanagement.time_sync" }
	S.tags = { "r", "interestmanagement", "prediction", "net" }
	S.description = "Interest Management Prediction And Reconciliation: client simulation, correction and input replay for the Interest Management subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 320,
		baseWeight = 0.86,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.interestmanagement.prediction_and_reconciliation", errorThreshold = 0.060, smoothing = 14.0, maxInputs = 140 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.interestmanagement.*", function(payload) inst.lastSignal = payload end)
			end
