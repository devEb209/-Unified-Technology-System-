-- ARKHER SYSTEM W.0222 :: Transition Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0222"
	S.key = "arkher.cine.cinetransition.playback_orchestrator"
	S.name = "Transition Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Transition"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.cinetransition.cut_policy" }
	S.tags = { "w", "cinetransition", "orchestrator", "cine" }
	S.description = "Transition Playback Orchestrator: the state machine that drives a cinematic from start to end for the Transition subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 240,
		baseWeight = 0.74,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.cinetransition.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.cinetransition.*", function(payload) inst.lastSignal = payload end)
			end
