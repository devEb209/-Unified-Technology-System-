-- ARKHER SYSTEM G.0307 :: Model Registry Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0307"
	S.key = "arkher.neural.modelregistry.sampling_graph"
	S.name = "Model Registry Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Model Registry"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.modelregistry.training_ledger" }
	S.tags = { "g", "modelregistry", "graph", "neural" }
	S.description = "Model Registry Sampling Graph: dependency graph of sampling stages for the Model Registry subsystem."
	S.params = {
		backlogLimit = 9,
		baseRadius = 520,
		baseWeight = 0.81,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.modelregistry.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.modelregistry.*", function(payload) inst.lastSignal = payload end)
			end
