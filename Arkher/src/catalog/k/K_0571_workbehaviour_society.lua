-- ARKHER SYSTEM K.0571 :: Work Behaviour Social Relations
...
return function(A)
	...
	local S = {}
	S.id = "K.0571"
	S.key = "arkher.npc.workbehaviour.social_relations"
	S.name = "Work Behaviour Social Relations"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Work Behaviour"
	S.aspect = "Social Relations"
	S.kit = "society"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.workbehaviour.crowd_steering" }
	S.tags = { "k", "workbehaviour", "society", "npc" }
	S.description = "Work Behaviour Social Relations: affinity, factions, reputation and gossip for the Work Behaviour subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 440,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("society", { id = "arkher.npc.workbehaviour.social_relations", decayRate = 0.014, gossipReach = 5 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.workbehaviour.*", function(payload) inst.lastSignal = payload end)
			end
