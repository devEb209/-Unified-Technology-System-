-- ARKHER SYSTEM J.0293 :: Character LOD Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0293"
	S.key = "arkher.character.characterlod.appearance_composer"
	S.name = "Character LOD Appearance Composer"
	S.category = "J"
	S.family = "CHARACTERS / DIGITAL HUMANS"
	S.area = "Character LOD"
	S.aspect = "Appearance Composer"
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.characterlod.locomotion" }
	S.tags = { "j", "characterlod", "composer", "character" }
	S.description = "Character LOD Appearance Composer: composition of appearance slots into a look for the Character LOD subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 160,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.characterlod.appearance_composer", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.characterlod.*", function(payload) inst.lastSignal = payload end)
			end
