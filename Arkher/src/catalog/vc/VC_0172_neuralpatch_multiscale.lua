-- ARKHER SYSTEM VC.0172 :: Neural Patch Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0172"
	S.key = "arkher.contneural.neuralpatch.multi-scale_fidelity"
	S.name = "Neural Patch Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.neuralpatch.sparse_residency" }
	S.tags = { "vc", "neuralpatch", "multiscale", "contneural" }
	S.description = "Neural Patch Multi-Scale Fidelity: distance-driven multi-scale level selection for the Neural Patch subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 440,
		baseWeight = 0.57,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.neuralpatch.multi-scale_fidelity", levels = 5, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.neuralpatch.*", function(payload) inst.lastSignal = payload end)
			end
