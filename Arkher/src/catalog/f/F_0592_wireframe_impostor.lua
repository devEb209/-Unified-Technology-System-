-- ARKHER SYSTEM F.0592 :: Wireframe Overlay Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0592"
	S.key = "arkher.render.wireframe.geometry_virtualization"
	S.name = "Wireframe Overlay Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Wireframe Overlay"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.wireframe.visibility_set" }
	S.tags = { "f", "wireframe", "impostor", "render" }
	S.description = "Wireframe Overlay Geometry Virtualization: impostor and HLOD synthesis under a budget for the Wireframe Overlay subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 280,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.wireframe.geometry_virtualization", atlasSlots = 80, viewCount = 8, errorThreshold = 1.30 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.wireframe.*", function(payload) inst.lastSignal = payload end)
			end
