-- ARKHER SYSTEM H.0594 :: Wind Force Character Motion
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0594"
	S.key = "arkher.physics.wind.character_motion"
	S.name = "Wind Force Character Motion"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Wind Force"
	...
	S.kit = "charmotor"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.wind.query" }
	S.tags = { "h", "wind", "charmotor", "physics" }
	S.description = "Wind Force Character Motion: capsule move-and-slide with slopes, steps and jumps for the Wind Force subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.64,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("charmotor", { id = "arkher.physics.wind.character_motion", radius = 0.39, height = 1.84, maxSpeed = 8.40, jumpHeight = 2.00, stepOffset = 0.39 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.wind.*", function(payload) inst.lastSignal = payload end)
			end
