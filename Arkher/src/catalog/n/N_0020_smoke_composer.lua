-- ARKHER SYSTEM N.0020 :: Smoke Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0020"
	S.key = "arkher.vfx.smoke.effect_composition"
	S.name = "Smoke Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Smoke"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.smoke.effect_graph" }
	S.tags = { "n", "smoke", "composer", "vfx" }
	S.description = "Smoke Effect Composition: blending of several effect signals into one output for the Smoke subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.57,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.smoke.effect_composition", mode = "add" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.smoke.*", function(payload) inst.lastSignal = payload end)
			end
