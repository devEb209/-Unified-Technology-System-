-- ARKHER SYSTEM I.0500 :: Cloth Animation Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0500"
	S.key = "arkher.anim.clothanim.retarget_graph"
	S.name = "Cloth Animation Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Cloth Animation"
	S.aspect = "Retarget Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.clothanim.physical_blend" }
	S.tags = { "i", "clothanim", "graph", "anim" }
	S.description = "Cloth Animation Retarget Graph: bone mapping and retarget path resolution for the Cloth Animation subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 400,
		baseWeight = 0.74,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.clothanim.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.clothanim.*", function(payload) inst.lastSignal = payload end)
			end
