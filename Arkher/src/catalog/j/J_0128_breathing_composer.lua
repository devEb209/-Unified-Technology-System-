-- ARKHER SYSTEM J.0128 :: Breathing Appearance Composer
...
return function(A)
	...
	local S = {}
	S.id = "J.0128"
	S.key = "arkher.character.breathing.appearance_composer"
	S.name = "Breathing Appearance Composer"
	S.category = "J"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.character.breathing.locomotion" }
	S.tags = { "j", "breathing", "composer", "character" }
	S.description = "Breathing Appearance Composer: composition of appearance slots into a look for the Breathing subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 560,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.character.breathing.appearance_composer", mode = "max" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.character.breathing.*", function(payload) inst.lastSignal = payload end)
			end
