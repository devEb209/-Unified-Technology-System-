-- ARKHER SYSTEM V.0077 :: Animation Clip Dependency Graph
...
-- Kit: graph (what references what, and in which order it must be built)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "V.0077"
	S.key = "arkher.asset.animationclip.dependency_graph"
	S.name = "Animation Clip Dependency Graph"
	S.category = "V"
	S.family = "ASSET PIPELINE"
	S.area = "Animation Clip"
	S.aspect = "Dependency Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.animationclip.asset_registry" }
	S.tags = { "v", "animationclip", "graph", "asset" }
	S.description = "Animation Clip Dependency Graph: what references what, and in which order it must be built for the Animation Clip subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 600,
		baseWeight = 0.55,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.animationclip.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.animationclip.*", function(payload) inst.lastSignal = payload end)
			end
