-- ARKHER SYSTEM VA.0256 :: Road Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0256"
	S.key = "arkher.contworld.roadcont.multi-scale_fidelity"
	S.name = "Road Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.roadcont.sparse_residency" }
	S.tags = { "va", "roadcont", "multiscale", "contworld" }
	S.description = "Road Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Road Continuum subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 600,
		baseWeight = 0.87,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.roadcont.multi-scale_fidelity", levels = 6, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.roadcont.*", function(payload) inst.lastSignal = payload end)
			end
