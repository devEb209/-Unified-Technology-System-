-- ARKHER SYSTEM H.0528 :: Breakable Joint Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0528"
	S.key = "arkher.physics.breakjoint.simulation_policy"
	S.name = "Breakable Joint Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Breakable Joint"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.breakjoint.budget_governor" }
	S.tags = { "h", "breakjoint", "policy", "physics" }
	S.description = "Breakable Joint Simulation Policy: device tier rules for substeps, iterations and sleep for the Breakable Joint subsystem."
	S.params = {
		backlogLimit = 47,
		baseRadius = 280,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.breakjoint.simulation_policy", algorithm = "lottery", starvationGuard = 55 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.breakjoint.*", function(payload) inst.lastSignal = payload end)
			end
