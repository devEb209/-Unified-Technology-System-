-- ARKHER SYSTEM T.0400 :: Performance Reasoning Outcome Prediction
...
return function(A)
	...
	local S = {}
	S.id = "T.0400"
	S.key = "arkher.ai.performancereasoning.outcome_prediction"
	S.name = "Performance Reasoning Outcome Prediction"
	S.category = "T"
	S.family = "SINGULARITY AI"
	S.area = "Performance Reasoning"
	S.aspect = "Outcome Prediction"
	S.kit = "predictor"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.performancereasoning.decision_policy" }
	S.tags = { "t", "performancereasoning", "predictor", "ai" }
	S.description = "Performance Reasoning Outcome Prediction: forecasting the result before the work is done for the Performance Reasoning subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 240,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("predictor", { id = "arkher.ai.performancereasoning.outcome_prediction", method = "markov", alpha = 0.20, window = 82 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.trustworthy() return inst.confidence() >= S.params.minConfidence end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ai.performancereasoning.*", function(payload) inst.lastSignal = payload end)
			end
