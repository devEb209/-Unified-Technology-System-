-- ARKHER SYSTEM G.0395 :: Fallback Heuristic Temporal History
...
return function(A)
	...
	local S = {}
	S.id = "G.0395"
	S.key = "arkher.neural.fallbackheuristic.temporal_history"
	S.name = "Fallback Heuristic Temporal History"
	S.category = "G"
	...
	S.kit = "temporal"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.fallbackheuristic.feature_extraction" }
	S.tags = { "g", "fallbackheuristic", "temporal", "neural" }
	S.description = "Fallback Heuristic Temporal History: history buffers reused across frames for the Fallback Heuristic subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 280,
		baseWeight = 0.53,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("temporal", { id = "arkher.neural.fallbackheuristic.temporal_history", feedback = 0.83, phase = 8 })
		inst.system = S
		inst.ctx = ctx
