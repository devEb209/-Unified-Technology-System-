...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Fairlight — CORE Stack #0120", size=Vector2.new(600, 420), pos=UDim2.fromOffset(460, 300), parent=parent, icon="⬔"})
...
Components.Slider(props, "Strength", 0,1,0.5, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_S"..120, v) end end)
Components.Checkbox(props, "Enabled", true, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_E"..120, v) end end)
