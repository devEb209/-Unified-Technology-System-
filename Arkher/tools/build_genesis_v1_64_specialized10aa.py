#!/usr/bin/env python3
"""GENESIS V1.64 SPECIALIZED 10aa — +10 únicos 27/33 (276/332) sem parar"""
...
def patch64(roots):
    items=[
        ("ARKHER_Affiliate","AFFILIATE — Link  •  Commission",0x33691E,"Link  •  Commission  •  Payout","Link Commission Payout  •  5%"),
        ("ARKHER_Creator","CREATOR — Code  •  Earnings",0x1B5E20,"Code  •  Earnings  •  Tier","Code Earnings Tier  •  Creator Code"),
        ("ARKHER_Support","SUPPORT — Ticket  •  Priority",0x01579B,"Ticket  •  Priority  •  SLA","Ticket Priority SLA  •  24h"),
        ("ARKHER_FAQ","FAQ — Search  •  Category",0x0288D1,"Search  •  Category  •  Helpful","Search Category Helpful  •  Was this helpful?"),
        ("ARKHER_Contact","CONTACT — Form  •  Topic",0x00838F,"Form  •  Topic  •  Attach","Form Topic Attach  •  Send"),
        ("ARKHER_Status","STATUS — Uptime  •  Incident",0x2E7D32,"Uptime  •  Incident  •  History","Uptime 99.9% Incident History"),
        ("ARKHER_Health","HEALTH — Service  •  Region",0x37474F,"Service  •  Region  •  Latency","Service Region Latency  •  20ms"),
        ("ARKHER_Network2","NETWORK2 — Ping  •  Loss",0x311B92,"Ping  •  Loss  •  Jitter","Ping Loss Jitter  •  0% loss"),
        ("ARKHER_Security","SECURITY — 2FA  •  Session",0xB71C1C,"2FA  •  Session  •  Device","2FA Session Device  •  Authenticator"),
        ("ARKHER_Privacy2","PRIVACY2 — Export  •  Delete",0x4A148C,"Export  •  Delete  •  Consent","Export Delete Consent  •  GDPR"),
    ]
...
for _,n in ipairs({"ARKHER_Affiliate","ARKHER_Creator","ARKHER_Support","ARKHER_FAQ","ARKHER_Contact","ARKHER_Status","ARKHER_Health","ARKHER_Network2","ARKHER_Security","ARKHER_Privacy2"}) do
...
print("[V1.64 10x] Affiliate/Creator/Support/FAQ/Contact/Status/Health/Network2/Security/Privacy2 276/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_64",{"Source":(T_STRING, FUNC64)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch63(roots)
    roots=patch64(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_64.rbxl")
    write(out, serialize(roots))
    print(f"V1.64 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_64.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.64 done")
