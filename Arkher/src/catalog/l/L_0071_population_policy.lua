-- ARKHER SYSTEM L.0071 :: Population Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0071"
	S.key = "arkher.sim.population.fidelity_policy"
	S.name = "Population Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.population.streaming" }
	S.tags = { "l", "population", "policy", "sim" }
	S.description = "Population Fidelity Policy: rules that pick full, cohort or statistical simulation for the Population subsystem."
	S.params = {
		backlogLimit = 40,
		baseRadius = 480,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.population.fidelity_policy", algorithm = "roundrobin", starvationGuard = 24 })
		inst.system = S
		inst.ctx = ctx
