-- ARKHER SYSTEM VA.0052 :: Epoch Frame Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0052"
	S.key = "arkher.contworld.epochframe.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.epochframe.sparse_residency" }
	S.tags = { "va", "epochframe", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.epochframe.multi-scale_fidelity", levels = 5, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.epochframe.*", function(payload) inst.lastSignal = payload end)
			end
