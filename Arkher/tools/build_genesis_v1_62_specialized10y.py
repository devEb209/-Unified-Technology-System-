#!/usr/bin/env python3
"""GENESIS V1.62 SPECIALIZED 10y — +10 únicos 25/33 (256/332) sem parar"""
...
def patch62(roots):
    items=[
        ("ARKHER_Settings2","SETTINGS2 — Graphics  •  Audio",0x37474F,"Graphics  •  Audio  •  Controls","Graphics Audio Controls  •  Preset Epic"),
        ("ARKHER_Keybind","KEYBIND — Map  •  Conflict",0x01579B,"Map  •  Conflict  •  Default","Map Conflict Default  •  WASD"),
        ("ARKHER_Graphics2","GRAPHICS2 — Quality  •  Scalability",0x0288D1,"Quality  •  Scalability  •  Auto","Quality Scalability Auto  •  Epic"),
        ("ARKHER_Audio2","AUDIO2 — Volume  •  Bus",0x00838F,"Volume  •  Bus  •  HRTF","Volume Bus HRTF  •  Master 100%"),
        ("ARKHER_Controls2","CONTROLS2 — Sensitivity  •  Invert",0x1B5E20,"Sensitivity  •  Invert  •  Deadzone","Sensitivity Invert Deadzone  •  1.0"),
        ("ARKHER_Access2","ACCESS2 — Subtitle  •  ColorBlind",0x00695C,"Subtitle  •  ColorBlind  •  Remap","Subtitle ColorBlind Remap  •  Deuter"),
        ("ARKHER_Language","LANGUAGE — Locale  •  Voice",0x33691E,"Locale  •  Voice  •  Subtitle","Locale Voice Subtitle  •  PT-BR"),
        ("ARKHER_Region","REGION — Server  •  Ping",0x0277BD,"Server  •  Ping  •  Auto","Server Ping Auto  •  SA 20ms"),
        ("ARKHER_Privacy","PRIVACY — Data  •  Share",0x4A148C,"Data  •  Share  •  Delete","Data Share Delete  •  GDPR"),
        ("ARKHER_Terms","TERMS — EULA  •  Accept",0x5D4037,"EULA  •  Accept  •  Version","EULA Accept Version  •  v1.62"),
    ]
...
for _,n in ipairs({"ARKHER_Settings2","ARKHER_Keybind","ARKHER_Graphics2","ARKHER_Audio2","ARKHER_Controls2","ARKHER_Access2","ARKHER_Language","ARKHER_Region","ARKHER_Privacy","ARKHER_Terms"}) do
...
print("[V1.62 10x] Settings2/Keybind/Graphics2/Audio2/Controls2/Access2/Language/Region/Privacy/Terms 256/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_62",{"Source":(T_STRING, FUNC62)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch61(roots)
    roots=patch62(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_62.rbxl")
    write(out, serialize(roots))
    print(f"V1.62 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_62.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.62 done")
