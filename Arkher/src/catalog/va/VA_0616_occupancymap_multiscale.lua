-- ARKHER SYSTEM VA.0616 :: Occupancy Map Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0616"
	S.key = "arkher.contworld.occupancymap.multi-scale_fidelity"
	S.name = "Occupancy Map Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.occupancymap.sparse_residency" }
	S.tags = { "va", "occupancymap", "multiscale", "contworld" }
	S.description = "Occupancy Map Multi-Scale Fidelity: distance-driven multi-scale level selection for the Occupancy Map subsystem."
	S.params = {
		backlogLimit = 29,
		baseRadius = 360,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.occupancymap.multi-scale_fidelity", levels = 6, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.occupancymap.*", function(payload) inst.lastSignal = payload end)
			end
