-- ARKHER SYSTEM VD.0216 :: Biome Continuum Emergence Continuum
...
return function(A)
	...
	local S = {}
	S.id = "VD.0216"
	S.key = "arkher.contsim.biomecont.emergence_continuum"
	S.name = "Biome Continuum Emergence Continuum"
	S.category = "VD"
	...
	S.kit = "emergence"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.biomecont.reality_continuum" }
	S.tags = { "vd", "biomecont", "emergence", "contsim" }
	S.description = "Biome Continuum Emergence Continuum: emergence detection over the continuum for the Biome Continuum subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 560,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("emergence", { id = "arkher.contsim.biomecont.emergence_continuum", windowSize = 6, threshold = 1.50, minCount = 2 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.biomecont.*", function(payload) inst.lastSignal = payload end)
			end
