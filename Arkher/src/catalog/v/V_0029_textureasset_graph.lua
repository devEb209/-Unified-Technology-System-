-- ARKHER SYSTEM V.0029 :: Texture Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0029"
	S.key = "arkher.asset.textureasset.dependency_graph"
	S.name = "Texture Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.textureasset.asset_registry" }
	S.tags = { "v", "textureasset", "graph", "asset" }
	S.description = "Texture Dependency Graph: what references what, and in which order it must be built for the Texture subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 200,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.textureasset.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.textureasset.*", function(payload) inst.lastSignal = payload end)
			end
