-- ARKHER SYSTEM P.0381 :: Checkpoint Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0381"
	S.key = "arkher.play.checkpoint.difficulty_controller"
	S.name = "Checkpoint Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Checkpoint"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.checkpoint.balance_analysis" }
	S.tags = { "p", "checkpoint", "controller", "play" }
	S.description = "Checkpoint Difficulty Controller: closed-loop tuning toward a target challenge for the Checkpoint subsystem."
	S.params = {
		backlogLimit = 8,
		baseRadius = 320,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.checkpoint.difficulty_controller", mode = "pid", target = 0.500, kp = 0.350, ki = 0.060, kd = 0.010, min = 0.05, max = 1.0, initial = 0.40, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.checkpoint.*", function(payload) inst.lastSignal = payload end)
			end
