-- ARKHER SYSTEM O.0416 :: Audio Debug Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0416"
	S.key = "arkher.audio.audiodebug.compression_codec"
	S.name = "Audio Debug Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Audio Debug"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.audiodebug.asset_registry" }
	S.tags = { "o", "audiodebug", "codec", "audio" }
	S.description = "Audio Debug Compression Codec: payload encoding of audio state and events for the Audio Debug subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.94,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.audiodebug.compression_codec", format = "binary", quantBits = 8 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.audiodebug.*", function(payload) inst.lastSignal = payload end)
			end
