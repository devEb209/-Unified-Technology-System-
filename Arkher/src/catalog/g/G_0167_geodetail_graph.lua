-- ARKHER SYSTEM G.0167 :: Geometry Detail Prediction Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0167"
	S.key = "arkher.neural.geodetail.sampling_graph"
	S.name = "Geometry Detail Prediction Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Geometry Detail Prediction"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.geodetail.training_ledger" }
	S.tags = { "g", "geodetail", "graph", "neural" }
	S.description = "Geometry Detail Prediction Sampling Graph: dependency graph of sampling stages for the Geometry Detail Prediction subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 200,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.geodetail.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.geodetail.*", function(payload) inst.lastSignal = payload end)
			end
