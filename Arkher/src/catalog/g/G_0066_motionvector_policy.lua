-- ARKHER SYSTEM G.0066 :: Motion Vector Estimation Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0066"
	S.key = "arkher.neural.motionvector.heuristic_policy"
	S.name = "Motion Vector Estimation Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.motionvector.result_cache" }
	S.tags = { "g", "motionvector", "policy", "neural" }
	S.description = "Motion Vector Estimation Heuristic Policy: rule fallback when the model is unavailable for the Motion Vector Estimation subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.68,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.motionvector.heuristic_policy", algorithm = "roundrobin", starvationGuard = 56 })
		inst.system = S
		inst.ctx = ctx
