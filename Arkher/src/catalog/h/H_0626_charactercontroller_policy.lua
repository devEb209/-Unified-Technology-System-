-- ARKHER SYSTEM H.0626 :: Character Controller Simulation Policy
-- Category H - PHYSICS
-- ARKHER Physics Abstraction capability: mass, contact, constraint and motion, deterministic and budgeted.
-- Kit: policy (device tier rules for substeps, iterations and sleep)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "H.0626"
	S.key = "arkher.physics.charactercontroller.simulation_policy"
	S.name = "Character Controller Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Character Controller"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.charactercontroller.budget_governor" }
	S.tags = { "h", "charactercontroller", "policy", "physics" }
	S.description = "Character Controller Simulation Policy: device tier rules for substeps, iterations and sleep for the Character Controller subsystem."
	S.params = {
		backlogLimit = 47,
		baseRadius = 440,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.charactercontroller.simulation_policy", algorithm = "lottery", starvationGuard = 47 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.charactercontroller.*", function(payload) inst.lastSignal = payload end)
			end
