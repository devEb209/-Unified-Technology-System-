#!/usr/bin/env python3
"""GENESIS V1.53 SPECIALIZED 10p — +10 únicos 16/33 (166/332) sem parar"""
...
def patch53(roots):
    items=[
        ("ARKHER_Mount","MOUNT — Ride  •  Saddle",0x5D4037,"Ride  •  Saddle  •  Anim","Ride Saddle Anim  •  Gallop"),
        ("ARKHER_Weapon","WEAPON — Mesh  •  Socket",0xB71C1C,"Mesh  •  Socket  •  Trail","Mesh Socket Trail  •  Muzzle"),
        ("ARKHER_Armor","ARMOR — Slot  •  Set",0x37474F,"Slot  •  Set  •  Bonus","Slot Set Bonus  •  6 pieces"),
        ("ARKHER_Skill","SKILL — Tree  •  Node",0x1B5E20,"Tree  •  Node  •  Cost","Tree Node Cost  •  50 points"),
        ("ARKHER_Talent","TALENT — Active  •  Passive",0x2E7D32,"Active  •  Passive  •  Cooldown","Active Passive Cooldown  •  30s"),
        ("ARKHER_Spell","SPELL — Cast  •  Mana",0x4A148C,"Cast  •  Mana  •  Channel","Cast Mana Channel  •  2s"),
        ("ARKHER_Buff","BUFF — Stack  •  Duration",0x00838F,"Stack  •  Duration  •  Icon","Stack Duration Icon  •  10s"),
        ("ARKHER_Debuff","DEBUFF — Stack  •  Tick",0xAD1457,"Stack  •  Tick  •  Cleanse","Stack Tick Cleanse  •  Poison"),
        ("ARKHER_Cooldown","COOLDOWN — Timer  •  GCD",0xFF6F00,"Timer  •  GCD  •  Ready","Timer GCD Ready  •  0.5s"),
        ("ARKHER_Combo2","COMBO2 — Chain  •  Finisher",0xB71C1C,"Chain  •  Finisher  •  Damage","Chain Finisher Damage  •  500%"),
    ]
...
for _,n in ipairs({"ARKHER_Mount","ARKHER_Weapon","ARKHER_Armor","ARKHER_Skill","ARKHER_Talent","ARKHER_Spell","ARKHER_Buff","ARKHER_Debuff","ARKHER_Cooldown","ARKHER_Combo2"}) do
...
print("[V1.53 10x] Mount/Weapon/Armor/Skill/Talent/Spell/Buff/Debuff/Cooldown/Combo2 166/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_53",{"Source":(T_STRING, FUNC53)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch52(roots)
    roots=patch53(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_53.rbxl")
    write(out, serialize(roots))
    print(f"V1.53 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_53.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.53 done")
