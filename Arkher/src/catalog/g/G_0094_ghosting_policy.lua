-- ARKHER SYSTEM G.0094 :: Ghosting Suppression Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0094"
	S.key = "arkher.neural.ghosting.heuristic_policy"
	S.name = "Ghosting Suppression Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.ghosting.result_cache" }
	S.tags = { "g", "ghosting", "policy", "neural" }
	S.description = "Ghosting Suppression Heuristic Policy: rule fallback when the model is unavailable for the Ghosting Suppression subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 480,
		baseWeight = 0.88,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.ghosting.heuristic_policy", algorithm = "roundrobin", starvationGuard = 60 })
		inst.system = S
		inst.ctx = ctx
