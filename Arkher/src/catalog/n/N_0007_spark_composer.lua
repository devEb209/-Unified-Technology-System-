-- ARKHER SYSTEM N.0007 :: Spark Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0007"
	S.key = "arkher.vfx.spark.effect_composition"
	S.name = "Spark Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Spark"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.spark.effect_graph" }
	S.tags = { "n", "spark", "composer", "vfx" }
	S.description = "Spark Effect Composition: blending of several effect signals into one output for the Spark subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 280,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.spark.effect_composition", mode = "overlay" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.spark.*", function(payload) inst.lastSignal = payload end)
			end
