-- ARKHER SYSTEM VC.0388 :: Memory Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0388"
	S.key = "arkher.contneural.memorycont.multi-scale_fidelity"
	S.name = "Memory Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.memorycont.sparse_residency" }
	S.tags = { "vc", "memorycont", "multiscale", "contneural" }
	S.description = "Memory Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Memory Continuum subsystem."
	S.params = {
		backlogLimit = 39,
		baseRadius = 600,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.memorycont.multi-scale_fidelity", levels = 6, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.memorycont.*", function(payload) inst.lastSignal = payload end)
			end
