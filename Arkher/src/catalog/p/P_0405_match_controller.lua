-- ARKHER SYSTEM P.0405 :: Match Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0405"
	S.key = "arkher.play.match.difficulty_controller"
	S.name = "Match Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Match"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.match.balance_analysis" }
	S.tags = { "p", "match", "controller", "play" }
	S.description = "Match Difficulty Controller: closed-loop tuning toward a target challenge for the Match subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 160,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.match.difficulty_controller", mode = "pid", target = 0.700, kp = 0.250, ki = 0.080, kd = 0.010, min = 0.05, max = 1.0, initial = 0.60, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.match.*", function(payload) inst.lastSignal = payload end)
			end
