-- ARKHER SYSTEM VB.0096 :: Residency Window Emergence Continuum
...
return function(A)
	...
	local S = {}
	S.id = "VB.0096"
	S.key = "arkher.conttime.residencywindow.emergence_continuum"
	...
	S.kit = "emergence"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.residencywindow.reality_continuum" }
	S.tags = { "vb", "residencywindow", "emergence", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("emergence", { id = "arkher.conttime.residencywindow.emergence_continuum", windowSize = 4, threshold = 1.40, minCount = 2 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.residencywindow.*", function(payload) inst.lastSignal = payload end)
			end
