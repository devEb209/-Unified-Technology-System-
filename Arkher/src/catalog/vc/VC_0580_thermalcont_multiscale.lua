-- ARKHER SYSTEM VC.0580 :: Thermal Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0580"
	S.key = "arkher.contneural.thermalcont.multi-scale_fidelity"
	S.name = "Thermal Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.thermalcont.sparse_residency" }
	S.tags = { "vc", "thermalcont", "multiscale", "contneural" }
	S.description = "Thermal Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Thermal Continuum subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 320,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.thermalcont.multi-scale_fidelity", levels = 5, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.thermalcont.*", function(payload) inst.lastSignal = payload end)
			end
