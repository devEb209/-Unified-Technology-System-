-- ARKHER SYSTEM T.0009 :: Intent Parsing Working Memory
...
return function(A)
	...
	local S = {}
	S.id = "T.0009"
	S.key = "arkher.ai.intentparsing.working_memory"
	S.name = "Intent Parsing Working Memory"
	S.category = "T"
	...
	S.kit = "memory"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.intentparsing.outcome_prediction" }
	S.tags = { "t", "intentparsing", "memory", "ai" }
	S.description = "Intent Parsing Working Memory: decaying, consolidating recall of what the agent learned for the Intent Parsing subsystem."
	S.params = {
		backlogLimit = 38,
		baseRadius = 560,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("memory", { id = "arkher.ai.intentparsing.working_memory", capacity = 78, decayRate = 0.020, consolidateAt = 4 })
		inst.system = S
		inst.ctx = ctx
