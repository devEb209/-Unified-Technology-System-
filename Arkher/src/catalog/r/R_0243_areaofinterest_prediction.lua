-- ARKHER SYSTEM R.0243 :: Area Of Interest Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0243"
	S.key = "arkher.net.areaofinterest.prediction_and_reconciliation"
	S.name = "Area Of Interest Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Area Of Interest"
	S.aspect = "Prediction And Reconciliation"
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.areaofinterest.time_sync" }
	S.tags = { "r", "areaofinterest", "prediction", "net" }
	S.description = "Area Of Interest Prediction And Reconciliation: client simulation, correction and input replay for the Area Of Interest subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.areaofinterest.prediction_and_reconciliation", errorThreshold = 0.065, smoothing = 15.0, maxInputs = 120 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.areaofinterest.*", function(payload) inst.lastSignal = payload end)
			end
