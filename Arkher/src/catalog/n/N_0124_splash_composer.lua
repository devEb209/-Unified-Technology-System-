-- ARKHER SYSTEM N.0124 :: Water Splash Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0124"
	S.key = "arkher.vfx.splash.effect_composition"
	S.name = "Water Splash Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Water Splash"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.splash.effect_graph" }
	S.tags = { "n", "splash", "composer", "vfx" }
	S.description = "Water Splash Effect Composition: blending of several effect signals into one output for the Water Splash subsystem."
	S.params = {
		backlogLimit = 23,
		baseRadius = 600,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.splash.effect_composition", mode = "overlay" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.splash.*", function(payload) inst.lastSignal = payload end)
			end
