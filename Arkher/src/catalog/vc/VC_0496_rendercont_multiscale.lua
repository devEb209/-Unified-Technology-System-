-- ARKHER SYSTEM VC.0496 :: Rendering Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0496"
	S.key = "arkher.contneural.rendercont.multi-scale_fidelity"
	S.name = "Rendering Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.rendercont.sparse_residency" }
	S.tags = { "vc", "rendercont", "multiscale", "contneural" }
	S.description = "Rendering Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Rendering Continuum subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 480,
		baseWeight = 0.6,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.rendercont.multi-scale_fidelity", levels = 6, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.rendercont.*", function(payload) inst.lastSignal = payload end)
			end
