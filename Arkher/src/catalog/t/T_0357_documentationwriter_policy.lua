-- ARKHER SYSTEM T.0357 :: Documentation Writer Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0357"
	S.key = "arkher.ai.documentationwriter.decision_policy"
	S.name = "Documentation Writer Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.documentationwriter.reasoning_graph" }
	S.tags = { "t", "documentationwriter", "policy", "ai" }
	S.description = "Documentation Writer Decision Policy: ordered rules deciding what the agent does next for the Documentation Writer subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.78,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.documentationwriter.decision_policy", algorithm = "roundrobin", starvationGuard = 32 })
		inst.system = S
		inst.ctx = ctx
