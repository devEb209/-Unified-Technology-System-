--!strict
-- NASA SRTM — ANIMATION Mixer #0307
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="NASA SRTM — ANIMATION Mixer #0307", size=Vector2.new(640, 460), pos=UDim2.fromOffset(491, 99), parent=parent, icon="≡"})
...
Components.Slider(right, "Rate", 0,3000,600, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_Rate"..307, v) end end)
