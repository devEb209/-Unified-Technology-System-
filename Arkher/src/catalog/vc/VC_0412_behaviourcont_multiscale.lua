-- ARKHER SYSTEM VC.0412 :: Behaviour Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0412"
	S.key = "arkher.contneural.behaviourcont.multi-scale_fidelity"
	S.name = "Behaviour Continuum Multi-Scale Fidelity"
	S.category = "VC"
	S.family = "CONTINUUM — NEURAL"
	S.area = "Behaviour Continuum"
	S.aspect = "Multi-Scale Fidelity"
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.behaviourcont.sparse_residency" }
	S.tags = { "vc", "behaviourcont", "multiscale", "contneural" }
	S.description = "Behaviour Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Behaviour Continuum subsystem."
	S.params = {
		backlogLimit = 9,
		baseRadius = 200,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.behaviourcont.multi-scale_fidelity", levels = 5, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.behaviourcont.*", function(payload) inst.lastSignal = payload end)
			end
