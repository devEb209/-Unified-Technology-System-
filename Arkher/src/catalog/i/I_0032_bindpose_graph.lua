-- ARKHER SYSTEM I.0032 :: Bind Pose Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0032"
	S.key = "arkher.anim.bindpose.retarget_graph"
	S.name = "Bind Pose Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Bind Pose"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.bindpose.physical_blend" }
	S.tags = { "i", "bindpose", "graph", "anim" }
	S.description = "Bind Pose Retarget Graph: bone mapping and retarget path resolution for the Bind Pose subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 480,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.bindpose.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.bindpose.*", function(payload) inst.lastSignal = payload end)
			end
