-- ARKHER SYSTEM Q.0405 :: UI Debug Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0405"
	S.key = "arkher.ui.uidebug.accessibility_policy"
	S.name = "UI Debug Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "UI Debug"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.uidebug.interaction_ledger" }
	S.tags = { "q", "uidebug", "policy", "ui" }
	S.description = "UI Debug Accessibility Policy: rules enforcing contrast, target size and motion limits for the UI Debug subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 320,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.uidebug.accessibility_policy", algorithm = "roundrobin", starvationGuard = 32 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.uidebug.*", function(payload) inst.lastSignal = payload end)
			end
