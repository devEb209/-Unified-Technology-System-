-- ARKHER SYSTEM K.0251 :: Temperament Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0251"
	S.key = "arkher.npc.temperament.thinking_budget"
	S.name = "Temperament Thinking Budget"
	S.category = "K"
	...
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.temperament.daily_schedule" }
	S.tags = { "k", "temperament", "budgeter", "npc" }
	S.description = "Temperament Thinking Budget: allocation of per-frame thinking time across NPCs for the Temperament subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 320,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.temperament.thinking_budget", total = 456, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.temperament.*", function(payload) inst.lastSignal = payload end)
			end
