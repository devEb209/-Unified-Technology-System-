#!/usr/bin/env python3
"""GENESIS V1.51 SPECIALIZED 10n — +10 únicos 14/33 (146/332) sem parar"""
...
def patch51(roots):
    items=[
        ("ARKHER_LiveOps","LIVEOPS — Event  •  Schedule",0xFF6F00,"Event  •  Schedule  •  Push","Event Schedule Push  •  Calendar"),
        ("ARKHER_Season","SEASON — Pass  •  Reward",0x7C4DFF,"Tier  •  Reward  •  Expire","Tier Reward Expire  •  90 days"),
        ("ARKHER_Battle","BATTLEPASS — Track  •  Premium",0x651FFF,"Track  •  Premium  •  Claim","Track Premium Claim  •  100 tiers"),
        ("ARKHER_Event","EVENT — Limited  •  Daily",0xFFD600,"Limited  •  Daily  •  Quest","Limited Daily Quest  •  Timer"),
        ("ARKHER_Daily","DAILY — Streak  •  Bonus",0x33691E,"Streak  •  Bonus  •  Reset","Streak Bonus Reset  •  7 days"),
        ("ARKHER_Reward","REWARD — Chest  •  Drop",0xFF6F00,"Chest  •  Drop  •  Rarity","Chest Drop Rarity  •  Loot"),
        ("ARKHER_Economy","ECONOMY — Sink  •  Faucet",0x1B5E20,"Sink  •  Faucet  •  Curve","Sink Faucet Curve  •  Inflation"),
        ("ARKHER_Balance","BALANCE — Stat  •  Meta",0x01579B,"Stat  •  Meta  •  Sim","Stat Meta Sim  •  10k sims"),
        ("ARKHER_Pricing","PRICING — Price  •  Test",0x00838F,"Price  •  Test  •  Elasticity","Price Test Elasticity  •  $0.99"),
        ("ARKHER_AB","AB TEST — Variant  •  Metric",0x37474F,"Variant  •  Metric  •  Signif","Variant Metric Signif  •  p<0.05"),
    ]
...
for _,n in ipairs({"ARKHER_LiveOps","ARKHER_Season","ARKHER_Battle","ARKHER_Event","ARKHER_Daily","ARKHER_Reward","ARKHER_Economy","ARKHER_Balance","ARKHER_Pricing","ARKHER_AB"}) do
...
print("[V1.51 10x] LiveOps/Season/Battle/Event/Daily/Reward/Economy/Balance/Pricing/AB 146/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_51",{"Source":(T_STRING, FUNC51)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch50(roots)
    roots=patch51(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_51.rbxl")
    write(out, serialize(roots))
    print(f"V1.51 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_51.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.51 done")
