-- ARKHER SYSTEM L.0081 :: Cohort Region Graph
...
return function(A)
	...
	local S = {}
	S.id = "L.0081"
	S.key = "arkher.sim.cohort.region_graph"
	S.name = "Cohort Region Graph"
	S.category = "L"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.cohort.calendar" }
	S.tags = { "l", "cohort", "graph", "sim" }
	S.description = "Cohort Region Graph: connectivity, components and shortest path between places for the Cohort subsystem."
	S.params = {
		backlogLimit = 14,
		baseRadius = 400,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.sim.cohort.region_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
