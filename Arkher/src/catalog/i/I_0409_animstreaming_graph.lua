-- ARKHER SYSTEM I.0409 :: Animation Streaming Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0409"
	S.key = "arkher.anim.animstreaming.retarget_graph"
	S.name = "Animation Streaming Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Animation Streaming"
	S.aspect = "Retarget Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.animstreaming.physical_blend" }
	S.tags = { "i", "animstreaming", "graph", "anim" }
	S.description = "Animation Streaming Retarget Graph: bone mapping and retarget path resolution for the Animation Streaming subsystem."
	S.params = {
		backlogLimit = 39,
		baseRadius = 280,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.animstreaming.retarget_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.animstreaming.*", function(payload) inst.lastSignal = payload end)
			end
