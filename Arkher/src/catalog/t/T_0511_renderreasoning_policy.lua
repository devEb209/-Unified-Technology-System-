-- ARKHER SYSTEM T.0511 :: Render Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0511"
	S.key = "arkher.ai.renderreasoning.decision_policy"
	S.name = "Render Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.renderreasoning.reasoning_graph" }
	S.tags = { "t", "renderreasoning", "policy", "ai" }
	S.description = "Render Reasoning Decision Policy: ordered rules deciding what the agent does next for the Render Reasoning subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 320,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.renderreasoning.decision_policy", algorithm = "roundrobin", starvationGuard = 44 })
		inst.system = S
		inst.ctx = ctx
