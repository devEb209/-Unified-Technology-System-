-- ARKHER SYSTEM K.0377 :: Utility Selection Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0377"
	S.key = "arkher.npc.utilityselection.thinking_budget"
	S.name = "Utility Selection Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Utility Selection"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.utilityselection.daily_schedule" }
	S.tags = { "k", "utilityselection", "budgeter", "npc" }
	S.description = "Utility Selection Thinking Budget: allocation of per-frame thinking time across NPCs for the Utility Selection subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 560,
		baseWeight = 0.78,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.utilityselection.thinking_budget", total = 238, strategy = "priority" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.utilityselection.*", function(payload) inst.lastSignal = payload end)
			end
