-- ARKHER SYSTEM I.0279 :: Hand Placement IK Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0279"
	S.key = "arkher.anim.handik.retarget_graph"
	S.name = "Hand Placement IK Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Hand Placement IK"
	S.aspect = "Retarget Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.handik.physical_blend" }
	S.tags = { "i", "handik", "graph", "anim" }
	S.description = "Hand Placement IK Retarget Graph: bone mapping and retarget path resolution for the Hand Placement IK subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 400,
		baseWeight = 0.5,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.handik.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.handik.*", function(payload) inst.lastSignal = payload end)
			end
