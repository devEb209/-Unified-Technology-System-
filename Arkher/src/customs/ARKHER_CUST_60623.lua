...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Anvil Crowd 10k CUSTOM 60623", size=Vector2.new(679, 471), pos=UDim2.fromOffset(81, 299), parent=parent, icon="▣"})
    win.Root.Name="ARKHER_CUST_60623"
...
local hdr=Instance.new("TextLabel"); hdr.Size=UDim2.new(1,0,0,22); hdr.BackgroundTransparency=1; hdr.Text="Anvil Crowd 10k CUSTOM 60623"; hdr.Font=Enum.Font.GothamBold; hdr.TextSize=11; hdr.TextColor3=Theme.tokens.arkherGlow; hdr.TextXAlignment=Enum.TextXAlignment.Left; hdr.Parent=content
Components.TextField(content, "Value", "CUSTOM_60623", function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_T", v) end end)
Components.Slider(content, "Level", 0,1,0.5, function(v) end)
    ...
    local idLbl=Instance.new("TextLabel"); idLbl.Size=UDim2.new(1,0,0,12); idLbl.Position=UDim2.new(0,0,1,-12); idLbl.BackgroundTransparency=1; idLbl.Text="ID "..(2000000+60623*137); idLbl.Font=Enum.Font.Code; idLbl.TextSize=8; idLbl.TextColor3=Theme.tokens.slate400; idLbl.Parent=img
