-- ARKHER SYSTEM T.0119 :: Long Term Memory Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0119"
	S.key = "arkher.ai.longtermmemory.decision_policy"
	S.name = "Long Term Memory Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.longtermmemory.reasoning_graph" }
	S.tags = { "t", "longtermmemory", "policy", "ai" }
	S.description = "Long Term Memory Decision Policy: ordered rules deciding what the agent does next for the Long Term Memory subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.longtermmemory.decision_policy", algorithm = "roundrobin", starvationGuard = 56 })
		inst.system = S
		inst.ctx = ctx
