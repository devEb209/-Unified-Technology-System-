-- ARKHER SYSTEM P.0417 :: Team Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0417"
	S.key = "arkher.play.team.difficulty_controller"
	S.name = "Team Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Team"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.team.balance_analysis" }
	S.tags = { "p", "team", "controller", "play" }
	S.description = "Team Difficulty Controller: closed-loop tuning toward a target challenge for the Team subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 560,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.team.difficulty_controller", mode = "pid", target = 0.600, kp = 0.350, ki = 0.030, kd = 0.010, min = 0.05, max = 1.0, initial = 0.50, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.team.*", function(payload) inst.lastSignal = payload end)
			end
