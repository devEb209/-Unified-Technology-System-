-- ARKHER SYSTEM VA.0280 :: Water Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0280"
	S.key = "arkher.contworld.watercont.multi-scale_fidelity"
	S.name = "Water Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.watercont.sparse_residency" }
	S.tags = { "va", "watercont", "multiscale", "contworld" }
	S.description = "Water Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Water Continuum subsystem."
	S.params = {
		backlogLimit = 11,
		baseRadius = 600,
		baseWeight = 0.63,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.watercont.multi-scale_fidelity", levels = 6, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.watercont.*", function(payload) inst.lastSignal = payload end)
			end
