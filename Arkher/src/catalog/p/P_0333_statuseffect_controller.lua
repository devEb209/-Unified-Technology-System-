-- ARKHER SYSTEM P.0333 :: Status Effect Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0333"
	S.key = "arkher.play.statuseffect.difficulty_controller"
	S.name = "Status Effect Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.statuseffect.balance_analysis" }
	S.tags = { "p", "statuseffect", "controller", "play" }
	S.description = "Status Effect Difficulty Controller: closed-loop tuning toward a target challenge for the Status Effect subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 280,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.statuseffect.difficulty_controller", mode = "bangbang", target = 0.690, kp = 0.340, ki = 0.080, kd = 0.050, min = 0.05, max = 1.0, initial = 0.59, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.statuseffect.*", function(payload) inst.lastSignal = payload end)
			end
