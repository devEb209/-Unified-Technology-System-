-- ARKHER SYSTEM VC.0532 :: Pipeline Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0532"
	S.key = "arkher.contneural.pipelinecont.multi-scale_fidelity"
	S.name = "Pipeline Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.pipelinecont.sparse_residency" }
	S.tags = { "vc", "pipelinecont", "multiscale", "contneural" }
	S.description = "Pipeline Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Pipeline Continuum subsystem."
	S.params = {
		backlogLimit = 14,
		baseRadius = 400,
		baseWeight = 0.86,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.pipelinecont.multi-scale_fidelity", levels = 4, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.pipelinecont.*", function(payload) inst.lastSignal = payload end)
			end
