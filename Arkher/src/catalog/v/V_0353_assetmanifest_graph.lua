-- ARKHER SYSTEM V.0353 :: Manifest Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0353"
	S.key = "arkher.asset.assetmanifest.dependency_graph"
	S.name = "Manifest Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.assetmanifest.asset_registry" }
	S.tags = { "v", "assetmanifest", "graph", "asset" }
	S.description = "Manifest Dependency Graph: what references what, and in which order it must be built for the Manifest subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 160,
		baseWeight = 0.68,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.assetmanifest.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.assetmanifest.*", function(payload) inst.lastSignal = payload end)
			end
