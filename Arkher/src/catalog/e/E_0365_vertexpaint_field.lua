-- ARKHER SYSTEM E.0365 :: Vertex Painting Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0365"
	S.key = "arkher.material.vertexpaint.procedural_detail"
	S.name = "Vertex Painting Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Vertex Painting"
	S.aspect = "Procedural Detail"
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.vertexpaint.texture_sampling" }
	S.tags = { "e", "vertexpaint", "field", "material" }
	S.description = "Vertex Painting Procedural Detail: coherent noise detail driving the surface for the Vertex Painting subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 280,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.vertexpaint.procedural_detail", seed = 73539, frequency = 0.0088, octaves = 6, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.vertexpaint.*", function(payload) inst.lastSignal = payload end)
			end
