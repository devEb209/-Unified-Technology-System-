-- ARKHER SYSTEM G.0349 :: Inference Scheduler Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0349"
	S.key = "arkher.neural.inferencesched.sampling_graph"
	S.name = "Inference Scheduler Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Inference Scheduler"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.inferencesched.training_ledger" }
	S.tags = { "g", "inferencesched", "graph", "neural" }
	S.description = "Inference Scheduler Sampling Graph: dependency graph of sampling stages for the Inference Scheduler subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 160,
		baseWeight = 0.72,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.inferencesched.sampling_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.inferencesched.*", function(payload) inst.lastSignal = payload end)
			end
