-- ARKHER SYSTEM L.0371 :: Trade Route Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0371"
	S.key = "arkher.sim.traderoute.fidelity_policy"
	S.name = "Trade Route Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.traderoute.streaming" }
	S.tags = { "l", "traderoute", "policy", "sim" }
	S.description = "Trade Route Fidelity Policy: rules that pick full, cohort or statistical simulation for the Trade Route subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 600,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.traderoute.fidelity_policy", algorithm = "lottery", starvationGuard = 51 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.traderoute.*", function(payload) inst.lastSignal = payload end)
			end
