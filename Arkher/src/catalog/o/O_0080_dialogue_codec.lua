-- ARKHER SYSTEM O.0080 :: Dialogue Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0080"
	S.key = "arkher.audio.dialogue.compression_codec"
	S.name = "Dialogue Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "Dialogue"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.dialogue.asset_registry" }
	S.tags = { "o", "dialogue", "codec", "audio" }
	S.description = "Dialogue Compression Codec: payload encoding of audio state and events for the Dialogue subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 400,
		baseWeight = 0.82,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.dialogue.compression_codec", format = "rle", quantBits = 14 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.dialogue.*", function(payload) inst.lastSignal = payload end)
			end
