-- ARKHER SYSTEM VA.0028 :: Zone Chunk Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0028"
	S.key = "arkher.contworld.zonechunk.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.zonechunk.sparse_residency" }
	S.tags = { "va", "zonechunk", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.zonechunk.multi-scale_fidelity", levels = 4, baseTriangles = 165000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.zonechunk.*", function(payload) inst.lastSignal = payload end)
			end
