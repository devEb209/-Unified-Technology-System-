-- ARKHER SYSTEM L.0042 :: Offscreen Simulation Aggregate Cache
...
return function(A)
	...
	local S = {}
	S.id = "L.0042"
	S.key = "arkher.sim.offscreensim.aggregate_cache"
	S.name = "Offscreen Simulation Aggregate Cache"
	S.category = "L"
	S.family = "WORLD SIMULATION"
	S.area = "Offscreen Simulation"
	S.aspect = "Aggregate Cache"
	S.kit = "cache"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.offscreensim.fidelity_policy" }
	S.tags = { "l", "offscreensim", "cache", "sim" }
	S.description = "Offscreen Simulation Aggregate Cache: reuse of expensive aggregate computations for the Offscreen Simulation subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 240,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("cache", { id = "arkher.sim.offscreensim.aggregate_cache", policy = "ttl", capacity = 398, ttl = 4 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.shrinkTo(capacity)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.offscreensim.*", function(payload) inst.lastSignal = payload end)
			end
