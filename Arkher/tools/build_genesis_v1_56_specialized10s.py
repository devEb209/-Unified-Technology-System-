#!/usr/bin/env python3
"""GENESIS V1.56 SPECIALIZED 10s — +10 únicos 19/33 (196/332) sem parar"""
...
def patch56(roots):
    items=[
        ("ARKHER_Tutorial2","TUTORIAL2 — Highlight  •  Step",0xFF6F00,"Highlight  •  Step  •  Skip","Highlight Step Skip  •  Progress 3/5"),
        ("ARKHER_Help","HELP — Search  •  Article",0x0288D1,"Search  •  Article  •  Video","Search Article Video  •  F1"),
        ("ARKHER_Feedback","FEEDBACK — Rate  •  Report",0x4A148C,"Rate  •  Report  •  Screenshot","Rate Report Screenshot  •  5 stars"),
        ("ARKHER_Bug","BUG — Repro  •  Log",0xB71C1C,"Repro  •  Log  •  Priority","Repro Log Priority  •  Critical"),
        ("ARKHER_Feature","FEATURE — Vote  •  Roadmap",0x1B5E20,"Vote  •  Roadmap  •  Status","Vote Roadmap Status  •  Planned"),
        ("ARKHER_Roadmap","ROADMAP — Quarter  •  Milestone",0x33691E,"Quarter  •  Milestone  •  Done","Quarter Milestone Done  •  Q4"),
        ("ARKHER_Changelog","CHANGELOG — Version  •  Note",0x37474F,"Version  •  Note  •  Date","Version Note Date  •  v1.55"),
        ("ARKHER_License","LICENSE — Key  •  Activate",0x00838F,"Key  •  Activate  •  Seat","Key Activate Seat  •  5 seats"),
        ("ARKHER_Update","UPDATE — Patch  •  Note",0x01579B,"Patch  •  Note  •  Download","Patch Note Download  •  120MB"),
        ("ARKHER_About","ABOUT — Info  •  Credits",0x5D4037,"Info  •  Credits  •  Legal","Info Credits Legal  •  ARKHER v1.55"),
    ]
...
for _,n in ipairs({"ARKHER_Tutorial2","ARKHER_Help","ARKHER_Feedback","ARKHER_Bug","ARKHER_Feature","ARKHER_Roadmap","ARKHER_Changelog","ARKHER_License","ARKHER_Update","ARKHER_About"}) do
...
print("[V1.56 10x] Tutorial2/Help/Feedback/Bug/Feature/Roadmap/Changelog/License/Update/About 196/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_56",{"Source":(T_STRING, FUNC56)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch55(roots)
    roots=patch56(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_56.rbxl")
    write(out, serialize(roots))
    print(f"V1.56 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_56.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.56 done")
