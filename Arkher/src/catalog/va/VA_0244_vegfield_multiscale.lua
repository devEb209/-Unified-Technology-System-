-- ARKHER SYSTEM VA.0244 :: Vegetation Field Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0244"
	S.key = "arkher.contworld.vegfield.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.vegfield.sparse_residency" }
	S.tags = { "va", "vegfield", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.vegfield.multi-scale_fidelity", levels = 4, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.vegfield.*", function(payload) inst.lastSignal = payload end)
			end
