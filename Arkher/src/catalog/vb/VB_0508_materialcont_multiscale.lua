-- ARKHER SYSTEM VB.0508 :: Material Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0508"
	S.key = "arkher.conttime.materialcont.multi-scale_fidelity"
	S.name = "Material Continuum Multi-Scale Fidelity"
	S.category = "VB"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.materialcont.sparse_residency" }
	S.tags = { "vb", "materialcont", "multiscale", "conttime" }
	S.description = "Material Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Material Continuum subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 200,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.materialcont.multi-scale_fidelity", levels = 5, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.materialcont.*", function(payload) inst.lastSignal = payload end)
			end
