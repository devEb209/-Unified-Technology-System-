-- ARKHER SYSTEM W.0354 :: Subtitle Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0354"
	S.key = "arkher.cine.subtitle.playback_orchestrator"
	S.name = "Subtitle Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Subtitle"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.subtitle.cut_policy" }
	S.tags = { "w", "subtitle", "orchestrator", "cine" }
	S.description = "Subtitle Playback Orchestrator: the state machine that drives a cinematic from start to end for the Subtitle subsystem."
	S.params = {
		backlogLimit = 15,
		baseRadius = 600,
		baseWeight = 0.67,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.subtitle.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.subtitle.*", function(payload) inst.lastSignal = payload end)
			end
