-- ARKHER SYSTEM T.0441 :: Player Modelling Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0441"
	S.key = "arkher.ai.playermodelling.decision_policy"
	S.name = "Player Modelling Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.playermodelling.reasoning_graph" }
	S.tags = { "t", "playermodelling", "policy", "ai" }
	S.description = "Player Modelling Decision Policy: ordered rules deciding what the agent does next for the Player Modelling subsystem."
	S.params = {
		backlogLimit = 16,
		baseRadius = 320,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.playermodelling.decision_policy", algorithm = "roundrobin", starvationGuard = 32 })
		inst.system = S
		inst.ctx = ctx
