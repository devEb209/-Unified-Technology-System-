-- ARKHER SYSTEM G.0346 :: Inference Scheduler Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0346"
	S.key = "arkher.neural.inferencesched.heuristic_policy"
	S.name = "Inference Scheduler Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.inferencesched.result_cache" }
	S.tags = { "g", "inferencesched", "policy", "neural" }
	S.description = "Inference Scheduler Heuristic Policy: rule fallback when the model is unavailable for the Inference Scheduler subsystem."
	S.params = {
		backlogLimit = 39,
		baseRadius = 600,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.inferencesched.heuristic_policy", algorithm = "lottery", starvationGuard = 39 })
		inst.system = S
		inst.ctx = ctx
