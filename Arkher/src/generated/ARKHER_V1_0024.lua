--!strict
-- WorldMachine Tiled — CORE Profiler #0024
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="WorldMachine Tiled — CORE Profiler #0024", size=Vector2.new(600, 500), pos=UDim2.fromOffset(412, 228), parent=parent, icon="▭"})
...
Components.TextField(right, "File", "input.png", function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_File"..24, v) end end)
