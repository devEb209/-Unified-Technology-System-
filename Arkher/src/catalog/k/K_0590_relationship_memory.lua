-- ARKHER SYSTEM K.0590 :: Relationship Memory
...
return function(A)
	...
	local S = {}
	S.id = "K.0590"
	S.key = "arkher.npc.relationship.memory"
	S.name = "Relationship Memory"
	S.category = "K"
	...
	S.kit = "memory"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.relationship.mind_network" }
	S.tags = { "k", "relationship", "memory", "npc" }
	S.description = "Relationship Memory: episodic memory with decay, recall and consolidation for the Relationship subsystem."
	S.params = {
		backlogLimit = 10,
		baseRadius = 560,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("memory", { id = "arkher.npc.relationship.memory", capacity = 90, decayRate = 0.030, consolidateAt = 4 })
		inst.system = S
		inst.ctx = ctx
