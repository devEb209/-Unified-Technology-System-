-- ARKHER SYSTEM VA.0460 :: Audio Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0460"
	S.key = "arkher.contworld.audiocont.multi-scale_fidelity"
	S.name = "Audio Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.audiocont.sparse_residency" }
	S.tags = { "va", "audiocont", "multiscale", "contworld" }
	S.description = "Audio Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Audio Continuum subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 320,
		baseWeight = 0.54,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.audiocont.multi-scale_fidelity", levels = 5, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.audiocont.*", function(payload) inst.lastSignal = payload end)
			end
