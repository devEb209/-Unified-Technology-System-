-- ARKHER SYSTEM G.0402 :: Fallback Heuristic Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0402"
	S.key = "arkher.neural.fallbackheuristic.heuristic_policy"
	S.name = "Fallback Heuristic Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.fallbackheuristic.result_cache" }
	S.tags = { "g", "fallbackheuristic", "policy", "neural" }
	S.description = "Fallback Heuristic Heuristic Policy: rule fallback when the model is unavailable for the Fallback Heuristic subsystem."
	S.params = {
		backlogLimit = 23,
		baseRadius = 600,
		baseWeight = 0.55,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.fallbackheuristic.heuristic_policy", algorithm = "lottery", starvationGuard = 39 })
		inst.system = S
		inst.ctx = ctx
