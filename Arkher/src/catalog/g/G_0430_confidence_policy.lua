-- ARKHER SYSTEM G.0430 :: Confidence Gating Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0430"
	S.key = "arkher.neural.confidence.heuristic_policy"
	S.name = "Confidence Gating Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.confidence.result_cache" }
	S.tags = { "g", "confidence", "policy", "neural" }
	S.description = "Confidence Gating Heuristic Policy: rule fallback when the model is unavailable for the Confidence Gating subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 360,
		baseWeight = 0.53,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.confidence.heuristic_policy", algorithm = "edf", starvationGuard = 21 })
		inst.system = S
		inst.ctx = ctx
