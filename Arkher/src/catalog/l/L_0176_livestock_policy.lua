-- ARKHER SYSTEM L.0176 :: Livestock Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0176"
	S.key = "arkher.sim.livestock.fidelity_policy"
	S.name = "Livestock Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.livestock.streaming" }
	S.tags = { "l", "livestock", "policy", "sim" }
	S.description = "Livestock Fidelity Policy: rules that pick full, cohort or statistical simulation for the Livestock subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 280,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.livestock.fidelity_policy", algorithm = "lottery", starvationGuard = 19 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.livestock.*", function(payload) inst.lastSignal = payload end)
			end
