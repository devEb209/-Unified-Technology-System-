-- ARKHER SYSTEM O.0176 :: City Audio Compression Codec
-- Category O - AUDIO
...
return function(A)
	...
	local S = {}
	S.id = "O.0176"
	S.key = "arkher.audio.cityaudio.compression_codec"
	S.name = "City Audio Compression Codec"
	S.category = "O"
	S.family = "AUDIO"
	S.area = "City Audio"
	...
	S.kit = "codec"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.cityaudio.asset_registry" }
	S.tags = { "o", "cityaudio", "codec", "audio" }
	S.description = "City Audio Compression Codec: payload encoding of audio state and events for the City Audio subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 480,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("codec", { id = "arkher.audio.cityaudio.compression_codec", format = "binary", quantBits = 12 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.transmit(value, channel)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.audio.cityaudio.*", function(payload) inst.lastSignal = payload end)
			end
