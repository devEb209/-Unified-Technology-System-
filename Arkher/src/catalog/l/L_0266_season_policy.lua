-- ARKHER SYSTEM L.0266 :: Season Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0266"
	S.key = "arkher.sim.season.fidelity_policy"
	S.name = "Season Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.season.streaming" }
	S.tags = { "l", "season", "policy", "sim" }
	S.description = "Season Fidelity Policy: rules that pick full, cohort or statistical simulation for the Season subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 200,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.season.fidelity_policy", algorithm = "edf", starvationGuard = 17 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.season.*", function(payload) inst.lastSignal = payload end)
			end
