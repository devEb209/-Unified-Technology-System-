-- ARKHER SYSTEM G.0433 :: Confidence Gating Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0433"
	S.key = "arkher.neural.confidence.sampling_graph"
	S.name = "Confidence Gating Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Confidence Gating"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.confidence.training_ledger" }
	S.tags = { "g", "confidence", "graph", "neural" }
	S.description = "Confidence Gating Sampling Graph: dependency graph of sampling stages for the Confidence Gating subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 360,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.confidence.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.confidence.*", function(payload) inst.lastSignal = payload end)
			end
