-- ARKHER SYSTEM Q.0273 :: Virtual Joystick Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0273"
	S.key = "arkher.ui.virtualjoystick.accessibility_policy"
	S.name = "Virtual Joystick Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Virtual Joystick"
	S.aspect = "Accessibility Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.virtualjoystick.interaction_ledger" }
	S.tags = { "q", "virtualjoystick", "policy", "ui" }
	S.description = "Virtual Joystick Accessibility Policy: rules enforcing contrast, target size and motion limits for the Virtual Joystick subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 200,
		baseWeight = 0.59,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.virtualjoystick.accessibility_policy", algorithm = "edf", starvationGuard = 17 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.virtualjoystick.*", function(payload) inst.lastSignal = payload end)
			end
