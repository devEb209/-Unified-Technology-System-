-- ARKHER SYSTEM VA.0568 :: Mobile Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0568"
	S.key = "arkher.contworld.mobilecont.multi-scale_fidelity"
	S.name = "Mobile Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.mobilecont.sparse_residency" }
	S.tags = { "va", "mobilecont", "multiscale", "contworld" }
	S.description = "Mobile Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Mobile Continuum subsystem."
	S.params = {
		backlogLimit = 37,
		baseRadius = 520,
		baseWeight = 0.69,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.mobilecont.multi-scale_fidelity", levels = 4, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.mobilecont.*", function(payload) inst.lastSignal = payload end)
			end
