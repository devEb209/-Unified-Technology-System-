--!strict
-- Mapbox GL — ANIMATION Profiler #0219
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Mapbox GL — ANIMATION Profiler #0219", size=Vector2.new(600, 500), pos=UDim2.fromOffset(147, 163), parent=parent, icon="⬔"})
...
Components.TextField(right, "File", "input.png", function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_File"..219, v) end end)
