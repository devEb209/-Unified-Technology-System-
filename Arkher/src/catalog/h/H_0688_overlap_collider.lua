-- ARKHER SYSTEM H.0688 :: Overlap Query Collision Shape
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0688"
	S.key = "arkher.physics.overlap.collision_shape"
	S.name = "Overlap Query Collision Shape"
	S.category = "H"
	S.family = "PHYSICS"
	...
	S.kit = "collider"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.overlap.body_dynamics" }
	S.tags = { "h", "overlap", "collider", "physics" }
	S.description = "Overlap Query Collision Shape: shape bounds, support mapping and closest points for the Overlap Query subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 440,
		baseWeight = 0.63,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("collider", { id = "arkher.physics.overlap.collision_shape", shape = "box", radius = 1.10, height = 1.90, layer = "vehicle" })
		inst.system = S
		inst.ctx = ctx
