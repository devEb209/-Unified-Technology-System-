-- ARKHER SYSTEM VC.0064 :: Sparse Page Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0064"
	S.key = "arkher.contneural.sparsepage.multi-scale_fidelity"
	S.name = "Sparse Page Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.sparsepage.sparse_residency" }
	S.tags = { "vc", "sparsepage", "multiscale", "contneural" }
	S.description = "Sparse Page Multi-Scale Fidelity: distance-driven multi-scale level selection for the Sparse Page subsystem."
	S.params = {
		backlogLimit = 9,
		baseRadius = 200,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.sparsepage.multi-scale_fidelity", levels = 5, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.sparsepage.*", function(payload) inst.lastSignal = payload end)
			end
