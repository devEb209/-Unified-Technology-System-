-- ARKHER SYSTEM R.0051 :: Player State Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0051"
	S.key = "arkher.net.playerstate.prediction_and_reconciliation"
	S.name = "Player State Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Player State"
	...
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.playerstate.time_sync" }
	S.tags = { "r", "playerstate", "prediction", "net" }
	S.description = "Player State Prediction And Reconciliation: client simulation, correction and input replay for the Player State subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 480,
		baseWeight = 0.86,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.playerstate.prediction_and_reconciliation", errorThreshold = 0.060, smoothing = 14.0, maxInputs = 100 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.playerstate.*", function(payload) inst.lastSignal = payload end)
			end
