-- ARKHER SYSTEM V.0125 :: Font Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0125"
	S.key = "arkher.asset.font.dependency_graph"
	S.name = "Font Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.font.asset_registry" }
	S.tags = { "v", "font", "graph", "asset" }
	S.description = "Font Dependency Graph: what references what, and in which order it must be built for the Font subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 480,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.font.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.font.*", function(payload) inst.lastSignal = payload end)
			end
