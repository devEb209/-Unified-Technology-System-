-- ARKHER SYSTEM VC.0028 :: Zone Chunk Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0028"
	S.key = "arkher.contneural.zonechunk.multi-scale_fidelity"
	S.name = "Zone Chunk Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.zonechunk.sparse_residency" }
	S.tags = { "vc", "zonechunk", "multiscale", "contneural" }
	S.description = "Zone Chunk Multi-Scale Fidelity: distance-driven multi-scale level selection for the Zone Chunk subsystem."
	S.params = {
		backlogLimit = 19,
		baseRadius = 440,
		baseWeight = 0.51,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.zonechunk.multi-scale_fidelity", levels = 5, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.zonechunk.*", function(payload) inst.lastSignal = payload end)
			end
