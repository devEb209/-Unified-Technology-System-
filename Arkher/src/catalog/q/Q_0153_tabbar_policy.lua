-- ARKHER SYSTEM Q.0153 :: Tab Bar Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0153"
	S.key = "arkher.ui.tabbar.accessibility_policy"
	S.name = "Tab Bar Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Tab Bar"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.tabbar.interaction_ledger" }
	S.tags = { "q", "tabbar", "policy", "ui" }
	S.description = "Tab Bar Accessibility Policy: rules enforcing contrast, target size and motion limits for the Tab Bar subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.57,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.tabbar.accessibility_policy", algorithm = "edf", starvationGuard = 37 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.tabbar.*", function(payload) inst.lastSignal = payload end)
			end
