-- ARKHER SYSTEM V.0401 :: Thumbnail Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0401"
	S.key = "arkher.asset.thumbnail.dependency_graph"
	S.name = "Thumbnail Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.thumbnail.asset_registry" }
	S.tags = { "v", "thumbnail", "graph", "asset" }
	S.description = "Thumbnail Dependency Graph: what references what, and in which order it must be built for the Thumbnail subsystem."
	S.params = {
		backlogLimit = 18,
		baseRadius = 560,
		baseWeight = 0.8,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.thumbnail.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.thumbnail.*", function(payload) inst.lastSignal = payload end)
			end
