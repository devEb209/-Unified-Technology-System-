-- ARKHER SYSTEM VA.0472 :: VFX Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0472"
	S.key = "arkher.contworld.vfxcont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.vfxcont.sparse_residency" }
	S.tags = { "va", "vfxcont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.vfxcont.multi-scale_fidelity", levels = 4, baseTriangles = 150000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.vfxcont.*", function(payload) inst.lastSignal = payload end)
			end
