-- ARKHER SYSTEM L.0551 :: Festival Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0551"
	S.key = "arkher.sim.festival.fidelity_policy"
	S.name = "Festival Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.festival.streaming" }
	S.tags = { "l", "festival", "policy", "sim" }
	S.description = "Festival Fidelity Policy: rules that pick full, cohort or statistical simulation for the Festival subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 600,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.festival.fidelity_policy", algorithm = "lottery", starvationGuard = 63 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.festival.*", function(payload) inst.lastSignal = payload end)
			end
