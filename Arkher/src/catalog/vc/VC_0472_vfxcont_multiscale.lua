-- ARKHER SYSTEM VC.0472 :: VFX Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0472"
	S.key = "arkher.contneural.vfxcont.multi-scale_fidelity"
	S.name = "VFX Continuum Multi-Scale Fidelity"
	S.category = "VC"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.vfxcont.sparse_residency" }
	S.tags = { "vc", "vfxcont", "multiscale", "contneural" }
	S.description = "VFX Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the VFX Continuum subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 200,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.vfxcont.multi-scale_fidelity", levels = 5, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.vfxcont.*", function(payload) inst.lastSignal = payload end)
			end
