-- ARKHER SYSTEM H.0668 :: Raycast Query Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0668"
	S.key = "arkher.physics.raycast.simulation_policy"
	S.name = "Raycast Query Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.raycast.budget_governor" }
	S.tags = { "h", "raycast", "policy", "physics" }
	S.description = "Raycast Query Simulation Policy: device tier rules for substeps, iterations and sleep for the Raycast Query subsystem."
	S.params = {
		backlogLimit = 25,
		baseRadius = 360,
		baseWeight = 0.67,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.raycast.simulation_policy", algorithm = "edf", starvationGuard = 33 })
		inst.system = S
		inst.ctx = ctx
