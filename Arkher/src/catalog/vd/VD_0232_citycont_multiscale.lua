-- ARKHER SYSTEM VD.0232 :: City Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0232"
	S.key = "arkher.contsim.citycont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.citycont.sparse_residency" }
	S.tags = { "vd", "citycont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.citycont.multi-scale_fidelity", levels = 4, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.citycont.*", function(payload) inst.lastSignal = payload end)
			end
