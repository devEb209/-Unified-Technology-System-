-- ARKHER SYSTEM VA.0340 :: Society Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0340"
	S.key = "arkher.contworld.societycont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.societycont.sparse_residency" }
	S.tags = { "va", "societycont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.societycont.multi-scale_fidelity", levels = 6, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.societycont.*", function(payload) inst.lastSignal = payload end)
			end
