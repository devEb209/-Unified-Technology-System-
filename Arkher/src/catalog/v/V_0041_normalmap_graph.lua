-- ARKHER SYSTEM V.0041 :: Normal Map Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0041"
	S.key = "arkher.asset.normalmap.dependency_graph"
	S.name = "Normal Map Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.normalmap.asset_registry" }
	S.tags = { "v", "normalmap", "graph", "asset" }
	S.description = "Normal Map Dependency Graph: what references what, and in which order it must be built for the Normal Map subsystem."
	S.params = {
		backlogLimit = 13,
		baseRadius = 360,
		baseWeight = 0.65,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.normalmap.dependency_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.normalmap.*", function(payload) inst.lastSignal = payload end)
			end
