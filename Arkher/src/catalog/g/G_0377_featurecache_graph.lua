-- ARKHER SYSTEM G.0377 :: Feature Cache Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0377"
	S.key = "arkher.neural.featurecache.sampling_graph"
	S.name = "Feature Cache Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Feature Cache"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.featurecache.training_ledger" }
	S.tags = { "g", "featurecache", "graph", "neural" }
	S.description = "Feature Cache Sampling Graph: dependency graph of sampling stages for the Feature Cache subsystem."
	S.params = {
		backlogLimit = 20,
		baseRadius = 320,
		baseWeight = 0.72,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.featurecache.sampling_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.featurecache.*", function(payload) inst.lastSignal = payload end)
			end
