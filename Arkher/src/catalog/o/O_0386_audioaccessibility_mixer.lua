-- ARKHER SYSTEM O.0386 :: Accessibility Audio Mixer Bus
-- Category O - AUDIO
-- ARKHER Audio Framework capability: buses, DSP, spatialization and music that reacts to the world.
-- Kit: mixer (hierarchical buses, gain, mute, solo, ducking and voice stealing)
--@arkher-module
return function(A)
	...
	local S = {}
	S.id = "O.0386"
	S.key = "arkher.audio.audioaccessibility.mixer_bus"
	S.name = "Accessibility Audio Mixer Bus"
	S.category = "O"
	S.family = "AUDIO"
	...
	S.kit = "mixer"
	S.version = "1.0.0"
	S.deps = { "arkher.audio.audioaccessibility.dsp_chain" }
	S.tags = { "o", "audioaccessibility", "mixer", "audio" }
	S.description = "Accessibility Audio Mixer Bus: hierarchical buses, gain, mute, solo, ducking and voice stealing for the Accessibility Audio subsystem."
	S.params = {
		backlogLimit = 41,
		baseRadius = 360,
		baseWeight = 0.63,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("mixer", { id = "arkher.audio.audioaccessibility.mixer_bus", maxVoices = 56 })
		inst.system = S
		inst.ctx = ctx
