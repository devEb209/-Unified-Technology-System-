-- ARKHER SYSTEM P.0261 :: Dialogue Choice Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0261"
	S.key = "arkher.play.dialoguechoice.difficulty_controller"
	S.name = "Dialogue Choice Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Dialogue Choice"
	S.aspect = "Difficulty Controller"
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.dialoguechoice.balance_analysis" }
	S.tags = { "p", "dialoguechoice", "controller", "play" }
	S.description = "Dialogue Choice Difficulty Controller: closed-loop tuning toward a target challenge for the Dialogue Choice subsystem."
	S.params = {
		backlogLimit = 31,
		baseRadius = 280,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.dialoguechoice.difficulty_controller", mode = "bangbang", target = 0.730, kp = 0.280, ki = 0.050, kd = 0.040, min = 0.05, max = 1.0, initial = 0.63, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.dialoguechoice.*", function(payload) inst.lastSignal = payload end)
			end
