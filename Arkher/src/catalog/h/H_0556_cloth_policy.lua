-- ARKHER SYSTEM H.0556 :: Cloth Simulation Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0556"
	S.key = "arkher.physics.cloth.simulation_policy"
	S.name = "Cloth Simulation Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Cloth Simulation"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.cloth.budget_governor" }
	S.tags = { "h", "cloth", "policy", "physics" }
	S.description = "Cloth Simulation Simulation Policy: device tier rules for substeps, iterations and sleep for the Cloth Simulation subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.cloth.simulation_policy", algorithm = "edf", starvationGuard = 61 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.cloth.*", function(payload) inst.lastSignal = payload end)
			end
