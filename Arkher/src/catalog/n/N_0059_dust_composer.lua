-- ARKHER SYSTEM N.0059 :: Dust Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0059"
	S.key = "arkher.vfx.dust.effect_composition"
	S.name = "Dust Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Dust"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.dust.effect_graph" }
	S.tags = { "n", "dust", "composer", "vfx" }
	S.description = "Dust Effect Composition: blending of several effect signals into one output for the Dust subsystem."
	S.params = {
		backlogLimit = 28,
		baseRadius = 320,
		baseWeight = 0.7,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.dust.effect_composition", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.dust.*", function(payload) inst.lastSignal = payload end)
			end
