-- ARKHER SYSTEM K.0209 :: Emotional State Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0209"
	S.key = "arkher.npc.emotionalstate.thinking_budget"
	S.name = "Emotional State Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Emotional State"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.emotionalstate.daily_schedule" }
	S.tags = { "k", "emotionalstate", "budgeter", "npc" }
	S.description = "Emotional State Thinking Budget: allocation of per-frame thinking time across NPCs for the Emotional State subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 480,
		baseWeight = 0.86,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.emotionalstate.thinking_budget", total = 196, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.emotionalstate.*", function(payload) inst.lastSignal = payload end)
			end
