-- ARKHER SYSTEM F.0494 :: HLOD Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0494"
	S.key = "arkher.render.hlod.geometry_virtualization"
	S.name = "HLOD Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "HLOD"
	...
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.hlod.visibility_set" }
	S.tags = { "f", "hlod", "impostor", "render" }
	S.description = "HLOD Geometry Virtualization: impostor and HLOD synthesis under a budget for the HLOD subsystem."
	S.params = {
		backlogLimit = 47,
		baseRadius = 440,
		baseWeight = 0.99,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.hlod.geometry_virtualization", atlasSlots = 48, viewCount = 8, errorThreshold = 2.90 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.hlod.*", function(payload) inst.lastSignal = payload end)
			end
