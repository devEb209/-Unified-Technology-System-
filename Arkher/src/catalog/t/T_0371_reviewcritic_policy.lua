-- ARKHER SYSTEM T.0371 :: Review Critic Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0371"
	S.key = "arkher.ai.reviewcritic.decision_policy"
	S.name = "Review Critic Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.reviewcritic.reasoning_graph" }
	S.tags = { "t", "reviewcritic", "policy", "ai" }
	S.description = "Review Critic Decision Policy: ordered rules deciding what the agent does next for the Review Critic subsystem."
	S.params = {
		backlogLimit = 40,
		baseRadius = 160,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.reviewcritic.decision_policy", algorithm = "roundrobin", starvationGuard = 16 })
		inst.system = S
		inst.ctx = ctx
