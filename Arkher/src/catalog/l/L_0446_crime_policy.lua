-- ARKHER SYSTEM L.0446 :: Crime Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0446"
	S.key = "arkher.sim.crime.fidelity_policy"
	S.name = "Crime Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.crime.streaming" }
	S.tags = { "l", "crime", "policy", "sim" }
	S.description = "Crime Fidelity Policy: rules that pick full, cohort or statistical simulation for the Crime subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.87,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.crime.fidelity_policy", algorithm = "edf", starvationGuard = 37 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.crime.*", function(payload) inst.lastSignal = payload end)
			end
