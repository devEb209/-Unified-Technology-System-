--!strict
-- Quixel Bridge — ANIMATION Profiler #0319
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Quixel Bridge — ANIMATION Profiler #0319", size=Vector2.new(640, 500), pos=UDim2.fromOffset(247, 303), parent=parent, icon="⬣"})
...
Components.TextField(form, "Name", "Object_"..319, function(v) local s=game:GetService("Selection"):Get()[1]; if s then game:GetService("ChangeHistoryService"):SetWaypoint("Before Name"); s.Name=v; game:GetService("ChangeHistoryService"):SetWaypoint("Name") end end)
