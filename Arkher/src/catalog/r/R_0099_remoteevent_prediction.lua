-- ARKHER SYSTEM R.0099 :: Remote Event Prediction And Reconciliation
...
return function(A)
	...
	local S = {}
	S.id = "R.0099"
	S.key = "arkher.net.remoteevent.prediction_and_reconciliation"
	S.name = "Remote Event Prediction And Reconciliation"
	S.category = "R"
	S.family = "NETWORKING"
	S.area = "Remote Event"
	...
	S.kit = "prediction"
	S.version = "1.0.0"
	S.deps = { "arkher.net.remoteevent.time_sync" }
	S.tags = { "r", "remoteevent", "prediction", "net" }
	S.description = "Remote Event Prediction And Reconciliation: client simulation, correction and input replay for the Remote Event subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 280,
		baseWeight = 0.69,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("prediction", { id = "arkher.net.remoteevent.prediction_and_reconciliation", errorThreshold = 0.075, smoothing = 17.0, maxInputs = 120 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.net.remoteevent.*", function(payload) inst.lastSignal = payload end)
			end
