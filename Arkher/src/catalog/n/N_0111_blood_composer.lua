-- ARKHER SYSTEM N.0111 :: Blood Effect Composition
-- Category N - VFX
...
return function(A)
	...
	local S = {}
	S.id = "N.0111"
	S.key = "arkher.vfx.blood.effect_composition"
	S.name = "Blood Effect Composition"
	S.category = "N"
	S.family = "VFX"
	S.area = "Blood"
	...
	S.kit = "composer"
	S.version = "1.0.0"
	S.deps = { "arkher.vfx.blood.effect_graph" }
	S.tags = { "n", "blood", "composer", "vfx" }
	S.description = "Blood Effect Composition: blending of several effect signals into one output for the Blood subsystem."
	S.params = {
		backlogLimit = 44,
		baseRadius = 320,
		baseWeight = 0.56,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("composer", { id = "arkher.vfx.blood.effect_composition", mode = "alpha" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.dominantLayer()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.vfx.blood.*", function(payload) inst.lastSignal = payload end)
			end
