-- ARKHER SYSTEM VA.0076 :: LOD Cascade Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0076"
	S.key = "arkher.contworld.lodcascade.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.lodcascade.sparse_residency" }
	S.tags = { "va", "lodcascade", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.lodcascade.multi-scale_fidelity", levels = 6, baseTriangles = 135000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.lodcascade.*", function(payload) inst.lastSignal = payload end)
			end
