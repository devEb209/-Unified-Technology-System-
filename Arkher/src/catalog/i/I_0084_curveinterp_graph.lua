-- ARKHER SYSTEM I.0084 :: Curve Interpolation Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0084"
	S.key = "arkher.anim.curveinterp.retarget_graph"
	S.name = "Curve Interpolation Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Curve Interpolation"
	S.aspect = "Retarget Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.curveinterp.physical_blend" }
	S.tags = { "i", "curveinterp", "graph", "anim" }
	S.description = "Curve Interpolation Retarget Graph: bone mapping and retarget path resolution for the Curve Interpolation subsystem."
	S.params = {
		backlogLimit = 40,
		baseRadius = 160,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.curveinterp.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.curveinterp.*", function(payload) inst.lastSignal = payload end)
			end
