-- ARKHER SYSTEM V.0089 :: Audio Clip Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0089"
	S.key = "arkher.asset.audioclip.dependency_graph"
	S.name = "Audio Clip Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.audioclip.asset_registry" }
	S.tags = { "v", "audioclip", "graph", "asset" }
	S.description = "Audio Clip Dependency Graph: what references what, and in which order it must be built for the Audio Clip subsystem."
	S.params = {
		backlogLimit = 32,
		baseRadius = 480,
		baseWeight = 0.74,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.audioclip.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.audioclip.*", function(payload) inst.lastSignal = payload end)
			end
