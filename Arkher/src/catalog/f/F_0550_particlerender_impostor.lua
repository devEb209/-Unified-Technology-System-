-- ARKHER SYSTEM F.0550 :: Particle Rendering Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0550"
	S.key = "arkher.render.particlerender.geometry_virtualization"
	S.name = "Particle Rendering Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Particle Rendering"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.particlerender.visibility_set" }
	S.tags = { "f", "particlerender", "impostor", "render" }
	S.description = "Particle Rendering Geometry Virtualization: impostor and HLOD synthesis under a budget for the Particle Rendering subsystem."
	S.params = {
		backlogLimit = 19,
		baseRadius = 280,
		baseWeight = 0.71,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.particlerender.geometry_virtualization", atlasSlots = 80, viewCount = 8, errorThreshold = 3.10 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.particlerender.*", function(payload) inst.lastSignal = payload end)
			end
