-- ARKHER SYSTEM G.0055 :: Frame Interpolation Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0055"
	S.key = "arkher.neural.frameinterp.sampling_graph"
	S.name = "Frame Interpolation Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Frame Interpolation"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.frameinterp.training_ledger" }
	S.tags = { "g", "frameinterp", "graph", "neural" }
	S.description = "Frame Interpolation Sampling Graph: dependency graph of sampling stages for the Frame Interpolation subsystem."
	S.params = {
		backlogLimit = 30,
		baseRadius = 400,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.frameinterp.sampling_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.frameinterp.*", function(payload) inst.lastSignal = payload end)
			end
