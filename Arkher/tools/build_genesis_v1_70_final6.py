#!/usr/bin/env python3
"""GENESIS V1.70 FINAL — +6 únicos 33/33 (332/332) sem parar — 100%"""
...
from build_genesis_v1_69_specialized10af import patch69
...
def patch70(roots):
    items=[
        ("ARKHER_Final1","FINAL1 — Export  •  Ship",0x1B5E20,"Export  •  Ship  •  Verify","Export Ship Verify  •  332/332 DONE"),
        ("ARKHER_Final2","FINAL2 — Publish  •  Live",0x2E7D32,"Publish  •  Live  •  Announce","Publish Live Announce  •  GENESIS"),
        ("ARKHER_Final3","FINAL3 — Celebrate  •  AAA",0xFFD600,"Celebrate  •  AAA  •  Mobile","Celebrate AAA Mobile  •  AAA Console/PC"),
        ("ARKHER_Final4","FINAL4 — Singularity  •  AGI",0x7C4DFF,"Singularity  •  AGI  •  Core","Singularity AGI Core  •  Unaware"),
        ("ARKHER_Final5","FINAL5 — D-O15  •  Opt",0xAD1457,"D-O15  •  Opt  •  620/9300","D-O15 620 Systems 9300 Features"),
        ("ARKHER_Final6","FINAL6 — Neural  •  DLSS",0x01579B,"Neural  •  DLSS  •  6X","Neural DLSS 6X Dynamic 240FPS"),
    ]
...
for _,n in ipairs({"ARKHER_Final1","ARKHER_Final2","ARKHER_Final3","ARKHER_Final4","ARKHER_Final5","ARKHER_Final6"}) do
...
print("[V1.70 FINAL 332/332] 100% TODAS AS 332 UIs ÚNICAS REFEITAS SEM PARAR QUALIDADE MAX")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_70",{"Source":(T_STRING, FUNC70)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch69(roots)
    roots=patch70(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_70.rbxl")
    write(out, serialize(roots))
    print(f"V1.70 FINAL 332/332 {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_70.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.70 done 100%")
