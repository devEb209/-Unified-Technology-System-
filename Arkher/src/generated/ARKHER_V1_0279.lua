--!strict
-- Gaea Nodes — ANIMATION Profiler #0279
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Gaea Nodes — ANIMATION Profiler #0279", size=Vector2.new(600, 500), pos=UDim2.fromOffset(127, 143), parent=parent, icon="▶"})
    win.Root.Name="ARKHER_V1_0279"
    win.Root.Visible=false
    local content=win.Content
    content.BackgroundColor3=Theme.tokens.void
    -- accent line per UI
    local accent=Instance.new("Frame"); accent.Size=UDim2.new(1,0,0,2); accent.BackgroundColor3=Theme.categoryColor("Gaea Nod"); accent.BorderSizePixel=0; accent.Parent=content
...
Components.TextField(right, "File", "input.png", function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_File"..279, v) end end)
