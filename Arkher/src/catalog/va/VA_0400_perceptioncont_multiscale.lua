-- ARKHER SYSTEM VA.0400 :: Perception Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0400"
	S.key = "arkher.contworld.perceptioncont.multi-scale_fidelity"
	S.name = "Perception Continuum Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.perceptioncont.sparse_residency" }
	S.tags = { "va", "perceptioncont", "multiscale", "contworld" }
	S.description = "Perception Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Perception Continuum subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 280,
		baseWeight = 0.97,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.perceptioncont.multi-scale_fidelity", levels = 4, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.perceptioncont.*", function(payload) inst.lastSignal = payload end)
			end
