-- ARKHER SYSTEM VD.0376 :: Dialogue Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0376"
	S.key = "arkher.contsim.dialoguecont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.dialoguecont.sparse_residency" }
	S.tags = { "vd", "dialoguecont", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.dialoguecont.multi-scale_fidelity", levels = 4, baseTriangles = 90000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.dialoguecont.*", function(payload) inst.lastSignal = payload end)
			end
