-- ARKHER SYSTEM V.0341 :: Patch Dependency Graph
...
return function(A)
	...
	local S = {}
	S.id = "V.0341"
	S.key = "arkher.asset.patch.dependency_graph"
	S.name = "Patch Dependency Graph"
	S.category = "V"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.asset.patch.asset_registry" }
	S.tags = { "v", "patch", "graph", "asset" }
	S.description = "Patch Dependency Graph: what references what, and in which order it must be built for the Patch subsystem."
	S.params = {
		backlogLimit = 10,
		baseRadius = 560,
		baseWeight = 0.82,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.asset.patch.dependency_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.asset.patch.*", function(payload) inst.lastSignal = payload end)
			end
