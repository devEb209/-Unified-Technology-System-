-- ARKHER SYSTEM G.0363 :: Batch Inference Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0363"
	S.key = "arkher.neural.batchinference.sampling_graph"
	S.name = "Batch Inference Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Batch Inference"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.batchinference.training_ledger" }
	S.tags = { "g", "batchinference", "graph", "neural" }
	S.description = "Batch Inference Sampling Graph: dependency graph of sampling stages for the Batch Inference subsystem."
	S.params = {
		backlogLimit = 21,
		baseRadius = 200,
		baseWeight = 0.93,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.batchinference.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.batchinference.*", function(payload) inst.lastSignal = payload end)
			end
