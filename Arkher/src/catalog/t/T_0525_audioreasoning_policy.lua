-- ARKHER SYSTEM T.0525 :: Audio Reasoning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0525"
	S.key = "arkher.ai.audioreasoning.decision_policy"
	S.name = "Audio Reasoning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.audioreasoning.reasoning_graph" }
	S.tags = { "t", "audioreasoning", "policy", "ai" }
	S.description = "Audio Reasoning Decision Policy: ordered rules deciding what the agent does next for the Audio Reasoning subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 560,
		baseWeight = 0.74,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.audioreasoning.decision_policy", algorithm = "wfq", starvationGuard = 38 })
		inst.system = S
		inst.ctx = ctx
