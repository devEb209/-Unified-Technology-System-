-- ARKHER SYSTEM W.0174 :: In Game Cinematic Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0174"
	S.key = "arkher.cine.ingamecinematic.playback_orchestrator"
	S.name = "In Game Cinematic Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "In Game Cinematic"
	S.aspect = "Playback Orchestrator"
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.ingamecinematic.cut_policy" }
	S.tags = { "w", "ingamecinematic", "orchestrator", "cine" }
	S.description = "In Game Cinematic Playback Orchestrator: the state machine that drives a cinematic from start to end for the In Game Cinematic subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 360,
		baseWeight = 0.57,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.ingamecinematic.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.ingamecinematic.*", function(payload) inst.lastSignal = payload end)
			end
