-- ARKHER SYSTEM T.0147 :: Tool Use Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0147"
	S.key = "arkher.ai.tooluse.decision_policy"
	S.name = "Tool Use Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.tooluse.reasoning_graph" }
	S.tags = { "t", "tooluse", "policy", "ai" }
	S.description = "Tool Use Decision Policy: ordered rules deciding what the agent does next for the Tool Use subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 320,
		baseWeight = 0.96,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.tooluse.decision_policy", algorithm = "roundrobin", starvationGuard = 56 })
		inst.system = S
		inst.ctx = ctx
