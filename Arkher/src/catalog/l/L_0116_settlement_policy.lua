-- ARKHER SYSTEM L.0116 :: Settlement Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0116"
	S.key = "arkher.sim.settlement.fidelity_policy"
	S.name = "Settlement Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.settlement.streaming" }
	S.tags = { "l", "settlement", "policy", "sim" }
	S.description = "Settlement Fidelity Policy: rules that pick full, cohort or statistical simulation for the Settlement subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 360,
		baseWeight = 0.95,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.settlement.fidelity_policy", algorithm = "edf", starvationGuard = 33 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.settlement.*", function(payload) inst.lastSignal = payload end)
			end
