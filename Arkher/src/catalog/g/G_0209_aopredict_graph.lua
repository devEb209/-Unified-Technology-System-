-- ARKHER SYSTEM G.0209 :: Ambient Occlusion Prediction Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0209"
	S.key = "arkher.neural.aopredict.sampling_graph"
	S.name = "Ambient Occlusion Prediction Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Ambient Occlusion Prediction"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.aopredict.training_ledger" }
	S.tags = { "g", "aopredict", "graph", "neural" }
	S.description = "Ambient Occlusion Prediction Sampling Graph: dependency graph of sampling stages for the Ambient Occlusion Prediction subsystem."
	S.params = {
		backlogLimit = 46,
		baseRadius = 240,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.aopredict.sampling_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.aopredict.*", function(payload) inst.lastSignal = payload end)
			end
