-- ARKHER SYSTEM P.0441 :: Matchmaking Difficulty Controller
-- Category P - GAMEPLAY
...
return function(A)
	...
	local S = {}
	S.id = "P.0441"
	S.key = "arkher.play.matchmaking.difficulty_controller"
	S.name = "Matchmaking Difficulty Controller"
	S.category = "P"
	S.family = "GAMEPLAY"
	S.area = "Matchmaking"
	...
	S.kit = "controller"
	S.version = "1.0.0"
	S.deps = { "arkher.play.matchmaking.balance_analysis" }
	S.tags = { "p", "matchmaking", "controller", "play" }
	S.description = "Matchmaking Difficulty Controller: closed-loop tuning toward a target challenge for the Matchmaking subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 240,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("controller", { id = "arkher.play.matchmaking.difficulty_controller", mode = "pid", target = 0.640, kp = 0.390, ki = 0.070, kd = 0.050, min = 0.05, max = 1.0, initial = 0.54, deadband = 0.01 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.qualityBand()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.play.matchmaking.*", function(payload) inst.lastSignal = payload end)
			end
