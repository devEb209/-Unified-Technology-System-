-- ARKHER SYSTEM F.0424 :: Occlusion Culling Geometry Virtualization
...
return function(A)
	...
	local S = {}
	S.id = "F.0424"
	S.key = "arkher.render.occlusion.geometry_virtualization"
	S.name = "Occlusion Culling Geometry Virtualization"
	S.category = "F"
	S.family = "RENDERING"
	S.area = "Occlusion Culling"
	S.aspect = "Geometry Virtualization"
	S.kit = "impostor"
	S.version = "1.0.0"
	S.deps = { "arkher.render.occlusion.visibility_set" }
	S.tags = { "f", "occlusion", "impostor", "render" }
	S.description = "Occlusion Culling Geometry Virtualization: impostor and HLOD synthesis under a budget for the Occlusion Culling subsystem."
	S.params = {
		backlogLimit = 40,
		baseRadius = 480,
		baseWeight = 0.62,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("impostor", { id = "arkher.render.occlusion.geometry_virtualization", atlasSlots = 64, viewCount = 8, errorThreshold = 1.20 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.render.occlusion.*", function(payload) inst.lastSignal = payload end)
			end
