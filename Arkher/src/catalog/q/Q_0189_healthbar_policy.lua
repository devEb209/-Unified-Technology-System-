-- ARKHER SYSTEM Q.0189 :: Health Bar Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0189"
	S.key = "arkher.ui.healthbar.accessibility_policy"
	S.name = "Health Bar Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Health Bar"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.healthbar.interaction_ledger" }
	S.tags = { "q", "healthbar", "policy", "ui" }
	S.description = "Health Bar Accessibility Policy: rules enforcing contrast, target size and motion limits for the Health Bar subsystem."
	S.params = {
		backlogLimit = 27,
		baseRadius = 440,
		baseWeight = 0.69,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.healthbar.accessibility_policy", algorithm = "lottery", starvationGuard = 35 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.healthbar.*", function(payload) inst.lastSignal = payload end)
			end
