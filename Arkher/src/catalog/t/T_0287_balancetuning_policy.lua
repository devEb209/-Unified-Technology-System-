-- ARKHER SYSTEM T.0287 :: Balance Tuning Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0287"
	S.key = "arkher.ai.balancetuning.decision_policy"
	S.name = "Balance Tuning Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.balancetuning.reasoning_graph" }
	S.tags = { "t", "balancetuning", "policy", "ai" }
	S.description = "Balance Tuning Decision Policy: ordered rules deciding what the agent does next for the Balance Tuning subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 360,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.balancetuning.decision_policy", algorithm = "edf", starvationGuard = 33 })
		inst.system = S
		inst.ctx = ctx
