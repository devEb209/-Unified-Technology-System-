...
| `src/persistence/` | StorageBackend (Memory/File), snapshots versionados com checksum+migração; **`utsdb.js`**: UTS-DB (journal append-only, replay tolerante a torn-tail, tx, índices, compaction, asStorage); **`autosave.js`**: checkpoint journaling (gzip, retenção, crash recovery com step-back ruidoso) |
