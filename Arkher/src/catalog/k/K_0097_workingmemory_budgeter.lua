-- ARKHER SYSTEM K.0097 :: Working Memory Thinking Budget
...
return function(A)
	...
	local S = {}
	S.id = "K.0097"
	S.key = "arkher.npc.workingmemory.thinking_budget"
	S.name = "Working Memory Thinking Budget"
	S.category = "K"
	S.family = "NPC / NEURAL MIND NETWORK"
	S.area = "Working Memory"
	S.aspect = "Thinking Budget"
	S.kit = "budgeter"
	S.version = "1.0.0"
	S.deps = { "arkher.npc.workingmemory.daily_schedule" }
	S.tags = { "k", "workingmemory", "budgeter", "npc" }
	S.description = "Working Memory Thinking Budget: allocation of per-frame thinking time across NPCs for the Working Memory subsystem."
	S.params = {
		backlogLimit = 43,
		baseRadius = 600,
		baseWeight = 0.85,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("budgeter", { id = "arkher.npc.workingmemory.thinking_budget", total = 295, strategy = "waterfill" })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.overSubscribed() return inst.pressure() > 1.0 end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.npc.workingmemory.*", function(payload) inst.lastSignal = payload end)
			end
