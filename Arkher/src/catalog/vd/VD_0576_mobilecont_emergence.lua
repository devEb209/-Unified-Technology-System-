-- ARKHER SYSTEM VD.0576 :: Mobile Continuum Emergence Continuum
...
return function(A)
	...
	local S = {}
	S.id = "VD.0576"
	S.key = "arkher.contsim.mobilecont.emergence_continuum"
	...
	S.kit = "emergence"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.mobilecont.reality_continuum" }
	S.tags = { "vd", "mobilecont", "emergence", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("emergence", { id = "arkher.contsim.mobilecont.emergence_continuum", windowSize = 6, threshold = 1.50, minCount = 2 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.mobilecont.*", function(payload) inst.lastSignal = payload end)
			end
