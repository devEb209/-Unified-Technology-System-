-- ARKHER SYSTEM VC.0220 :: Terrain Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VC.0220"
	S.key = "arkher.contneural.terraincont.multi-scale_fidelity"
	S.name = "Terrain Continuum Multi-Scale Fidelity"
	S.category = "VC"
	S.family = "CONTINUUM — NEURAL"
	S.area = "Terrain Continuum"
	S.aspect = "Multi-Scale Fidelity"
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contneural.terraincont.sparse_residency" }
	S.tags = { "vc", "terraincont", "multiscale", "contneural" }
	S.description = "Terrain Continuum Multi-Scale Fidelity: distance-driven multi-scale level selection for the Terrain Continuum subsystem."
	S.params = {
		backlogLimit = 19,
		baseRadius = 280,
		baseWeight = 0.61,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contneural.terraincont.multi-scale_fidelity", levels = 4, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contneural.terraincont.*", function(payload) inst.lastSignal = payload end)
			end
