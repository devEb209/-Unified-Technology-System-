-- ARKHER SYSTEM G.0108 :: Adaptive Sharpening Heuristic Policy
...
return function(A)
	...
	local S = {}
	S.id = "G.0108"
	S.key = "arkher.neural.adaptivesharpen.heuristic_policy"
	S.name = "Adaptive Sharpening Heuristic Policy"
	S.category = "G"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.adaptivesharpen.result_cache" }
	S.tags = { "g", "adaptivesharpen", "policy", "neural" }
	S.description = "Adaptive Sharpening Heuristic Policy: rule fallback when the model is unavailable for the Adaptive Sharpening subsystem."
	S.params = {
		backlogLimit = 41,
		baseRadius = 360,
		baseWeight = 0.83,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.neural.adaptivesharpen.heuristic_policy", algorithm = "edf", starvationGuard = 33 })
		inst.system = S
		inst.ctx = ctx
