-- ARKHER SYSTEM Z.0403 :: Battery Aware Pipeline Simulation Fabric
...
return function(A)
	...
	local S = {}
	S.id = "Z.0403"
	S.key = "arkher.origin.batteryaware.simulation_fabric"
	S.name = "Battery Aware Pipeline Simulation Fabric"
	S.category = "Z"
	...
	S.kit = "fabric"
	S.version = "1.0.0"
	S.deps = { "arkher.origin.batteryaware.complexity_governance" }
	S.tags = { "z", "batteryaware", "fabric", "origin" }
	S.description = "Battery Aware Pipeline Simulation Fabric: every simulation domain stepped under one budgeted clock for the Battery Aware Pipeline subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 240,
		baseWeight = 0.78,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("fabric", { id = "arkher.origin.batteryaware.simulation_fabric", budgetMs = 2.5, starvationLimit = 6 })
		inst.system = S
		inst.ctx = ctx
