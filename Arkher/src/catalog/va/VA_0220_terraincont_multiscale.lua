-- ARKHER SYSTEM VA.0220 :: Terrain Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0220"
	S.key = "arkher.contworld.terraincont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.terraincont.sparse_residency" }
	S.tags = { "va", "terraincont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.terraincont.multi-scale_fidelity", levels = 4, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.terraincont.*", function(payload) inst.lastSignal = payload end)
			end
