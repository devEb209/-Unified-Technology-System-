-- ARKHER SYSTEM W.0270 :: Letterbox Playback Orchestrator
...
return function(A)
	...
	local S = {}
	S.id = "W.0270"
	S.key = "arkher.cine.letterbox.playback_orchestrator"
	S.name = "Letterbox Playback Orchestrator"
	S.category = "W"
	S.family = "CINEMATIC"
	S.area = "Letterbox"
	...
	S.kit = "orchestrator"
	S.version = "1.0.0"
	S.deps = { "arkher.cine.letterbox.cut_policy" }
	S.tags = { "w", "letterbox", "orchestrator", "cine" }
	S.description = "Letterbox Playback Orchestrator: the state machine that drives a cinematic from start to end for the Letterbox subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 600,
		baseWeight = 0.87,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("orchestrator", { id = "arkher.cine.letterbox.playback_orchestrator", states = { "idle", "warmup", "active", "degraded", "recovering", "halted" } })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.uptimeRatio()
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.cine.letterbox.*", function(payload) inst.lastSignal = payload end)
			end
