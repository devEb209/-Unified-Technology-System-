-- ARKHER SYSTEM E.0110 :: Ambient Occlusion Procedural Detail
...
return function(A)
	...
	local S = {}
	S.id = "E.0110"
	S.key = "arkher.material.ao.procedural_detail"
	S.name = "Ambient Occlusion Procedural Detail"
	S.category = "E"
	S.family = "MATERIALS"
	S.area = "Ambient Occlusion"
	S.aspect = "Procedural Detail"
	S.kit = "field"
	S.version = "1.0.0"
	S.deps = { "arkher.material.ao.texture_sampling" }
	S.tags = { "e", "ao", "field", "material" }
	S.description = "Ambient Occlusion Procedural Detail: coherent noise detail driving the surface for the Ambient Occlusion subsystem."
	S.params = {
		backlogLimit = 10,
		baseRadius = 400,
		baseWeight = 0.92,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("field", { id = "arkher.material.ao.procedural_detail", seed = 5442, frequency = 0.0045, octaves = 5, gain = 0.5, lacunarity = 2.0 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.material.ao.*", function(payload) inst.lastSignal = payload end)
			end
