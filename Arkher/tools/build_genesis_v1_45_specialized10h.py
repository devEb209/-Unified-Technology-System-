#!/usr/bin/env python3
"""GENESIS V1.45 SPECIALIZED 10h — +10 únicos 8/33 (86/332) sem parar"""
...
def patch45(roots):
    items=[
        ("ARKHER_Hair","HAIR — Groom  •  Strand",0x4E342E,"Guide  •  Clump  •  Frizz","Guide  •  Clump  •  Frizz  •  Strand 8k"),
        ("ARKHER_Cloth2","CLOTH — PBD  •  Vellum",0xAD1457,"Stretch  •  Bend  •  Collide","PBD Stretch Bend  •  Vellum  •  Collide"),
        ("ARKHER_Fluid2","FLUID — FLIP  •  APIC",0x0277BD,"Particle  •  Grid  •  Foam","FLIP APIC  •  Particle 2M  •  Foam"),
        ("ARKHER_Snow","SNOW — Accum  •  Footstep",0xE0E0E0,"Height  •  Melt  •  Print","Height  •  Melt  •  Footstep Print"),
        ("ARKHER_Rain","RAIN — Drops  •  Splash",0x01579B,"Drop  •  Ripple  •  Puddle","Drop  •  Ripple  •  Puddle  •  Wetness"),
        ("ARKHER_Wind","WIND — Vector  •  Gust",0x2E7D32,"Field  •  Gust  •  Turbulence","Field  •  Gust  •  Turbulence  •  12 m/s"),
        ("ARKHER_Time","TIME — Day Night  •  Clock",0xFF6F00,"Sun  •  Moon  •  Curve","Sun Moon Curve  •  24h  •  Latitude"),
        ("ARKHER_Sky","SKY — Hosek  •  Nishita",0x0288D1,"Sun  •  Mie  •  Rayleigh","Hosek Nishita  •  Mie  •  Rayleigh"),
        ("ARKHER_Cloud2","CLOUD2 — Volumetric  •  Noise",0x37474F,"Noise  •  Erosion  •  Light","Volumetric  •  Noise  •  Erosion  •  Light"),
        ("ARKHER_Atmo","ATMOS — Fog  •  Aerial",0x5D4037,"Fog  •  Height  •  Inscatter","Fog Height  •  Inscatter  •  Extinction"),
    ]
...
for _,n in ipairs({"ARKHER_Hair","ARKHER_Cloth2","ARKHER_Fluid2","ARKHER_Snow","ARKHER_Rain","ARKHER_Wind","ARKHER_Time","ARKHER_Sky","ARKHER_Cloud2","ARKHER_Atmo"}) do
...
print("[V1.45 10x] Hair/Cloth2/Fluid2/Snow/Rain/Wind/Time/Sky/Cloud2/Atmo 86/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_45",{"Source":(T_STRING, FUNC45)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch44(roots)
    roots=patch45(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_45.rbxl")
    write(out, serialize(roots))
    print(f"V1.45 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_45.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.45 done")
