-- ARKHER SYSTEM Q.0105 :: Modal Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0105"
	S.key = "arkher.ui.modal.accessibility_policy"
	S.name = "Modal Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Modal"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.modal.interaction_ledger" }
	S.tags = { "q", "modal", "policy", "ui" }
	S.description = "Modal Accessibility Policy: rules enforcing contrast, target size and motion limits for the Modal subsystem."
	S.params = {
		backlogLimit = 33,
		baseRadius = 200,
		baseWeight = 0.75,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.modal.accessibility_policy", algorithm = "edf", starvationGuard = 41 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.modal.*", function(payload) inst.lastSignal = payload end)
			end
