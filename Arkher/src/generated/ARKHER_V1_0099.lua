--!strict
-- USGS DEM — ANIMATION Profiler #0099
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="USGS DEM — ANIMATION Profiler #0099", size=Vector2.new(600, 500), pos=UDim2.fromOffset(187, 203), parent=parent, icon="⬢"})
...
Components.TextField(right, "File", "input.png", function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_File"..99, v) end end)
