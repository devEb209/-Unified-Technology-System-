-- ARKHER SYSTEM I.0058 :: Animation Clip Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0058"
	S.key = "arkher.anim.animclip.retarget_graph"
	S.name = "Animation Clip Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.animclip.physical_blend" }
	S.tags = { "i", "animclip", "graph", "anim" }
	S.description = "Animation Clip Retarget Graph: bone mapping and retarget path resolution for the Animation Clip subsystem."
	S.params = {
		backlogLimit = 12,
		baseRadius = 160,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.animclip.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.animclip.*", function(payload) inst.lastSignal = payload end)
			end
