-- ARKHER SYSTEM VA.0112 :: Temporal Slice Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0112"
	S.key = "arkher.contworld.temporalslice.multi-scale_fidelity"
	S.name = "Temporal Slice Multi-Scale Fidelity"
	S.category = "VA"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.temporalslice.sparse_residency" }
	S.tags = { "va", "temporalslice", "multiscale", "contworld" }
	S.description = "Temporal Slice Multi-Scale Fidelity: distance-driven multi-scale level selection for the Temporal Slice subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 160,
		baseWeight = 0.86,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.temporalslice.multi-scale_fidelity", levels = 4, baseTriangles = 60000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.temporalslice.*", function(payload) inst.lastSignal = payload end)
			end
