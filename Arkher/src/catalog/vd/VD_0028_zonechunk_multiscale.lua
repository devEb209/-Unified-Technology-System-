-- ARKHER SYSTEM VD.0028 :: Zone Chunk Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VD.0028"
	S.key = "arkher.contsim.zonechunk.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contsim.zonechunk.sparse_residency" }
	S.tags = { "vd", "zonechunk", "multiscale", "contsim" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contsim.zonechunk.multi-scale_fidelity", levels = 6, baseTriangles = 75000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contsim.zonechunk.*", function(payload) inst.lastSignal = payload end)
			end
