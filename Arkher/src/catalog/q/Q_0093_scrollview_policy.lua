-- ARKHER SYSTEM Q.0093 :: Scroll View Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0093"
	S.key = "arkher.ui.scrollview.accessibility_policy"
	S.name = "Scroll View Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Scroll View"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.scrollview.interaction_ledger" }
	S.tags = { "q", "scrollview", "policy", "ui" }
	S.description = "Scroll View Accessibility Policy: rules enforcing contrast, target size and motion limits for the Scroll View subsystem."
	S.params = {
		backlogLimit = 45,
		baseRadius = 520,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.scrollview.accessibility_policy", algorithm = "edf", starvationGuard = 61 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.scrollview.*", function(payload) inst.lastSignal = payload end)
			end
