-- ARKHER SYSTEM H.0038 :: Kinematic Body Simulation Policy
-- Category H - PHYSICS
...
return function(A)
	...
	local S = {}
	S.id = "H.0038"
	S.key = "arkher.physics.kinematicbody.simulation_policy"
	S.name = "Kinematic Body Simulation Policy"
	S.category = "H"
	S.family = "PHYSICS"
	S.area = "Kinematic Body"
	S.aspect = "Simulation Policy"
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.physics.kinematicbody.budget_governor" }
	S.tags = { "h", "kinematicbody", "policy", "physics" }
	S.description = "Kinematic Body Simulation Policy: device tier rules for substeps, iterations and sleep for the Kinematic Body subsystem."
	S.params = {
		backlogLimit = 36,
		baseRadius = 480,
		baseWeight = 0.98,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.physics.kinematicbody.simulation_policy", algorithm = "roundrobin", starvationGuard = 60 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.physics.kinematicbody.*", function(payload) inst.lastSignal = payload end)
			end
