-- ARKHER SYSTEM VA.0388 :: Memory Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VA.0388"
	S.key = "arkher.contworld.memorycont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.contworld.memorycont.sparse_residency" }
	S.tags = { "va", "memorycont", "multiscale", "contworld" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.contworld.memorycont.multi-scale_fidelity", levels = 6, baseTriangles = 120000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.contworld.memorycont.*", function(payload) inst.lastSignal = payload end)
			end
