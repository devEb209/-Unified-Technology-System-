-- ARKHER SYSTEM I.0227 :: Root Motion Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0227"
	S.key = "arkher.anim.rootmotion.retarget_graph"
	S.name = "Root Motion Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Root Motion"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.rootmotion.physical_blend" }
	S.tags = { "i", "rootmotion", "graph", "anim" }
	S.description = "Root Motion Retarget Graph: bone mapping and retarget path resolution for the Root Motion subsystem."
	S.params = {
		backlogLimit = 22,
		baseRadius = 560,
		baseWeight = 0.84,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.rootmotion.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.rootmotion.*", function(payload) inst.lastSignal = payload end)
			end
