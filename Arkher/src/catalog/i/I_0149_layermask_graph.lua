-- ARKHER SYSTEM I.0149 :: Layer Mask Retarget Graph
...
return function(A)
	...
	local S = {}
	S.id = "I.0149"
	S.key = "arkher.anim.layermask.retarget_graph"
	S.name = "Layer Mask Retarget Graph"
	S.category = "I"
	S.family = "ANIMATION"
	S.area = "Layer Mask"
	...
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.anim.layermask.physical_blend" }
	S.tags = { "i", "layermask", "graph", "anim" }
	S.description = "Layer Mask Retarget Graph: bone mapping and retarget path resolution for the Layer Mask subsystem."
	S.params = {
		backlogLimit = 26,
		baseRadius = 560,
		baseWeight = 0.58,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.anim.layermask.retarget_graph", directed = true })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.anim.layermask.*", function(payload) inst.lastSignal = payload end)
			end
