-- ARKHER SYSTEM G.0475 :: Training Data Synthesis Sampling Graph
...
return function(A)
	...
	local S = {}
	S.id = "G.0475"
	S.key = "arkher.neural.datasynth.sampling_graph"
	S.name = "Training Data Synthesis Sampling Graph"
	S.category = "G"
	S.family = "NEURAL / RECONSTRUCTION"
	S.area = "Training Data Synthesis"
	S.aspect = "Sampling Graph"
	S.kit = "graph"
	S.version = "1.0.0"
	S.deps = { "arkher.neural.datasynth.training_ledger" }
	S.tags = { "g", "datasynth", "graph", "neural" }
	S.description = "Training Data Synthesis Sampling Graph: dependency graph of sampling stages for the Training Data Synthesis subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 280,
		baseWeight = 0.67,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("graph", { id = "arkher.neural.datasynth.sampling_graph", directed = false })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.hotNodes(limit)
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.neural.datasynth.*", function(payload) inst.lastSignal = payload end)
			end
