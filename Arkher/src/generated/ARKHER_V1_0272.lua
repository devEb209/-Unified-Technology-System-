...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="Quixel Bridge — CORE Mixer #0272", size=Vector2.new(680, 460), pos=UDim2.fromOffset(436, 284), parent=parent, icon="⬢"})
    win.Root.Name="ARKHER_V1_0272"
    win.Root.Visible=false
    local content=win.Content
    content.BackgroundColor3=Theme.tokens.void
    -- accent line per UI
    local accent=Instance.new("Frame"); accent.Size=UDim2.new(1,0,0,2); accent.BackgroundColor3=Theme.categoryColor("Quixel B"); accent.BorderSizePixel=0; accent.Parent=content
...
Components.Checkbox(list, "Enable Feature", true, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_F"..272, v) end end)
...
Components.Slider(list, "Intensity", 0,2,1, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_I"..272, v) end end)
Components.Dropdown(list, "Quality", {"Low","Medium","High","Ultra"}, "High", function() end)
Components.TextField(list, "Tag", "ARKHER_"..272, function(v) local s=game:GetService("Selection"):Get()[1]; if s then s:SetAttribute("ARKHER_Tag", v) end end)
