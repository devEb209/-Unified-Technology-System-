#!/usr/bin/env python3
"""GENESIS V1.58 SPECIALIZED 10u — +10 únicos 21/33 (216/332) sem parar"""
...
def patch58(roots):
    items=[
        ("ARKHER_Import","IMPORT — FBX  •  glTF",0x1B5E20,"FBX  •  glTF  •  USD","FBX glTF USD  •  Scale 100"),
        ("ARKHER_Export","EXPORT — FBX  •  USD",0x2E7D32,"FBX  •  USD  •  Alembic","FBX USD Alembic  •  Export"),
        ("ARKHER_Convert","CONVERT — ReMesh  •  LOD",0x37474F,"ReMesh  •  LOD  •  Quad","ReMesh LOD Quad  •  Decimate"),
        ("ARKHER_Optimize","OPTIMIZE — Merge  •  Atlas",0x33691E,"Merge  •  Atlas  •  Batch","Merge Atlas Batch  •  1800 draw"),
        ("ARKHER_Validate","VALIDATE — Check  •  Fix",0xAD1457,"Check  •  Fix  •  Report","Check Fix Report  •  0 errors"),
        ("ARKHER_Preview2","PREVIEW2 — Thumb  •  Turntable",0x01579B,"Thumb  •  Turntable  •  Light","Thumb Turntable Light  •  360°"),
        ("ARKHER_Capture","CAPTURE — Screen  •  Video",0x4A148C,"Screen  •  Video  •  GIF","Screen Video GIF  •  4K"),
        ("ARKHER_Record2","RECORD2 — Replay  •  Take",0xB71C1C,"Replay  •  Take  •  Slate","Replay Take Slate  •  Take 1"),
        ("ARKHER_Play2","PLAY2 — Test  •  PIE",0x00838F,"Test  •  PIE  •  Standalone","Test PIE Standalone  •  Play"),
        ("ARKHER_Debug2","DEBUG2 — Break  •  Watch",0x212121,"Break  •  Watch  •  Callstack","Break Watch Callstack  •  F9"),
    ]
...
for _,n in ipairs({"ARKHER_Import","ARKHER_Export","ARKHER_Convert","ARKHER_Optimize","ARKHER_Validate","ARKHER_Preview2","ARKHER_Capture","ARKHER_Record2","ARKHER_Play2","ARKHER_Debug2"}) do
...
print("[V1.58 10x] Import/Export/Convert/Optimize/Validate/Preview2/Capture/Record2/Play2/Debug2 216/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_58",{"Source":(T_STRING, FUNC58)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch57(roots)
    roots=patch58(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_58.rbxl")
    write(out, serialize(roots))
    print(f"V1.58 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_58.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.58 done")
