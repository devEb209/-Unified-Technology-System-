#!/usr/bin/env python3
"""GENESIS V1.46 SPECIALIZED 10i — +10 únicos 9/33 (96/332) sem parar"""
...
def patch46(roots):
    items=[
        ("ARKHER_Ocean2","OCEAN — Gerstner  •  FFT",0x006064,"Wave  •  Spectrum  •  Wake","Gerstner  •  FFT Spectrum  •  Wake"),
        ("ARKHER_River","RIVER — Spline  •  Flow",0x0277BD,"Spline  •  Width  •  Velocity","Spline  •  Width  •  Velocity  •  Foam"),
        ("ARKHER_Lake","LAKE — Volume  •  Shore",0x01579B,"Shore  •  Depth  •  Wave","Shore  •  Depth  •  Wave  •  Transparency"),
        ("ARKHER_Biome","BIOME — Climate  •  Blend",0x33691E,"Temp  •  Humid  •  Altitude","Temp Humid Altitude  •  Blend 3 biomes"),
        ("ARKHER_Scatter","SCATTER — Instance  •  Density",0x2E7D32,"Density  •  Scale  •  Random","Density 500k  •  Scale  •  Random"),
        ("ARKHER_ProcMesh","PROCMESH — Gen  •  Boolean",0x37474F,"Gen  •  Boolean  •  Bevel","Gen  •  Boolean  •  Bevel  •  UV"),
        ("ARKHER_Spline","SPLINE — Path  •  Rail",0x5D4037,"Point  •  Tangent  •  Closed","Point Tangent Closed  •  Rail"),
        ("ARKHER_Road","ROAD — Lane  •  Mark",0x424242,"Lane  •  Mark  •  Sidewalk","Lane Mark Sidewalk  •  Intersection"),
        ("ARKHER_Bridge","BRIDGE — Pillar  •  Cable",0x3E2723,"Span  •  Pillar  •  Cable","Span Pillar Cable  •  LOD"),
        ("ARKHER_Tunnel","TUNNEL — Bore  •  Portal",0x212121,"Bore  •  Portal  •  Light","Bore Portal Light  •  Ventilation"),
    ]
...
for _,n in ipairs({"ARKHER_Ocean2","ARKHER_River","ARKHER_Lake","ARKHER_Biome","ARKHER_Scatter","ARKHER_ProcMesh","ARKHER_Spline","ARKHER_Road","ARKHER_Bridge","ARKHER_Tunnel"}) do
...
print("[V1.46 10x] Ocean2/River/Lake/Biome/Scatter/ProcMesh/Spline/Road/Bridge/Tunnel 96/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_46",{"Source":(T_STRING, FUNC46)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch45(roots)
    roots=patch46(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_46.rbxl")
    write(out, serialize(roots))
    print(f"V1.46 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_46.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.46 done")
