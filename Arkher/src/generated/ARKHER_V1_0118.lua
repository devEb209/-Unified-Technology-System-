--!strict
-- WorldMachine Tiled — CORE Map #0118
...
function M.new(parent: Instance)
    local win=BaseWindow.new({title="WorldMachine Tiled — CORE Map #0118", size=Vector2.new(640, 480), pos=UDim2.fromOffset(434, 266), parent=parent, icon="⬡"})
...
Components.Slider(props, "Value", -5,5,0, function(v) local s=game:GetService("Selection"):Get()[1]; if s and s:IsA("BasePart") then s:SetAttribute("ARKHER_T"..118, v) end end)
