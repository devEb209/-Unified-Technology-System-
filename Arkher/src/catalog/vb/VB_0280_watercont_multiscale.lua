-- ARKHER SYSTEM VB.0280 :: Water Continuum Multi-Scale Fidelity
...
return function(A)
	...
	local S = {}
	S.id = "VB.0280"
	S.key = "arkher.conttime.watercont.multi-scale_fidelity"
	...
	S.kit = "multiscale"
	S.version = "1.0.0"
	S.deps = { "arkher.conttime.watercont.sparse_residency" }
	S.tags = { "vb", "watercont", "multiscale", "conttime" }
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("multiscale", { id = "arkher.conttime.watercont.multi-scale_fidelity", levels = 4, baseTriangles = 105000 })
		inst.system = S
		inst.ctx = ctx
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.conttime.watercont.*", function(payload) inst.lastSignal = payload end)
			end
