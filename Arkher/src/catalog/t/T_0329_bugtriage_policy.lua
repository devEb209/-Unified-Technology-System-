-- ARKHER SYSTEM T.0329 :: Bug Triage Decision Policy
...
return function(A)
	...
	local S = {}
	S.id = "T.0329"
	S.key = "arkher.ai.bugtriage.decision_policy"
	S.name = "Bug Triage Decision Policy"
	S.category = "T"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ai.bugtriage.reasoning_graph" }
	S.tags = { "t", "bugtriage", "policy", "ai" }
	S.description = "Bug Triage Decision Policy: ordered rules deciding what the agent does next for the Bug Triage subsystem."
	S.params = {
		backlogLimit = 24,
		baseRadius = 160,
		baseWeight = 0.76,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ai.bugtriage.decision_policy", algorithm = "roundrobin", starvationGuard = 40 })
		inst.system = S
		inst.ctx = ctx
