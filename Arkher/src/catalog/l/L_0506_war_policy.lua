-- ARKHER SYSTEM L.0506 :: War Fidelity Policy
...
return function(A)
	...
	local S = {}
	S.id = "L.0506"
	S.key = "arkher.sim.war.fidelity_policy"
	S.name = "War Fidelity Policy"
	S.category = "L"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.sim.war.streaming" }
	S.tags = { "l", "war", "policy", "sim" }
	S.description = "War Fidelity Policy: rules that pick full, cohort or statistical simulation for the War subsystem."
	S.params = {
		backlogLimit = 17,
		baseRadius = 200,
		baseWeight = 0.89,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.sim.war.fidelity_policy", algorithm = "edf", starvationGuard = 41 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.sim.war.*", function(payload) inst.lastSignal = payload end)
			end
