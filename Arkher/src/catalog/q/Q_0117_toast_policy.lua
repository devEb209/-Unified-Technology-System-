-- ARKHER SYSTEM Q.0117 :: Toast Accessibility Policy
-- Category Q - UI / UX
...
return function(A)
	...
	local S = {}
	S.id = "Q.0117"
	S.key = "arkher.ui.toast.accessibility_policy"
	S.name = "Toast Accessibility Policy"
	S.category = "Q"
	S.family = "UI / UX"
	S.area = "Toast"
	...
	S.kit = "policy"
	S.version = "1.0.0"
	S.deps = { "arkher.ui.toast.interaction_ledger" }
	S.tags = { "q", "toast", "policy", "ui" }
	S.description = "Toast Accessibility Policy: rules enforcing contrast, target size and motion limits for the Toast subsystem."
	S.params = {
		backlogLimit = 35,
		baseRadius = 600,
		baseWeight = 0.77,
	...
	function S.create(ctx)
		ctx = ctx or {}
		local inst = Kits.create("policy", { id = "arkher.ui.toast.accessibility_policy", algorithm = "lottery", starvationGuard = 27 })
		inst.system = S
		inst.ctx = ctx
	...
	function inst.backlogged() return inst.pending() > S.params.backlogLimit end
		...
		function inst.integrate(engine)
			...
			inst.engine = engine
			if engine.bus then
				engine.bus:subscribe("arkher.ui.toast.*", function(payload) inst.lastSignal = payload end)
			end
