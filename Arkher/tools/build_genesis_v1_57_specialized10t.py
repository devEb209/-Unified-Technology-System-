#!/usr/bin/env python3
"""GENESIS V1.57 SPECIALIZED 10t — +10 únicos 20/33 (206/332) sem parar"""
...
def patch57(roots):
    items=[
        ("ARKHER_Console","CONSOLE — Cmd  •  History",0x212121,"Cmd  •  History  •  Autocomplete","Cmd History Autocomplete  •  ~"),
        ("ARKHER_Command","COMMAND — Palette  •  Search",0x37474F,"Palette  •  Search  •  Recent","Palette Search Recent  •  Ctrl+Shift+P"),
        ("ARKHER_Shortcut","SHORTCUT — Bind  •  Conflict",0x01579B,"Bind  •  Conflict  •  Export","Bind Conflict Export  •  Keybindings.json"),
        ("ARKHER_Macro","MACRO — Record  •  Play",0xAD1457,"Record  •  Play  •  Loop","Record Play Loop  •  10x"),
        ("ARKHER_Plugin","PLUGIN — Install  •  Enable",0x1B5E20,"Install  •  Enable  •  Update","Install Enable Update  •  Marketplace"),
        ("ARKHER_Ext","EXT — API  •  Hook",0x2E7D32,"API  •  Hook  •  Event","API Hook Event  •  arkher.api"),
        ("ARKHER_Theme","THEME — Color  •  Font",0x4A148C,"Color  •  Font  •  Icon","Color Font Icon  •  Cyber-blue"),
        ("ARKHER_Layout","LAYOUT — Dock  •  Save",0x00838F,"Dock  •  Save  •  Reset","Dock Save Reset  •  Layout.json"),
        ("ARKHER_Workspace2","WORKSPACE2 — Project  •  Recent",0x33691E,"Project  •  Recent  •  Pin","Project Recent Pin  •  5 recent"),
        ("ARKHER_Asset2","ASSET2 — Browser  •  Preview",0x5D4037,"Browser  •  Preview  •  Filter","Browser Preview Filter  •  10k assets"),
    ]
...
for _,n in ipairs({"ARKHER_Console","ARKHER_Command","ARKHER_Shortcut","ARKHER_Macro","ARKHER_Plugin","ARKHER_Ext","ARKHER_Theme","ARKHER_Layout","ARKHER_Workspace2","ARKHER_Asset2"}) do
...
print("[V1.57 10x] Console/Command/Shortcut/Macro/Plugin/Ext/Theme/Layout/Workspace2/Asset2 206/332")
"""
    for r in roots:
        if r.cls=="StarterPlayer":
            for f in r.children:
                if f.name=="StarterPlayerScripts":
                    f.add(Inst("LocalScript","ARKHER_Specialized_57",{"Source":(T_STRING, FUNC57)}))
    return roots
if __name__=="__main__":
    ...
    roots=patch56(roots)
    roots=patch57(roots)
    out=os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_57.rbxl")
    write(out, serialize(roots))
    print(f"V1.57 10x {out} ({os.path.getsize(out)/1048576:.2f} MB)")
    subprocess.run(["python3","tools/validate_rbxm.py",out], cwd=ROOT)
    from build_complete import arkher_installer_folder
    write(os.path.join(REL,"ARKHER_STUDIO_1_GENESIS_EDITION_V1_57.rbxm"), serialize([arkher_installer_folder()]))
    print("rbxm V1.57 done")
